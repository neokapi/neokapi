...
const TABS = [
  {
    ...
    icon: FolderOpen,
    description: 'Work with projects for organized, repeatable workflows, or go ad-hoc on individual files. Same project config works across Desktop and CLI.',
    screen: ProjectsScreen,
  },
  {
    label: 'Tools & Flows',
    icon: Play,
    description: 'Execute individual tools like ai-translate, pseudo-translate, or word-count. Compose them into flows and run with one click.',
    screen: ToolsScreen,
  },
  {
    label: 'Formats & Settings',
    icon: Wrench,
    description: 'Configure and tweak format readers and writers. Adjust parameters, encoding, segmentation rules, and output options.',
    screen: FormatsScreen,
  },
  {
    label: 'TM & Termbases',
    icon: BookOpen,
    description: 'Manage translation memories and terminology databases. Import, export, browse entries, and configure matching thresholds.',
    screen: TmScreen,
  },
]
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between px-1 pb-2">
        <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">Recent Projects</span>
        <span className="rounded bg-brand-500/15 px-2 py-0.5 text-[10px] font-medium text-brand-400">+ New</span>
      </div>
      {projects.map((p) => (
        <div key={p.name} className="flex items-center justify-between rounded-lg bg-surface-800/60 px-3 py-2.5 border border-surface-700/30">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={cn(
              ...
              p.status === 'active' ? 'bg-brand-500/15 text-brand-400' : 'bg-surface-600/30 text-neutral-500'
            )}>{p.status}</span>
          </div>
        </div>
      ))}
      <div className="mt-2 rounded-lg border border-dashed border-surface-600 bg-surface-800/20 px-3 py-2.5">
        <div className="flex items-center gap-2 text-xs text-neutral-400">
          ...
          <span>Or drag files here for <span className="text-brand-400">ad-hoc</span> processing</span>
        </div>
      </div>
...
function ToolsScreen() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider px-1">Tools</span>
        <div className="mt-2 space-y-1.5">
          {tools.map((t) => (
            <div key={t.name} className="flex items-center justify-between rounded-lg bg-surface-800/40 border border-surface-700/30 px-3 py-2">
              <div className="flex items-center gap-2">
                <code className={cn('text-xs font-medium', t.accent)}>{t.name}</code>
                <span className="text-[11px] text-neutral-600">{t.desc}</span>
              </div>
              <button className="rounded bg-surface-700/50 px-2 py-0.5 text-[10px] text-neutral-400 hover:bg-surface-600/50 hover:text-neutral-300 transition">run</button>
            </div>
          ))}
        </div>
      </div>
      <div>
        <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider px-1">Flows</span>
        <div className="mt-2 space-y-1.5">
          {flows.map((f) => (
            <div key={f.name} className="flex items-center justify-between rounded-lg bg-surface-800/40 border border-surface-700/30 px-3 py-2">
              <div>
              ...
              <button className="rounded bg-brand-500/15 px-2 py-0.5 text-[10px] font-medium text-brand-400 hover:bg-brand-500/25 transition">
                <Play className="inline h-2.5 w-2.5 mr-0.5" />run
              </button>
            </div>
          ))}
        </div>
...
function FormatsScreen() {
  ...
  return (
    <div className="space-y-2">
      <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider px-1">Format Configuration</span>
      <div className="mt-2 space-y-1.5">
        {formats.map((f) => (
          <div key={f.name} className="rounded-lg bg-surface-800/40 border border-surface-700/30 px-3 py-2.5">
    ...
    <div className="space-y-3">
      <div>
        <div className="flex items-center justify-between px-1 pb-1">
          <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">Resources</span>
          <span className="rounded bg-brand-500/15 px-2 py-0.5 text-[10px] font-medium text-brand-400">+ Import</span>
        </div>
        <div className="space-y-1.5 mt-1">
          {tms.map((tm) => (
            <div key={tm.name} className="flex items-center justify-between rounded-lg bg-surface-800/40 border border-surface-700/30 px-3 py-2">
              <div className="flex items-center gap-2">
                <span className={cn(
                  'rounded px-1.5 py-0.5 text-[9px] font-bold',
                  tm.type === 'TM' ? 'bg-forest-500/15 text-forest-400' : 'bg-accent-amber/15 text-accent-amber'
                )}>{tm.type}</span>
                <div>
                  <code className="text-xs text-neutral-200">{tm.name}</code>
                  <div className="text-[10px] text-neutral-600">{tm.entries} entries &middot; {tm.langs}</div>
                </div>
              </div>
            </div>
          ))}
      ...
      <div>
        <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider px-1">Recent Matches</span>
        <div className="space-y-1 mt-2">
          {recentEntries.map((e, i) => (
            <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2 rounded-lg bg-surface-800/40 border border-surface-700/30 px-3 py-1.5">
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-16 text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-lg text-neutral-400">
            Everything the CLI does, with a visual interface. Work ad-hoc or with projects &mdash; shared across Desktop and CLI.
          </p>
        </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-surface-700/50 bg-surface-900/60 shadow-2xl glow-teal order-2 lg:order-1">
            ...
            <div className="flex items-center justify-between border-b border-surface-700/30 px-4 py-2.5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-display text-xs text-neutral-500 font-medium">Kapi Desktop</span>
              </div>
              <div className="w-16" />
            </div>
