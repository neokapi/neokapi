---
id: placeholder-check
title: 'Placeholder Check tool'
description: 'Verify interpolation placeholders and numbered tags are preserved in the target.'
sidebar_label: 'Placeholder Check'
slug: /reference/tools/placeholder-check
hide_table_of_contents: true
---

{/*
  GENERATED FILE — DO NOT EDIT.
  Produced by web/scripts/gen-reference-pages.ts from @neokapi/reference-data.
  To change this page, edit the data (regenerate with `make generate-reference-docs`)
  or the page component, then rerun `make generate-reference-pages`.
*/}

import ToolReferencePage from "@site/src/components/reference/pages/ToolReferencePage";

# ▒ Þļàçéĥöļđéŕ Çĥéçķ ţööļ ▒

<ToolReferencePage id={"placeholder-check"} source={"built-in"} />
