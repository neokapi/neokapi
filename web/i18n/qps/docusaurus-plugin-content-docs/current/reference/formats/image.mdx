---
id: image
title: 'Image format'
description: 'Translate raster images — emit the picture as a translatable Media asset, recognize embedded text with OCR, and translate alt-text/caption sidecars.'
sidebar_label: 'Image'
slug: /reference/formats/image
hide_table_of_contents: true
---

{/*
  GENERATED FILE — DO NOT EDIT.
  Produced by web/scripts/gen-reference-pages.ts from @neokapi/reference-data.
  To change this page, edit the data (regenerate with `make generate-reference-docs`)
  or the page component, then rerun `make generate-reference-pages`.
*/}

import FormatReferencePage from "@site/src/components/reference/pages/FormatReferencePage";

# ▒ Îḿàĝé ƒöŕḿàţ (.þñĝ, .ĵþĝ, .ĵþéĝ, .ĝîƒ, .ƃḿþ, .ţîƒ, .ţîƒƒ, .ŵéƃþ, .ĥéîç, .ĥéîƒ, .àṽîƒ) ▒

<FormatReferencePage id={"image"} />
