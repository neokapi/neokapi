...
`context/memory/*.memory.json`. This exercises the paid product — the review loop, the
...
`kapi extract/merge` → a refreshed `context/memory/*.memory.json` commit. The nightly job
