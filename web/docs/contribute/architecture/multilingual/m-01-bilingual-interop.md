...
that loop — it pre-fills on extract and absorbs on merge — which is what makes
...
engineer reading `kapi --help` sees them beside `run`, and their flag shapes —
a repeatable `-i`, a collection filter, a conflict policy — do not fit a flow
step's config.
...
`kapi.yaml` recipe ([C-01](../context/c-01-project-model.md)). `merge` requires
a project; without one it says so rather than guessing.
...
- `kapi merge` applies a **translator's returned targets** and, by default,
  absorbs every accepted target into the content memory. That accretion is the
  point of merge; it is governed by the conflict policy and stale-segment
...
- [E-02: The format system](../engine/e-02-format-system.md) — the XLIFF, PO and skeleton machinery the carriers ride on
- [M-02: Segmentation](m-02-segmentation.md) — the overlay a segment span comes from
- [M-06: Content packages](m-06-content-packages.md) — the `.kpz` container and its bilingual profile
