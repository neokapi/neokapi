...
follow, and answering them by accident produces one defect wearing three hats:
...
cannot emit translations for is not a strategy; it is a truncated reply.
...
output limit, with a backstop on the number of blocks. That backstop is 64 — a
guard rail, not the decision, because the token budget binds first on anything
...
So truncation is **carried, not inferred**. Every provider reads its API's
...
honest boundary, and it is the one documented.
...
roughly constant and generating every block distinct — duplicate source text
...
cap — not a model limit, but the consequence of holding a synchronous HTTP
connection open for minutes. A batch whose reply would exceed it comes back
...
An oversized batch is not dangerous; it is wasteful, and silently so.
...
- **Memory matches and prior translations of the same key do not reach the
  prompt.** The evidence says they would help, and kapi already holds them.
