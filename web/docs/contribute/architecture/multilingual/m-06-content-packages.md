...
test fixture — and, critically, a lossy serialization cannot faithfully
**regenerate the derived stores** a runtime builds from this content.
...
- The **terms bundle** reuses the already-JSON-tagged concept type directly —
  one source of truth, no parallel wire type to drift — and so preserves every
  native field by construction.
...
limited to one per store. Terms is the exception that proves it: a project has
exactly one set of terms, so it gets a conventional location
([C-08](../context/c-08-terms.md)). Content memory gets none, because a project
accumulates a bundle per content surface and a fallback would have nothing
single to name.
