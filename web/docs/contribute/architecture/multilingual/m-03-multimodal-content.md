...
one pattern, not three. A **fast local extractor** turns raw media into blocks,
each anchored back to a *slice* of the source and carrying a per-unit
...
- **`kapi-av`** bundles the per-platform demux binaries. Unlike the other two it
  ...
  find the bundled binaries, so the plugin's job is to be discoverable.
...
## Video is composition, not new code
