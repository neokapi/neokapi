...
id: 019-neokapi-i18n
sidebar_position: 19
title: "AD-019: neokapi-i18n extraction model"
description: "Architecture decision: neokapi-i18n extracts translatable content from React/JSX source at Vite build time, producing Block records whose Source is a typed Run[] — the same content model as the rest of the framework."
keywords: [neokapi-i18n, extraction model, JSX, Vite plugin, Block, Run, architecture decision]
---

# AD-019: neokapi-i18n extraction model
...
positions when rendering translations. A lint package (`i18n-react-lint`)
...
neokapi-i18n addresses both: an SWC-based AST walker extracts translatable
...
neokapi-i18n is the first first-party extractor to apply this convention to
JSX.
...
A SWC-AST walker (`packages/i18n-react/src/extract/walker.ts`) descends
...
(`packages/i18n-react/src/plugin/defaults.ts`). For each translatable
...
`__tx` (`packages/i18n-react/src/runtime/index.ts`) resolves the hash to
...
`@neokapi/i18n-react-lint` is a source-authoring ESLint/oxlint plugin.
...
`recommended-strict` configs (`packages/i18n-react-lint/src/configs/`).

The rules (`packages/i18n-react-lint/src/rules/`) flag patterns that
...
neokapi-i18n CLI (`packages/i18n-react/src/cli.ts`) routes those through
...
- **Lint keeps source extractable.** `@neokapi/i18n-react-lint` flags
...
- **Framework convention extends to JSX.** neokapi-i18n uses the same
