...
(`graph_nodes` / `graph_edges`) relating them. One file, one connection pool, one
migration ledger per subsystem.
...
holds withheld originals. Those are local-only by design — never committed, never
sent anywhere — so nothing else has a copy, and losing them is not a rebuild but
a loss.
