...
search-and-replace — living under `core/redaction/`, two pipeline tools
...
`vault/` is separate from `cache/` on purpose, and the distinction is
load-bearing. **The cache is defined by being disposable** — losing it costs CPU.
**The vault is defined by an exclusion**: a named destination must never read it,
and losing it means redacted content can never be restored. Filing it under
`cache/` would make it look regenerable, which it is not, and put it one deletion
away from unrecoverable placeholders. So `rm -rf .kapi/work/cache` stays free, and
`vault/` is the one thing under `work/` that kapi never deletes on its own
initiative.
...
overlays onto the redacted runs in one pass: a term annotation from an upstream
tool follows the rewrite and still reaches downstream steps, while a span
overlapping a redacted span, including the consumed entity spans, is dropped. The
applier vaults the originals as it replaces, so source rewrite and secret capture
are atomic.
...
1. **`kapi extract`** resolves and applies it, emitting a redacted bilingual file
   ...
   the content memory is queried with — and pre-fills targets from — redacted
   text, and no sensitive value reaches the emitted file by way of a memory
   match. On `kapi merge` the incoming source is always restored, so per-block
   staleness compares original against original; the target is restored unless
   `--no-restore` is set.
2. **Ingest**. `host.ProjectRedaction` resolves the project's policy and
...
Redaction today is binary at each reader: a span is either withheld or it is not.
The design it points at is **clearance** — governance at a point, over a ladder of
