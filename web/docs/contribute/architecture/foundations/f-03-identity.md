...
predictable IDs — worse than crashing. This matches the standard library's own
convention.
...
exists. The fourth is honest rather than clever: when both signals change, nothing
links the block to its predecessor, and a guess would attach a real decision to the
wrong words.
...
hash is scoped to the document, via `reconcile.Identify(scope, block)`. Content is
...
they were and are consumed as the context signal. That is the point of putting
...
That key is **not** the bare content hash, and it is not the reader's block ID
either. Readers assign file-local IDs that restart in every source file, so keying
