...
compiling while the fields they disagree about quietly vanish. The remedy is not
review discipline but a single definition plus a test that fails when a second one
appears.
...
Two model dimensions deliberately do not cross this schema: target variant tone
and channel (targets are keyed by locale in `TargetEntry`), and per-target status,
origin, and score. A protocol that needs them carries them in its own envelope
properties rather than widening the canonical messages.
...
- deterministic output — protojson deliberately randomizes its whitespace, so both
...
Separately-licensed layers may define their own envelopes around canonical
payloads. Those are envelopes, not redefinitions: they add scoping, hashes, and
escapes, and delegate every run and segment conversion to `protoconvert`.
