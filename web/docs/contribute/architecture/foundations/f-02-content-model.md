...
tone or channel — not bare locale-keyed strings.
...
A target is a **first-class record**, not a bare run sequence, and it is keyed by
a **variant** rather than a bare locale:
...
content hash. How the pair is graded to decide what carries over between reads,
and how the durable per-project key is derived, is
[F-03: Identity](f-03-identity.md).
...
roles, and the form-field family — not a title/heading/caption shortlist. A tool
reads only the facet it needs; a Block with no media facet is plain
anchored-by-runs text.
...
was produced?" and nothing more. It is deliberately narrower than an engine's
...
A Block's content is not a string with embedded markers — it is a flat sequence of
`Run` values, held directly on the Block:
