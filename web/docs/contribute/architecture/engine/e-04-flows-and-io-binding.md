...
project or `.kpz` and emits no file; materialization is a separate, later sink
...
**Transformers** ([E-03](e-03-tool-system.md)) come in two kinds: *idempotent
...
A pipeline runs at many origins and destinations. The same translation flow
...
([E-01](e-01-processing-engine.md)), and it is never required to run one tool.
...
a small, separate **binding** vocabulary (`core/flow.BindingKind`), resolved from
invocation context:
...
binding is the default, so an unqualified `kapi run <flow> -i file.json -o
out.json` is `source: file`, `sink: file`. A `.kpz` workspace is `source: store`;
`merge` is `source: store` with `sink: file`. One binder interface backs them
all, so the engine never special-cases an origin.
...
exists, anchored to the current block hashes
([M-06](../multilingual/m-06-content-packages.md)). The store *is* the workspace.

### 4. Transformers: settlers and brackets

Transformers ([E-03](e-03-tool-system.md)) are ordinary ordered steps; the
...
in order: an explicit CLI flag, the project or `.kpz` context, the flow's
...
would otherwise read as a scheme, and any unrecognised `prefix:` is treated as
part of a path, so a Windows drive letter never parses as a scheme. Each example
below shows the resolved `source → sink`:
...
**In a project, a run lands in the store.** When a `kapi.yaml` recipe is in
scope, a run with no explicit sink commits its work as overlays to the project
block store and emits no document. Materializing the per-locale files is a
separate, explicit step (`kapi merge`). The store is the working copy: a re-run
reuses the overlays already present and recomputes only what changed
...
reader and writer, not flattened into a parent document.
...
random access (central directory plus seeks) and streams a TAR or TAR.GZ; it
visits one entry at a time, materialises an entry's bytes only when the processor
...
container edits each eligible entry and repacks. Without an in-place flag, a
single-entry edit prints the edited entry and a whole-container edit streams the
repacked archive to stdout ([S-04](../surfaces/s-04-toolbox.md)).
...
([E-05](e-05-plugin-system.md)); the local file and git connectors are the
filesystem instance of the same idea.
...
1. **The sink is rarely byte-exact and rarely atomic.** A filesystem archive is
...
So a remote connector is the *provider* family of the same source/sink
abstraction: it reuses the enumerate → process → write-back contract and the
...
- The flow editor surfaces source and sink as **endpoint pickers** (file · store
  · interchange · none) rather than reader/writer nodes; transformer placement
  ([E-03](e-03-tool-system.md)) and overlay capability
  ([F-02](../foundations/f-02-content-model.md)) are independent of bindings.
...
- An archive is a fan-out over the `file` binding, not a format: per-entry
  ...
  *inside* an archive round-trips faithfully), and a barrier sink repacks,
  ...
  inspection face.
- The same enumerate → process → write-back shape extends to remote connectors
