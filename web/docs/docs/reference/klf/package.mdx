...
- **`.klz` as an ad-hoc workspace (`extract` / transform / `merge`).** No project
  required — the `.klz` *is* the workspace. `extract` ingests sources, running a
  tool or flow on the `.klz` transforms it in place, and `merge` emits the
  finished files. A small **recipe** (target locales + output layout) travels with
  the file, so `merge` needs no flags.
  ...
  Transforming reuses the work already in the package instead of recomputing it,
  so the expensive steps don't re-run, and each document's work stays isolated.
