...
├── source/<name>          original input document(s) (for run-written packages)
...
- **`.klz` as run I/O (ad-hoc).** Any tool or flow writes a `.klz` when the output
  has a `.klz` extension, and resumes when the input is one. No project required —
  the `.klz` *is* the workspace.

  ```bash
  kapi pseudo-translate app.json -o work.klz   # write the in-progress package
  #   ... later, or on another machine ...
  kapi pseudo-translate work.klz  -o app.fr.json  # resume → finished file
  ```

  Resuming reuses the work in the package instead of recomputing it, so the
  output is byte-identical to a one-shot run and the expensive steps don't re-run.
  Several inputs pack into one `.klz`; resume writes them back with an `-o`
  template, each document's work kept isolated.
...
`(kind, blockHash)`, alongside the `source/<name>` documents a run-written package
carries so it can be resumed. Overlays in a multi-document package are scoped per
source, since block ids are only unique within one document.
