...
Several commands below carry a **▸ Run** button. Pressing it runs the **real
`kapi` binary** right here in your browser — no install, and nothing leaves your
machine. The commands that need an API key or local files are shown as static
blocks; run those after
[installing](/get-started/installation).

## Keep AI output on-brand

Print a brand voice guide you can paste into your AI assistant's context, or
inject automatically via [MCP](/cli/mcp). Start from a built-in starter
pack:
...
See [Using Kapi with AI Assistants](/cli/mcp) for Claude Code, Cursor, and
Windsurf setup.
...
- [Features](/framework/formats) — detailed feature documentation
