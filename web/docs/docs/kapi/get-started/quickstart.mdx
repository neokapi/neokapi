...
Estimate translation volume for cost planning — drop a file (or try a sample) to
see the translatable blocks and source-word count right here:

<StatsWidget autoSampleId="json" />

From the CLI it's one command — handy for scripting and CI (`--json` for a
...
`kapi credentials add` — see [Installation](/kapi/get-started/installation). When a
