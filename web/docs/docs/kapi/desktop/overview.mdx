...
The installer is Authenticode-signed, bootstraps the Microsoft WebView2 runtime if it is missing, and registers a Start-menu entry and uninstaller. A portable `kapi-desktop-X.Y.Z-windows-<arch>.zip` is also published for unattended use.
