...
The installer is Authenticode-signed and registers a Start-menu entry and uninstaller. A portable `kapi-desktop-X.Y.Z-windows-<arch>.zip` is also published for unattended setups; unlike the installer it assumes the Microsoft WebView2 runtime is already present (standard on Windows 11 and current Windows 10).
