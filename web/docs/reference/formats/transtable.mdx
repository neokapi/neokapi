---
id: transtable
title: 'Translation Table format'
description: 'Read and write Okapi''s TransTable v1 tab-separated bilingual exchange table.'
sidebar_label: 'Translation Table'
slug: /reference/formats/transtable
hide_table_of_contents: true
---

{/*
  GENERATED FILE — DO NOT EDIT.
  Produced by web/scripts/gen-reference-pages.ts from @neokapi/reference-data.
  To change this page, edit the data (regenerate with `make generate-reference-docs`)
  or the page component, then rerun `make generate-reference-pages`.
*/}

import FormatReferencePage from "@site/src/components/reference/pages/FormatReferencePage";

# Translation Table format (.tab, .tsv)

<FormatReferencePage id={"transtable"} />
