...
`targets`. Historically it is the Kapi Bundle Format, and `kbf` remains its
package and marker name.
...
`kind` was previously `kapi-localization-format` (and `kapi-tm-format` /
`kapi-termbase-format` for the content-memory and terms bundles). Those values
are **not** accepted:
reinterpreting a file written by an earlier release would make the envelope
useless as a version marker. A stale file is still *detected* as the format it
is, so the reader can name the rename and the command that rewrites it rather
than failing as an unknown format — regenerate it with `kapi extract`,
`kapi memory export`, or `kapi terms export`.
