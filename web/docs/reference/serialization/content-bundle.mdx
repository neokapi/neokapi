...
- **Major** — a breaking change. A consumer **must reject** a file whose major
  version it does not recognize.
- **Minor** — a forward-compatible addition. A consumer **should accept** unknown
...
what lets a span cross run boundaries — a phrase running through a bold segment
is one range rather than three — and lets a boundary sit either side of a
...
anchor resolve — or fail with one of the reasons above — against a live document.
...
the Go engine and the TypeScript mirror — see the [KBF Tests page](/kbf-tests).
