...
1. **The recipe binding**: `defaults.voice` in `kapi.yaml`, naming a `profile`
   in the project's voice store, a `pack`, or a `profile_file` whose stored
   profile the import filed under that path.
...
A comment over a length limit is a major finding, and so is a change over the
