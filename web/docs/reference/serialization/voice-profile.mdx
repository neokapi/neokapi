...
A comment over a length limit is a major finding. Each limit is a positive
number of words: a profile with a zero or negative limit, or a minor sentence
limit that is not below the major one, fails to load with an error naming the
key.
