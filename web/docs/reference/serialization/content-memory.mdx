...
It is a **bundle**, not a store. The working store is SQLite — the file is
still named `tm.db`, under the gitignored `.kapi/` state directory — and a
bundle is what you get when you export that store into a single reviewable,
hashable file. See the [overview](/reference/serialization/overview) for why
kapi keeps that distinction.
...
Overlays ship in both `.overlays.json` and `.overlays.jsonl`, so the obvious
question is why content memory has no line-delimited form.
...
There is no append path to optimize for. An append-only file would defeat
...
The conditional is worth stating, because it is the thing that would change the
answer: **if content memory ever becomes an appended journal rather than an
exported snapshot, JSON Lines becomes the right serialization**, and a
`.memory.jsonl` could join the JSON form exactly as overlays already carry
both. The current shape reflects how the file is produced, not a preference for
JSON.
...
- **Fuzzy-match scores or thresholds.** Matching is a runtime concern driven by
  the recipe, not a property of stored content.
