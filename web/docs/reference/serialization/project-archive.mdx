...
Each store has exactly one native spelling, the bundle:
...
wherever it likes and may keep several. Which bundle an import reads and a
snapshot writes is set by the recipe
([`defaults.terms_source` / `defaults.memory_source`](/reference/project-file#context-files-a-recipe-names)),
and an explicit `-o` path names one outright.

### Where a bundle sits when nothing binds it

A terms bundle written for a project belongs at **`.kapi/terms.json`**, which
is where [`kapi context import`](/reference/commands/context-import) looks with
no path and where `kapi context snapshot` writes one. A bundle kept there shows
up in a diff and goes through review like any other source. The
[terms page](/reference/serialization/terms#where-an-import-looks-for-one) says
what the recipe can bind instead.

Content memory has no matching single name: **a project has one terms bundle but
many content-memory bundles**, typically one per content surface. This
repository's own dogfood keeps a bundle per surface under `.kapi/memory/`
against a single `.kapi/terms.json`, and an import reads every one of them.
