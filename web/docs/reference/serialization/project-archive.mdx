...
terminology as source. The bundle is the committed source; `.kapi/terms.db`
is the throwaway cache it compiles into. The
...
repository's own dogfood commits a bundle per surface under `context/memory/` against a
single `context/terms.json`, so a memory seed is always named rather than guessed.
