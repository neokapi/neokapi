...
Your terms bundle belongs at the **repository root**, as `terms.json` — not
under `.kapi/`. `.kapi/` is gitignored, so a terms bundle kept there would never be
...
keeps its terms bundle local and uncommitted.

Content memory has no such fallback, and that asymmetry is deliberate: **a
project has one terms bundle but many content-memory bundles**, typically one per
