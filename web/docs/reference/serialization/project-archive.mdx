...
```bash
kapi memory export -o .kapi/context/memory/cli-nb.memory.json
...
`.kapi/work/store.db` are the derived index it compiles into. The
...
repository's own dogfood commits a bundle per surface under `.kapi/context/memory/`
against a single `.kapi/context/terms.json`, so a memory seed is always named rather
than guessed.
...
- **Project snapshot (`pack` / `unpack`).** Inside a `.kapi` project the working
  ...
  `.kapi/work/store.db`, so resume there is simply running again.
