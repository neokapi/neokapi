...
terminology as source. The bundle is the committed source; `.kapi/termbase.db`
...
repository's own dogfood commits a bundle per surface under `l10n/tm/` against a
single `l10n/terms.json`, so a memory seed is always named rather than guessed.
