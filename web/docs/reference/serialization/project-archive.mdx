...
Each store has a native bundle format:
...
[`kapi context import`](/reference/commands/context-import) reads
**`.kapi/terms.json`** by default. A snapshot writes `terms.json` under the
required `--out` directory. You can commit that snapshot for review; when using
`.kapi/`, adjust its default ignore rules first. See
[Terms](/reference/serialization/terms#where-an-import-looks-for-one) for import
locations.
