...
```bash
kapi memory export -o context/memory/cli-nb.memory.json
...
separate, lossy JSON export that does not preserve concept identity. Reach for
`bundle` when you want the lossless form.
...
Your terms bundle belongs at the **repository root**, as `terms.json`. `.kapi/`
is gitignored, so a terms bundle kept there would never be committed, never show
up in a diff, and never get reviewed, which defeats the point of keeping
terminology as source. The bundle is the committed source; the terms tables in
`.kapi/store.db` are the derived index it compiles into. The
[terms page](/reference/serialization/terms#the-lookup-ladder) specifies the
ladder.
...
repository's own dogfood commits a bundle per surface under `context/memory/`
against a single `context/terms.json`, so a memory seed is always named rather
than guessed.
...
A `.kpz` packs the **source of truth**, not the regenerable index. It excludes
`.kapi/store.db` and everything under `.kapi/cache/`, which rebuild on demand,
...
- **Project snapshot (`pack` / `unpack`).** Inside a `.kapi` project the working
  ...
  `.kapi/store.db`, so resume there is simply running again.
