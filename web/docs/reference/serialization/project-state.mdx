...
sidebar_position: 7
title: Project state
description: "The committed decision record under .kapi/state/ (JSON Lines, one shard per document, one line per unit), the ledger kapi commit exports it from, and the unit state fields."
keywords:
  [
    project state,
    kapi/units,
...
## The ledger records, `kapi commit` exports
...
recorded. `kapi commit` writes the record out in the form git tracks: for each
unit this checkout holds, what was decided about the source and the translation
it holds now.

The consequences follow from that:

- The two directions agree. `kapi commit` exports the shards; opening the
  project reads them back in, so a fresh clone and a `git pull` both pick up
...
- Committing writes nothing when nothing changed, which keeps a CI step from
...
`.kapi/work/` is ignored, and that is where the checkout's derived projection
and its caches live. A record of who approved what must be reviewable and must
survive a clone; the databases that index it belong to one machine. The record
...
- [The project store](/kapi/project-store): the committed record, the two databases, and the cache.
