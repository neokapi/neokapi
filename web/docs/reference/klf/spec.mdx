...
sidebar_position: 2
title: Specification
description: The normative schema for the Kapi Localization Format (.klf) — the file envelope, the run discriminated union, per-locale targets, validation rules, and the companion .klfl annotation overlay.
keywords:
  [
    KLF,
...
This is the normative schema for the **Kapi Localization Format**. For what KLF
is and the role it plays, start with the [overview](/reference/klf/overview);
for worked examples you can edit live, see [Examples](/reference/klf/examples).
...
at [`core/klf`](https://github.com/neokapi/neokapi/tree/main/core/klf) and
...
on the [KLF Tests page](/klf-tests).
...
The [KLF anatomy page](/klf-lab) lets you edit a `.klfl` overlay and watch each
anchor resolve — or fail with one of the reasons above — against a live document.

## See also

- [Overview](/reference/klf/overview) — what KLF is and its role in the pipeline.
- [Examples](/reference/klf/examples) — worked examples of every run kind.
- [KLF vs XLIFF](/reference/klf/vs-xliff) — the mapping onto XLIFF 2.x.
