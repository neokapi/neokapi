---
id: batch-translation
title: 'Batch Translation tool'
description: 'Creates translations from an external program for a given input document. Expects: raw document. Sends back: raw document.'
sidebar_label: 'Batch Translation'
slug: /reference/tools/batch-translation
hide_table_of_contents: true
---

{/*
  GENERATED FILE — DO NOT EDIT.
  Produced by web/scripts/gen-reference-pages.ts from @neokapi/reference-data.
  To change this page, edit the data (regenerate with `make generate-reference-docs`)
  or the page component, then rerun `make generate-reference-pages`.
*/}

import ToolReferencePage from "@site/src/components/reference/pages/ToolReferencePage";

# Batch Translation tool

<ToolReferencePage id={"batch-translation"} source={"okapi"} />
