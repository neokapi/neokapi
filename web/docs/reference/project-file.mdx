...
| `tm`               | MemoryDefaults          | TM pre-fill on `kapi extract` and write-back on `kapi merge` (see [tm](#tm))                                             |
...
| `termbase_source`  | string              | Committed, git-tracked native source artifact (a `.ktb`) the project termbase is compiled from; `kapi apply` edits it and re-imports into the gitignored termbase cache, so `git diff` is the review surface |
| `tm_source`        | string              | Committed, git-tracked native source artifact (a `.kmb`) the project translation memory is compiled from — the TM analogue of `termbase_source` |
