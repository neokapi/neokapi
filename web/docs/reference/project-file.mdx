...
| `terms_source`     | string              | Committed, git-tracked native source bundle (a `.terms.json`) the project's terms are compiled from; `kapi apply` edits it and re-imports into the [project store](/kapi/project-store), so `git diff` is the review surface. Unset, kapi falls back to `<root>/terms.json`, then `<root>/.kapi/terms.json` |
