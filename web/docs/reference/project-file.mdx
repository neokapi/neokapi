...
| `terms`      | string            | A standalone terms store governing this product's content, resolved relative to the project root. Unset, the project's own store governs |
...
content. A profile's `terms:` does not: it is a path into the local project, and
...
| terms | `--termstore` → `profiles.<name>.terms` → `.kapi/profiles/<name>/terms.json` (when the file exists) → the project's own store |
