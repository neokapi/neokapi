...
| --------- | ------ | ------------------------------ |
| `version` | string | Schema version, must be `"v2"` |
...
| `channel`          | string          | The point in the context space this collection's content sits at — `profile/channel`, or a bare `channel` (see [Profiles and channels](#profiles-and-channels)) |
...
collections:
  ...
  - name: neokapi-docs
    channel: neokapi/docs # both products ship `docs`, so qualify it
    content:
      - path: "web/docs/**/*.mdx"
```
...
A collection's `channel:` is written `profile/channel`, or bare when exactly one
profile declares that channel. Two profiles declaring the same channel makes the
bare form ambiguous, and that is a load error naming both qualified spellings
rather than a silent pick. Binding a channel requires a named collection. A
