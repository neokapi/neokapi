...
| `termbase_source`  | string              | Committed, git-tracked native source artifact (a `.klftb`) the project termbase is compiled from; `kapi apply` edits it and re-imports into the gitignored termbase cache, so `git diff` is the review surface |
| `tm_source`        | string              | Committed, git-tracked native source artifact (a `.klftm`) the project translation memory is compiled from — the TM analogue of `termbase_source` |
...
Ship gates decide when localized content is *shippable*: coverage thresholds
...
```yaml
ship_gates:
  - when: { locales: [de-DE] } # contractually reviewed market
...
- [kapi.yaml Project File Format](/contribute/notes-internal/kapi-project-file) — the implementation note: the `KapiProject`, `Defaults`, and `ContentCollection` struct layouts, the named-collection content shape, validation rules, and how platform extensions decode unknown keys.
