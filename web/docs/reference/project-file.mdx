...
the same load-time validation, for the files that item matches. It is available
to per-file governance resolution; a flow run and a push resolve governance per
collection, so an item's own channel does not change what either carries.
...
The window is reported rather than enforced.
[`kapi context search`](/reference/commands/context-search) reads every bounded profile
against the current time and labels it `active`, `upcoming` or `expired`; a run
resolves the profile its collection binds either way.
