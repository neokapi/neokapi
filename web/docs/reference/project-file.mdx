...
| `tm`               | MemoryDefaults      | Content-memory pre-fill on `kapi extract` and write-back on `kapi merge` (see [tm](#tm))                                 |
...
| `termbase_source`  | string              | Committed, git-tracked native source artifact (a `.ktb`) the project terms store is compiled from; `kapi apply` edits it and re-imports into the gitignored terms cache, so `git diff` is the review surface |
| `tm_source`        | string              | Committed, git-tracked native source artifact (a `.kmb`) the project content memory is compiled from — the content-memory analogue of `termbase_source` |
| `state`            | string              | Committed, git-tracked [project state store](/kapi/project-store) (a kapi-project-state JSON document) — the authoritative carrier of per-unit workflow decisions (review ladder, approvals, parking) a plain target file can't hold. Empty defaults to `.kapi-state.json`; a server-backed project pushes state to a remote instead |
