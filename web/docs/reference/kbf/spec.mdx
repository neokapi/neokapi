...
sidebar_position: 2
title: Specification
description: The normative schema for the Kapi Bundle Format (.kbf) — the file envelope, the run discriminated union, per-locale targets, validation rules, and the companion .overlays.jsonl annotation overlay.
keywords:
  [
    KBF,
    kapi bundle format,
...
A `.kbf` file is auto-detected by its extension and by sniffing the document for
the magic string `"kapi-bundle"`. The format is implemented in Go
...
`kapi-termbase-format` for `.kmb` / `.ktb`). Those values are **not** accepted:
...
The top-level `File` object is the `.kbf` document. Field order and shape are
...
inside the `.kbf` itself. A `.overlays.jsonl` file is **JSON-Lines**: each line
