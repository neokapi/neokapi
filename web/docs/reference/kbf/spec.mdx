...
matches — live in a companion `.overlays.jsonl` file rather than inside the `.kbf` itself.
A `.overlays.jsonl` file is **JSON-Lines**: each line is one compact JSON record (no
