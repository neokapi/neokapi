...
sidebar_position: 2
title: Specification
description: The normative schema for the Kapi Bundle Format (.kbf.json) — the file envelope, the run discriminated union, per-locale targets, validation rules, and the companion .overlays.jsonl annotation overlay.
keywords:
  [
    KBF,
    kapi bundle format,
...
This is the normative schema for the **Kapi Bundle Format**. For what KBF
is and the role it plays, start with the [overview](/reference/kbf/overview);
for worked examples you can edit live, see [Examples](/reference/kbf/examples).

A `.kbf.json` file is auto-detected by its extension and by sniffing the document
for the magic string `"kapi-bundle"`. The compound suffix is deliberate: the file
is JSON, so the `.json` tail lets `jq`, GitHub, and an editor treat it as JSON,
while the `.kbf` segment still says which JSON it is. The format is implemented in Go
at [`core/kbf`](https://github.com/neokapi/neokapi/tree/main/core/kbf) and
...
- [Overview](/reference/kbf/overview) — what KBF is and its role in the pipeline.
- [Examples](/reference/kbf/examples) — worked examples of every run kind.
- [KBF vs XLIFF](/reference/kbf/vs-xliff) — the mapping onto XLIFF 2.x.
- [Content model](/framework/content-model) — the in-memory `Block`/`Run` model KBF serializes.
- [Project file](/reference/project-file) — the `kapi.yaml` recipe that drives extraction and merge.
- The reference implementations: [`core/kbf`](https://github.com/neokapi/neokapi/tree/main/core/kbf) (Go, canonical) and [`@neokapi/kapi-format`](https://github.com/neokapi/neokapi/tree/main/packages/kapi-format) (TypeScript mirror).
