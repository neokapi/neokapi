...
sidebar_position: 4
title: KBF vs XLIFF
description: How the Kapi Bundle Format maps onto XLIFF 2.2, where the two formats differ in philosophy, where XLIFF is the stronger choice, and when to reach for each.
keywords:
  [
    KBF,
    XLIFF,
...
anchors. They never touch the `.kbf` content, so re-extracting source does not
...
- [Specification](/reference/kbf/spec) — the normative KBF schema.
