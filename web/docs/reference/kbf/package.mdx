---
sidebar_position: 5
title: The Kapi format family & .kpz package
description: The Kapi format family of native lossless formats (blocks, content memory, terms) and the .kpz package that bundles a project's authoritative content — distinct from the lossy interchange formats (XLIFF/PO, TMX, TBX).
...
[block](/reference/kbf/spec#block). A project owns more than blocks, though: a
...
XLIFF play for blocks (see [KBF vs XLIFF](/reference/kbf/vs-xliff)):
...
not something anyone opens by hand.
...
contract as [KBF](/reference/kbf/spec#versioning), and a deterministic serializer
...
kapi terms export --format bundle -o glossary.terms.json   # or name it explicitly
```

The explicit spelling is `--format bundle` for both stores. It is not called
`json`, because `kapi terms --format json` already means something else — a
separate, lossy JSON export that predates the native tier and does not preserve
concept identity. Reach for `bundle` when you want the lossless form.
...
The suffix, not the location, is what identifies a bundle — a project puts one
wherever it likes and may keep several. Which bundle a given project uses is set
by the recipe
([`defaults.termbase_source` / `defaults.tm_source`](/reference/project-file)) or
by an explicit `-o` path.

### Where terms live when nothing is bound

Your glossary belongs at the **repository root**, as `terms.json` — not under
`.kapi/`. `.kapi/` is gitignored, so a glossary kept there would never be
committed, never show up in a diff, and never get reviewed, which defeats the
point of keeping terminology as source. The bundle is the committed source;
`.kapi/termbase.db` is the throwaway cache it compiles into.

So the fallback ladder is `<root>/terms.json` first, then
`<root>/.kapi/terms.json` — the second rung only for a project that deliberately
keeps its glossary local and uncommitted.

Content memory has no such fallback, and that asymmetry is deliberate: **a
project has one glossary but many content-memory bundles**, typically one per
content surface. This repository's own dogfood commits a bundle per surface
under `l10n/tm/` against a single `l10n/terms.json`. A convention needs one
obvious answer to point at; terms has one and memory does not, so a memory seed
is always named rather than guessed.
...
[sync protocol](/contribute/architecture/025-kbf-package) moves over the wire:
...
- `kind: kapi-interchange` — a task-scoped **bilingual** slice for one locale
  ...
  `extract` and ingested by `merge` (see [KBF vs XLIFF](/reference/kbf/vs-xliff)).

Both are *parcels*, not workspaces: day-to-day work happens in the ambient `.kapi`
project ([AD-025](/contribute/architecture/025-kbf-package) §7).

Because every member is a native lossless format, the whole package is lossless:
unpacking can seed a fresh content memory, terms store, and block store, and the
...
A `.kpz` packs the **source of truth**, not the regenerable caches. It excludes
...
A `.kpz` is not only an at-rest snapshot of finished content — it can also carry
...
- **Project snapshot (`pack` / `unpack`).** Inside a `.kapi` project the working
  ...
  (`.kapi/cache/blocks.db`), so resume there is simply running again.
...
- [AD-025: KBF Family and the .kpz Package](/contribute/architecture/025-kbf-package) — the decision and rationale.
- [Specification](/reference/kbf/spec) — the KBF block format the family is built around.
- [KBF vs XLIFF](/reference/kbf/vs-xliff) — the native-vs-interchange split for blocks.
