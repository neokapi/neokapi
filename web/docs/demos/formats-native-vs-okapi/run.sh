...
python3 - "$NATIVE_WC" "$OKAPI_WC" "$OKAPI_STATUS" <<'PY' > parity.json
import json, sys
native, okapi, status = int(sys.argv[1]), int(sys.argv[2]), sys.argv[3]
print(json.dumps({
    "asset": "fixtures/page.html",
    "native": {"reader": "html", "word_count": native},
    "okapi":  {"reader": "okf_html", "word_count": okapi, "status": status},
    "parity": status == "ok" and native == okapi,
    "note": "Authoritative native↔okapi parity across 30+ formats: see cli/parity (`make parity`).",
}, indent=2))
PY
