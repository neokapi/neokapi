...
`kapi init` also wires the project for the coding agents that work in it, naming every file as it writes it: an MCP entry that starts `kapi mcp` for this project, and a copy of the kapi skill. Choose the agents with `--agents`, or opt out with `--agents none`. See [what `kapi init` wires up](/kapi/get-started/use-with-claude#what-kapi-init-wires-up).
