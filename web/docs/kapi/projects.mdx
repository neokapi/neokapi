...
A process-only run persists only *working* output — target overlays in the block tables of `.kapi/work/store.db`, pre-merge. It never records a review **decision**: those are authored by `kapi apply` and published by `kapi commit` to `.kapi/context/decisions/`, not by a flow's sink. The store rebuilds from what the project commits; the decisions are the authored part. See [the project store](/kapi/project-store).
