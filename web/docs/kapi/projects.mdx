...
A process-only run persists only *working* output — target overlays in the block tables of `.kapi/store.db`, pre-merge. It never records a review **decision**: those are authored by `kapi apply` and published by `kapi commit` to `.kapi/units/`, not by a flow's sink. The store rebuilds from what the project commits; the decisions are the authored part. See [the project store](/kapi/project-store).
...
Deliberately **not** carried: `.kapi/cache/` (regenerable — parse cache, extractions, collections), credentials (they live in the OS keychain), and server sync tokens. A snapshot is state you could not recompute, never secrets and never derived data. A part with nothing in it is simply absent from the archive.
