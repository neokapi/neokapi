...
Address the unit exactly as the worklist lists it. `status` is `"reviewed"` by default; `"signed-off"` is the optional final rung for contractual or regulated content. The decision lands in the [project store](/kapi/project-store)'s working set; `kapi commit` writes it into the unit record under `.kapi/units/`, which you **commit with your sources** so the decision travels with the project:

```bash
kapi status                 # what is staged but not yet committed
kapi commit                 # → .kapi/units/*.jsonl
git add .kapi/units
```
