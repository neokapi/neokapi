...
description: "The human half of the loop: work the review queue in Kapi Desktop — inspect, edit, approve, reject, sign off — or drive the same worklist from the CLI or your assistant. Approvals land in the committed unit record and raise the coverage a ship gate measures."
...
Address the unit exactly as the worklist lists it. `status` is `"reviewed"` by default; `"signed-off"` is the optional final rung for contractual or regulated content. The decision lands in the [project store](/kapi/project-store)'s working set; `kapi commit` writes it into the unit record under `.kapi/context/decisions/`, which you **commit with your sources** so the decision travels with the project:

```bash
kapi status                 # what is staged but not yet committed
kapi commit                 # → .kapi/context/decisions/*.jsonl
git add .kapi/context/decisions
```
