...
1. **Run.** `kapi up` settles each source block (terminology, voice, source QA), stamps its authoring status (*authored → checked → approved*), and holds any block below the gate. Ready blocks translate; held blocks do not. A run that could translate nothing surfaces as *held on source* — never a silent skip.
