...
description: "The human half of the loop: work the review queue in Kapi Desktop (inspect, edit, approve, reject, sign off) or drive the same worklist from the CLI or your assistant. Approvals land in the project's store and raise the coverage a ship gate measures."
...
1. **Run.** `kapi up` settles each source block (terminology, voice, source checks), stamps its authoring status (*authored → checked → approved*), and holds any block below the gate. Ready blocks translate; held blocks do not. A run that could translate nothing surfaces as *held on source* rather than as a silent skip.
...
Address the unit exactly as the worklist lists it. `status` is `"reviewed"` by default; `"signed-off"` is the optional final rung for contractual or regulated content. The decision is recorded in the [project store](/kapi/project-store) and is durable at once, so every checkout of the project reads it and there is no second step. Read it back, with the evidence behind it, from `kapi context log`.

To put the record in git as well, write it out with `kapi context snapshot --out <dir>` and commit the shards it produces. See [Moving context](/kapi/context-portability).
