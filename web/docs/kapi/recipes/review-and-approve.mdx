...
3. **Settle.** Fix the flagged terms, brand findings, or source text — through the round-trip (`kapi apply`) or your editor — so the next check clears them. An explicit source sign-off is a `kind:"review"` entry on the source, for the strict `approved` gate.
