...
*parcel*, not the place you normally work, since the day-to-day working model is
the ambient [`.kapi` project](/kapi/modes). The steps below use the same bindings
any flow uses:
...
the multi-file, team/server form. A project keeps its config in a committed
`.kapi` *recipe* (declared content, flows, target languages) and its working
state in a `.kapi/` directory (the persistent cache, TM, and termbase). It is the
day-to-day working model; the single-file `.klz` workspace above is the portable
*parcel* form of the same round-trip — same content, same output, different
packaging.
