...
sidebar_position: 4
description: "The three places a kapi content memory or terms store can live — a loose file in the current directory, a named store under your config home, or a project's .kapi/store.db — and the flags that select each."
...
both inside the project's single `.kapi/store.db` — that the extract/merge
...
the project's single database at `<project-root>/.kapi/store.db`. The project is
...
| Project store | no flag, inside a `.kapi` project | tables in `<root>/.kapi/store.db` | a repository you translate repeatedly |
...
set and committed to `.kapi/units/*.jsonl` by `kapi commit`. Approving a unit
