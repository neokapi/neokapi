...
- **Terms that must survive verbatim.** `kapi exec dnt-check` (do-not-translate) fails a target where a product name, trademark or code identifier listed under `--terms` was translated, transliterated or dropped; the translate step masks those spans, so the model never sees them. A concept the terms store marks do-not-translate is held by `term-check` instead, and the translate step asks the model to keep it verbatim.
