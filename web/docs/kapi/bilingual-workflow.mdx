...
agency — you can hand the work out and take it back: kapi produces a file the
translator opens in whatever tool they already use, accepts the returned version,
and rebuilds your original files byte-for-byte from it. It is an escape hatch,
there when you need it, not a step in the normal path — and the corrections that
come back are captured so they carry into the next run.

For the translation-memory model — matching, fuzzy thresholds, and the
pre-fill/absorb cycle — see [Translation Memory](/framework/translation-memory);
this recipe walks the extract/merge task.
...
TM absorb is **recycle leverage** — it makes the pair available to pre-fill
future translation. It is *not* a review decision. Whether a reviewer
**approved** a particular target is a separate, committed fact recorded in the
project [state store](/kapi/project-store) (`.kapi-state.json`) by `kapi apply`
(a `kind:"review"` change), not in the TM. Merge keeps the TM as the recycle
corpus; approval is the deliberate, content-hash-bound decision that lives
elsewhere.
...
- Extraction bookkeeping (the captured skeletons and a manifest) recorded
  in the project's gitignored cache so `kapi merge` can reattach the
  returned translations and rebuild the source byte-for-byte — for document
  and markup formats and for keyed catalog formats alike (JSON, YAML,
...
state — the translation memory, the termbase, and the stand-off overlays,
tone/channel variants, and per-target provenance and status that the
[content model](/framework/content-model) records.
