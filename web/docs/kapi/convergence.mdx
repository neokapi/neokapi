...
The plan asks what the pass asks. A pass reads the source documents, fills from
the content memory, and drafts the remainder — so a unit costs a provider call
when the corpus has no answer for its source, whatever a target file happens to
hold. Missing units, **stale** ones (a decision's source moved) and
...
next derivation simply reads the working tree and compares each decision against
...
```bash
kapi up --plan
# scope  missing  stale  unanswered  content memory exact  AI work  ~tokens
# de           0      1           0                     0        1        9
#
...
say so on the `stale` axis above, and the rest simply come back as pending work.
