...
`.kbf.json` parcel carries the full status across a hand-off.
...
1. **Settle the source.** Check terminology and mark the terms that must not be
   translated, check voice profile, run source QA. This lifts source units up the
...
different questions. **Content** is a position — what is here and not there —
...
the project's bound quality gates (voice, terminology, QA, staleness) together
with the ship and source coverage gates over the project's content, and exits non-zero when any
...
reuse (recycle) followed by AI translate — so a recipe needs no flow YAML at all
to catch up.
...
One pass closes what a machine can close. A stale unit is **work**, not only a
report: the loop re-drafts it against the source the project has now — content
memory first, then AI for the remainder, exactly as it produces a unit that has
no translation at all — and the plan quotes that work before any tokens burn:
...
sentence it *does* translate, where a later fuzzy match can still recycle it,
and the entry itself is drafted afresh.

That holds **in every language**, not only where someone had reviewed the unit.
...
be: that pairing was authored, not left behind, so it is kept as it stands.

What the loop does not do is decide. Producing a translation is not approving
one, so the re-drafted unit comes back at *draft*, in the review worklist, with
the earlier approval **not** restored — the scope does not ship until a person
reviews the new pairing. That is the property that matters on both sides: drift
is never converged over, and it is never left un-worked either.
...
An approval is not about a translation; it is about a **pairing** — this
rendering, *of this source*. The record binds both halves by content hash: the
...
Decisions recorded before the basis was tracked carry none. That is *unknown*,
not drift: such a unit keeps its rung and ships as before, and `kapi status`
