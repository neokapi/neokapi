...
translation), so it is recorded in the [project store](/kapi/project-store) and
is durable at once, and `kapi commit` writes it into the committed record under
`.kapi/state/`. The content memory holds none of it.
