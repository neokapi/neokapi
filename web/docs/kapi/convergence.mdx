...
the project's bound quality gates (voice, terminology, QA) together with the ship
...
**server-connected** project — a recipe with a `bowrain:` block — the same
...
itself is never implicit: whenever a `bowrain:` block makes the choice ambiguous,
...
:::note
The server venue — the `bowrain:` block, `kapi push`/`pull`/`up --local`, the
