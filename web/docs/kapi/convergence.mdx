...
One pass closes what a machine can close — where the edit re-keys the unit (a
prose paragraph, whose identity is its wording), it is re-translated, memory
first, then AI:
...
Where the unit keeps its key across the edit — a catalog entry, whose identity is
its name — the old translation stays attached to it, so a pass has nothing to
produce and the unit waits in the worklist for a person. Either way the scope
does not ship until the pairing is settled, which is the property that matters:
drift is reported, never converged over.
...
coverage threshold makes a translation of a rewritten sentence shippable. Restore
the source and it converges back on the decision already recorded; nobody
re-reviews anything.
