...
export default function TestComparison() {
  ...
  // The comparison dataset is generated by the parity pipeline (not committed),
  // so it is loaded at RUNTIME from the docs CDN (or same-origin /data when no
  // CDN is configured) rather than imported at build time — a missing/ungenerated
  // report then degrades to a friendly notice instead of breaking the build.
  const { siteConfig } = useDocusaurusContext();
  const cdn = readCdnConfig(siteConfig);
  const sameOriginURL = useBaseUrl("/data/test-comparison.json");
  const dataURL = cdnEnabled(cdn) ? cdnHref(cdn, "/data/test-comparison.json") : sameOriginURL;

  const [raw, setRaw] = useState<TestComparisonData | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const json = (await res.json()) as TestComparisonData;
        if (!cancelled) {
          setRaw(json);
          setLoadState("ready");
        }
  ...
  return (
    <Layout title="Test Comparison" description="Okapi vs neokapi filter test comparison">
      <main className="container margin-vert--lg">
        ...
        {loadState === "loading" && <p>Loading the comparison report…</p>}
        {loadState === "unavailable" && (
          <p>
            The test-comparison report isn't available in this build. It's generated by the parity
            pipeline (<code>make parity-report</code>) and published to the docs CDN; this page
            loads it at runtime.
          </p>
        )}
