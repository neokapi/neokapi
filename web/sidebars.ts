...
const sidebars: SidebarsConfig = {
  // PR-B — "With toggle" surface model. ONE recipe set, made surface-aware with
  // a persistent <Cli>/<Desktop> toggle (a floating control that self-hides on
  // pages with no dual content). Kapi CLI and Kapi Desktop are thin OVERVIEW
  // pages — not separate recipe trees; the single recipe set below serves both
  // surfaces, flipped by the toggle.
  kapiSidebar: [
    {
      type: "category",
      label: "Get started",
      ...
      // Action before architecture: install → do something (quickstart) → make
      // it a project → drive it from AI → React on-ramp. `modes` (bindings/
      // ad-hoc/project architecture) sits AFTER first-project, not at the head.
      items: [
        ...
        "kapi/get-started/react-projects",
      ],
    },
    // Thin overview pages for each surface; the recipes below serve both via the
    // <Cli>/<Desktop> toggle.
    {
      type: "category",
      label: "Kapi CLI",
    ...
    // One source-first recipe set, surface-aware via the toggle: prepare your
    // source, then go multilingual.
    {
      type: "category",
