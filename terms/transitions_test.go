...
func TestKnownTermStatus(t *testing.T) {
	for _, s := range []model.TermStatus{
		...
		assert.True(t, terms.KnownTermStatus(s), string(s))
	}
	assert.False(t, terms.KnownTermStatus(""))
	assert.False(t, terms.KnownTermStatus("bogus"))
}
...
func TestValidateTransition(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := terms.ValidateTransition(tt.from, tt.to)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
...
func TestIsGovernedTransition(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, terms.IsGovernedTransition(tt.from, tt.to))
		})
	}
}
