...
func sqliteSoftwareConcepts() []terms.Concept {
...
func sqlitePopulateTB(t *testing.T, tb terms.Terminology) {
	t.Helper()
...
func TestSQLiteTerms_AddAndGet(t *testing.T) {
...
func TestSQLiteTerms_Delete(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
...
func TestSQLiteTerms_Update(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
...
func TestSQLiteTerms_Lookup(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
	...
	matches := mustLookup(t, tb, "Save", terms.LookupOptions{
...
func TestSQLiteTerms_LookupAll(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
...
func TestSQLiteTerms_Search(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
...
func TestSQLiteTerms_InterfaceCompliance(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)

	var _ terms.Terminology = tb
	var _ terms.Store = tb
	require.NoError(t, tb.Close())
}

func TestSQLiteTerms_AddConceptWithStream(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()

	mainConcept := terms.Concept{
		ID:     "c-main",
		Domain: "software",
		Terms: []terms.Term{
...
func TestSQLiteTerms_FuzzyLookup(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()

	require.NoError(t, tb.AddConcept(context.Background(), terms.Concept{ID: "c1", Domain: "IT",
		Terms: []terms.Term{{Text: "computer", Locale: "en-US", Status: "approved"}}}))
	require.NoError(t, tb.AddConcept(context.Background(), terms.Concept{ID: "c2", Domain: "IT",
		Terms: []terms.Term{{Text: "computing", Locale: "en-US", Status: "approved"}}}))
	require.NoError(t, tb.AddConcept(context.Background(), terms.Concept{ID: "c3", Domain: "IT",
		Terms: []terms.Term{{Text: "unrelated", Locale: "en-US", Status: "approved"}}}))

	matches := mustLookup(t, tb, "computers", terms.LookupOptions{
		SourceLocale: "en-US",
		MinScore:     0.7,
	})
...
func TestSQLiteTerms_ScaleTest(t *testing.T) {
	if testing.Short() {
	...
	matches := mustLookup(t, tb, "terminology entry number 42", terms.LookupOptions{
		SourceLocale: "en-US",
		MinScore:     0.7,
	})
	assert.GreaterOrEqual(t, len(matches), 1)
...
func TestSQLiteTerms_TermSourceField(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
	...
	assert.True(t, ok)
	assert.Equal(t, terms.TermSourceTerminology, c.Source)

	c, ok = mustGetConcept(t, tb, "brand-1")
	assert.True(t, ok)
	assert.Equal(t, terms.TermSourceBrandVocabulary, c.Source)
}

func TestSQLiteTerms_SourceFilterLookup(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
	...
	matches := mustLookup(t, tb, "Deploy", terms.LookupOptions{
		SourceLocale: model.LocaleEnglish,
	})
	...
	matches = mustLookup(t, tb, "Deploy", terms.LookupOptions{
		SourceLocale: model.LocaleEnglish,
		SourceFilter: []terms.TermSource{terms.TermSourceTerminology},
	})
	require.Len(t, matches, 1)
	assert.Equal(t, "term-1", matches[0].Concept.ID)
}

func TestSQLiteTerms_SourceFilterLookupAll(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
	...
	matches := mustLookupAll(t, tb, "Click Save", terms.LookupOptions{
		SourceLocale: model.LocaleEnglish,
		SourceFilter: []terms.TermSource{terms.TermSourceBrandVocabulary},
	})
	require.Len(t, matches, 1)
...
func TestSQLiteTerms_CompetitorTermField(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()
...
func TestConceptRelation_JSON(t *testing.T) {
	...
	var decoded terms.ConceptRelation
...
func TestTermDesignation_WithValidity(t *testing.T) {
	...
	var decoded terms.TermDesignation
	require.NoError(t, json.Unmarshal(data, &decoded))
