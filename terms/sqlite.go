...
var tbMigrations = []storage.Migration{
	...
	},
	{
		...
		SQL:         termsschema.RenderTermsSQLiteV6(),
	},
}
...
func (tb *SQLiteStore) AddConceptWithStream(ctx context.Context, concept Concept, stream string) error {
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO tb_concepts (id, project_id, stream, domain, definition, properties, source, do_not_translate, advisory, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func (tb *SQLiteStore) scanConcept(ctx context.Context, id string) (Concept, error) {
	...
	err := tb.db.QueryRowContext(ctx, `
		SELECT id, project_id, domain, definition, properties, source, do_not_translate, advisory, created_at, updated_at
		FROM tb_concepts WHERE id = ?
	`, id).Scan(&c.ID, &c.ProjectID, &c.Domain, &c.Definition, &propsJSON, &source, &c.DoNotTranslate, &c.Advisory, &createdStr, &updatedStr)
	if err != nil {
...
func (tb *SQLiteStore) queryTermsByLocale(ctx context.Context, locale model.LocaleID, domains []string, statusFilter []model.TermStatus, opts LookupOptions) ([]LocaleTerm, error) {
	...
	rows, err := tb.db.QueryContext(ctx, fmt.Sprintf(`
		SELECT c.id, c.project_id, c.domain, c.definition, c.source, c.do_not_translate, c.advisory, t.text, t.locale, t.status, t.part_of_speech, t.gender, t.note, t.competitor_term, t.valid_from, t.valid_to, t.tags, t.forms
		FROM tb_terms t JOIN tb_concepts c ON t.concept_id = c.id
		WHERE %s
	...
	for rows.Next() {
		...
		var competitorInt int
		var validFrom, validTo sql.NullString
		if err := rows.Scan(&cID, &projectID, &domain, &definition, &source, &doNotTranslate, &advisory, &text, &loc, &status, &pos, &gender, &note, &competitorInt, &validFrom, &validTo, &tags, &forms); err != nil {
			continue
		}
		...
		results = append(results, LocaleTerm{
			Concept: Concept{ID: cID, ProjectID: projectID, Domain: domain, Definition: definition, Source: TermSource(source), DoNotTranslate: doNotTranslate, Advisory: advisory},
			Term: Term{
				Text:           text,
