...
func conceptByID(t *testing.T, tb Terminology, id string) Concept {
	t.Helper()
...
func TestImportTBX(t *testing.T) {
	tests := []struct {
		...
		wantNum int
		check   func(t *testing.T, tb Terminology)
	}{
		{
			name:    "TBX-Basic v3",
...
// assertConceptsEquivalent verifies that two terms stores hold equivalent concepts
// (ignoring timestamps and term ordering within a locale).
func assertConceptsEquivalent(t *testing.T, a, b Terminology) {
	t.Helper()
