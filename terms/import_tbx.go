...
// TBXImportOptions controls how a TBX (Terminology eXchange) document is interpreted.
...
// to a terms store Concept. Returns the number of concepts imported.
func ImportTBX(ctx context.Context, tb Terminology, reader io.Reader, opts TBXImportOptions) (int, error) {
...
// buildTerm converts a parsed term section into a terms store Term.
...
func parseTBXStatus(s string) model.TermStatus {
	...
	case "supersededterm-admn-sts", "superseded", "supersededterm":
		// No distinct "superseded" status in the terms store vocabulary;
