...
// TermLookupTool scans translatable blocks for terms from a Terminology and
...
func NewTermLookupTool(tb Terminology, cfg TermLookupConfig) *TermLookupTool {
	if cfg.MinScore <= 0 {
		cfg.MinScore = 0.8
	}
	...
	t.ToolDescription = "Annotates blocks with matching terms from a terms store"
...
// from a Terminology. For each source term found, it verifies that a preferred
...
func NewTermEnforceTool(tb Terminology, cfg TermEnforceConfig) *TermEnforceTool {
...
func resolveReplacement(ctx context.Context, tb Terminology, conceptID string, scope *graph.Scope) (Concept, bool, error) {
