...
// relationBackends returns the framework Terminology implementations so every
// relation and validity test runs against memory and SQLite alike.
func relationBackends(t *testing.T) map[string]terms.Terminology {
	t.Helper()
	sqliteTB, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	t.Cleanup(func() { _ = sqliteTB.Close() })
	return map[string]terms.Terminology{
		"memory": terms.NewInMemoryStore(),
		"sqlite": sqliteTB,
	}
}

// seedRelationConcepts adds three plain concepts to relate.
func seedRelationConcepts(t *testing.T, tb terms.Terminology) {
	t.Helper()
...
func mustRelationsOf(t *testing.T, tb terms.Terminology, conceptID string, scope *graph.Scope) []terms.ConceptRelation {
	t.Helper()
...
func mustListRelations(t *testing.T, tb terms.Terminology, scope *graph.Scope) []terms.ConceptRelation {
	t.Helper()
...
func relationIDs(rels []terms.ConceptRelation) []string {
...
func TestTerms_RelationsCRUD(t *testing.T) {
...
func TestTerms_AddRelationValidation(t *testing.T) {
	for name, tb := range relationBackends(t) {
		t.Run(name, func(t *testing.T) {
			seedRelationConcepts(t, tb)
			ctx := context.Background()
			...
			tests := []struct {
				...
				{"missing ID", terms.ConceptRelation{SourceID: "c1", TargetID: "c2", RelationType: graph.LabelRelated}},
				{"missing source", terms.ConceptRelation{ID: "r1", TargetID: "c2", RelationType: graph.LabelRelated}},
				{"missing target", terms.ConceptRelation{ID: "r1", SourceID: "c1", RelationType: graph.LabelRelated}},
				{"unknown relation type", terms.ConceptRelation{ID: "r1", SourceID: "c1", TargetID: "c2", RelationType: "FRIENDS_WITH"}},
				{"concept-to-term label rejected", terms.ConceptRelation{ID: "r1", SourceID: "c1", TargetID: "c2", RelationType: graph.LabelHasTerm}},
				{"source concept not found", terms.ConceptRelation{ID: "r1", SourceID: "ghost", TargetID: "c2", RelationType: graph.LabelRelated}},
				{"target concept not found", terms.ConceptRelation{ID: "r1", SourceID: "c1", TargetID: "ghost", RelationType: graph.LabelRelated}},
			}
...
func TestTerms_RelationValidityScoping(t *testing.T) {
	...
	for name, tb := range relationBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			rels := mustListRelations(t, tb, nil)
			byID := make(map[string]terms.ConceptRelation, len(rels))
			for _, r := range rels {
				byID[r.ID] = r
			}
...
func TestTerms_DeleteConceptCascadesRelations(t *testing.T) {
	for name, tb := range relationBackends(t) {
		t.Run(name, func(t *testing.T) {
			seedRelationConcepts(t, tb)
			ctx := context.Background()
...
func TestTerms_TermValidityLookup(t *testing.T) {
	now := time.Now().UTC().Truncate(time.Second)
	past := now.Add(-time.Hour)
	...
	for name, tb := range relationBackends(t) {
		t.Run(name, func(t *testing.T) {
			...
			matches := mustLookup(t, tb, "Howdy", terms.LookupOptions{SourceLocale: model.LocaleEnglish})
			assert.Len(t, matches, 1)

			// A "now" scope skips the retired term.
			nowScope := graph.ScopeAt(now)
			matches = mustLookup(t, tb, "Howdy", terms.LookupOptions{
			...
			us := graph.Scope{At: now, Tags: map[string]string{"market": "us"}}
			matches = mustLookup(t, tb, "Hello", terms.LookupOptions{
			...
			matches = mustLookupAll(t, tb, "Hello Howdy Hi", terms.LookupOptions{
				SourceLocale: model.LocaleEnglish, Scope: &dach,
			})
...
func TestTerms_AddConceptRejectsUnknownTermStatus(t *testing.T) {
	for name, tb := range relationBackends(t) {
		t.Run(name, func(t *testing.T) {
			err := tb.AddConcept(context.Background(), terms.Concept{
				ID: "c-bad",
				Terms: []terms.Term{
...
func TestSQLiteTerms_RelationsForStream(t *testing.T) {
	tb, err := terms.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tb.Close()

	ctx := context.Background()
	require.NoError(t, tb.AddConceptWithStream(ctx, terms.Concept{
		ID: "c1", Domain: "software",
		Terms: []terms.Term{{Text: "File", Locale: "en-US", Status: model.TermApproved}},
	}, "main"))
	require.NoError(t, tb.AddConceptWithStream(ctx, terms.Concept{
		ID: "c2", Domain: "software",
		Terms: []terms.Term{{Text: "Document", Locale: "en-US", Status: model.TermApproved}},
	}, "main"))
	...
	require.NoError(t, tb.AddRelationWithStream(ctx, terms.ConceptRelation{
