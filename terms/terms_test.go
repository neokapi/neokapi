...
func populateTB(t *testing.T, tb terms.Terminology) {
	t.Helper()
...
func TestInMemoryTerms_AddAndGet(t *testing.T) {
...
func TestInMemoryTerms_Delete(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_Update(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_LookupExact(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_LookupCaseInsensitive(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_LookupCaseSensitive(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_LookupFuzzy(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)

	matches := mustLookup(t, tb, "Repositor", terms.LookupOptions{
		SourceLocale: model.LocaleEnglish,
		MinScore:     0.7,
...
func TestInMemoryTerms_LookupAll(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_LookupWithDomainFilter(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_LookupWithStatusFilter(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)

	// Should find both "Repository" (preferred) and "Repo" (admitted).
	matches := mustLookup(t, tb, "Repository", terms.LookupOptions{
		SourceLocale: model.LocaleEnglish,
	})
	assert.Len(t, matches, 1)
...
func TestInMemoryTerms_Search(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_Concepts(t *testing.T) {
	tb := terms.NewInMemoryStore()
	populateTB(t, tb)
...
func TestInMemoryTerms_ConceptHelpers(t *testing.T) {
...
`
	...
	err = terms.ExportCSV(context.Background(), tb, &buf, model.LocaleEnglish, model.LocaleFrench, true)
	require.NoError(t, err)
...
on-brand,Consistent with the voice
`
	...
	matches := mustLookup(t, tb, "Bowrain", terms.LookupOptions{
		SourceLocale: model.LocaleEnglish,
	})
	require.Len(t, matches, 1)
...
func TestCSVImportMonolingualTermOnly(t *testing.T) {
	...
	matches := mustLookup(t, tb, "kapi", terms.LookupOptions{
		SourceLocale: model.LocaleEnglish,
	})
	require.Len(t, matches, 1)
...
func TestJSONImportExport(t *testing.T) {
	...
	// Export.
	var buf bytes.Buffer
	err := terms.ExportJSON(context.Background(), tb, &buf, "test-terms")
	require.NoError(t, err)

	exported := buf.String()
	assert.Contains(t, exported, "test-terms")
	assert.Contains(t, exported, "Repository")

	// Re-import.
	tb2 := terms.NewInMemoryStore()
	count, err := terms.ImportJSON(context.Background(), tb2, strings.NewReader(exported))
	require.NoError(t, err)
	assert.Equal(t, 3, count)
