...
func richConcepts() []terms.Concept {
...
func TestMarshalRelationsDeterministic(t *testing.T) {
	...
	fileB.Relations = []terms.ConceptRelation{rels[1], rels[0]}
	b, err := Marshal(fileB)
	require.NoError(t, err)
	...
	fileEmpty.Relations = []terms.ConceptRelation{}
...
func TestUnmarshalRejectsBadEnvelope(t *testing.T) {
	...
	_, err = Unmarshal([]byte(`{"schemaVersion":"2.0","kind":"kapi-terms","concepts":[]}`))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "unsupported major")

	_, err = Unmarshal([]byte(`{"schemaVersion":"1.7","kind":"kapi-terms","concepts":[]}`))
...
func TestMarshalShape(t *testing.T) {
	...
	assert.Contains(t, out, `"kind": "kapi-terms"`)
}
