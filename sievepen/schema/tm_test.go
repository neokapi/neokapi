...
// TestTMSQLiteGolden asserts the rendered SQLite DDL is byte-identical to the
...
func TestTMSQLiteGolden(t *testing.T) {
	cases := []struct {
		name string
		got  string
	}{
		{"tm_sqlite_v1.golden.sql", RenderTMSQLiteV1("icu")},
		{"tm_sqlite_v2.golden.sql", RenderTMSQLiteV2()},
		{"tm_sqlite_v3.golden.sql", RenderTMSQLiteV3()},
	}
...
// TestTMPostgresSemanticEquivalence asserts the descriptor-rendered Postgres
...
func TestTMPostgresSemanticEquivalence(t *testing.T) {
	assertStmtSetEqual(t, "v4 create", pgV4Reference, RenderTMPostgresCreate())
	assertStmtSetEqual(t, "v5 concept_id", pgV5Reference, RenderTMPostgresConceptID())
}
