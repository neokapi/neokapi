...
// TMCandidateSource is the dialect seam for the shared tiered-lookup
...
// callbacks return framework TMEntry values already hydrated with their
// variants.
type TMCandidateSource struct {
	...
	// "general_key".
	Exact func(ctx context.Context, column, key string, sourceLocale model.LocaleID, opts LookupOptions) ([]TMEntry, error)
	...
	FuzzyCandidates func(ctx context.Context, plainKey, structKey, generalKey string, sourceLocale model.LocaleID, opts LookupOptions) ([]TMEntry, error)
}

// TieredLookup runs the shared exact→fuzzy tiered match against a backend's
// candidate source. It is the single home of the TM ranking policy; the SQLite
