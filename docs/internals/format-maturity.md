...
- **Certification decays.** When `last_certified` is older than 45 days the
  dashboard flags the tier *stale* (a long absence produces stale badges rather
  than breakage; one triage-score run clears them); older than 120 days, the
...
- The floor's `docs` and `detection` cells remain constants
  ...
  *enforced* for users via the Supported tier (Engine ≥ L3 ∧ Knowledge ≥ K2).
  ...
  `register_test.go`'s detection lists), tracked as a scorer issue rather than
  silently changed.
...
`testdata/fuzz/`), `core/safeio` budgets, and hostile-corpus sweeps score on
their own **Security axis** (§2.6: S0–S4, bounded → fuzzed →
hostile-hardened → continuously assured) rather than on Engine. The Engine
...
- **`core/formats/constructs.yaml`** (repo-level registry, versioned, stable
  IDs) is the construct row space, seeded from ITS 2.0 data categories, the
...
- **`core/formats/<id>/vocabulary.yaml`** is the format's matrix. Per
...
unresolvable citation makes the cell count as `unknown` in the census, so an
...
| **V2** | Bidirectional | V1 **plus**: the writer consumes canonical types, so a target authored in canonical vocabulary serializes to correct native markup (`write` cells claimed and resolved); the cross-format equivalence test passes (`core/formats/vocab_equivalence_test.go`: a shared fixture sentence carrying bold/italic/link/image yields the same canonical `Type` sequence as the reference formats); block-level kinds populated where the format expresses them. |
...
The Editor axis has **no quality dimensions**, and its determinism comes from
the probes rather than from trusting the YAML: `integrations.yaml` is on the
...
- **`core/formats/<id>/dossier.yaml`** is the per-format backbone:
  authoritative spec sources (`{id, version, url, watch}`), other
  implementations (Okapi filter id plus the obvious others,
...
- **The spec knowledge base** (`specs/` at the repo root): `catalog.yaml`
  ...
  (vendor-class committed; cache-class gitignored; **ISO text never**, so use
...
the ledger's `runs[].evidence`) rather than on bare watermarks, so a watermark
updated without its check is detectable, and the calibration phase of
`process-health` spot-replays one recorded check per cycle.
...
**Tier C synthetic** (deterministic seeded generators + fuzz seeds, where the
...
| **S3** | Hostile-hardened | S2 **plus** a clean corpus-sweep record in the ops ledger: 0 `CRASH`/`HANG`/`OOM` **and** 0 `ROUNDTRIP_DRIFT` over the wild/hostile set. (`govulncheck`-clean is module-wide, a noted co-signal rather than a per-format gate.) Absent a ledger sweep record, the ceiling stays S2. |
...
through the `tier-review` ritual and recorded here when taken, rather than an
audit outcome.
...
`core/model/structure.go` (additive, with no proto/KBF schema change). It is the
...
| **G2** | Linear body text | G1 **plus**: the body content recovered as text in correct reading order (stream order), optionally grouped by the `Layer`/`Group` tree, with roles still generic (`Block.Type` only) and no normalized `StructureAnnotation.Role`. |
...
parity fidelity of the *serialization*, so a reader can be L4 byte-faithful
while flattening to one block per page (G0). Vocabulary (V) measures *inline /
...
*block-level + cross-block + spatial* structure via the
`StructureAnnotation.Role` standoff layer, a different field that
structure-aware readers populate (`SetSemanticRole`) and `constructs.yaml` does
not model. Proof of independence: `docling` is V0 (plain runs) but high-G
(roles / tables / geometry / order); `html` is V1 (typed inline) but low-G (no
geometry). G is not an extension of V.
...
(`structure.go:124-126`); their `geometry` cell is `na` as a **ceiling cap**, so
the audit sets `structure.ceiling = G3` (or G2) and they cannot reach G4. Unlike
every other axis, `na` on this *cumulative depth* ladder must mean "ceiling
capped below this rung," **not** "this rung passes", or a roles-only
...
- **G3 certifies table *topology*, not span fidelity.** The `roles` signal
  ...
  home in `StructureAnnotation.ColSpan`/`RowSpan` (DocLang `lcel`/`ucel`/`xcel`),
...
- **G2 reading-order is grep-lenient.** The `readingorder` signal accepts
  emitting `PartGroupStart`, which most grouped formats do trivially, so it proves
...
authoritative tags or an ML guess, at the same depth with different trust
(`GeometryAnnotation.SourceRef` already records provenance). Carry the authority
as a per-format qualifier in the `core/formats/<id>/structure.yaml` artifact
...
2. **Run the floor**: `python3 .skills/refresh-format-maturity/scripts/audit-format.py <id>`,
   noting each axis's `base..ceiling` band and missing-artifact list.
...
10. **Check upstream + drift** (unchanged): Okapi tracker sweep recipe in
    ...
    `make contract-audit`; regenerate dashboards, because a regression can hide
...
## Maturity report (snapshot: 2026-09-14)

Generated by the `bootstrap-publish` (deterministic floor; no quality
demotions) over 37 real formats. Regenerated by every ritual that
...
- **Engine**: L0:7 · L1:10 · L2:13 · L3:7 · L4:0
- **Vocabulary**: V0:32 · V1:5 · V2:0 · V3:0
- **Editor**: E0:34 · E1:3 · E2:0 · E3:0 · E4:0
- **Knowledge**: K0:8 · K1:16 · K2:13 · K3:0
- **Corpus**: C0:23 · C1:14 · C2:0 · C3:0
- **Security**: S0:7 · S1:26 · S2:4 · S3:0 · S4:0
- **Structure & Geometry**: G0:23 · G1:1 · G2:6 · G3:4 · G4:3
- **Prose**: P0:26 · P1:0 · P2:11 · P3:0 · P4:0

### Per-format vector

| Format | Tier | Engine | Vocabulary | Editor | Knowledge | Corpus | Security | Structure & Geometry | Prose | Top engine gap |
|---|---|---|---|---|---|---|---|---|---|---|
| `androidxml` | available | L1 | V0 | E0 | K2 | C0 | S1 | G0 | P2 | add malformed_test.go |
...
| `doclang` | available | L1 | V0 | E0 | K0 | C0 | S1 | G4 | P2 | add malformed_test.go |
...
| `html` | available | L1 | V1 | E1 | K1 | C1 | S2 | G3 | P2 | add malformed_test.go |
...
| `markdown` | available | L3 | V1 | E1 | K1 | C1 | S2 | G3 | P2 | none |
| `mdx` | available | L1 | V0 | E1 | K2 | C0 | S1 | G2 | P2 | add malformed_test.go |
...
| `po` | maintained | L2 | V0 | E0 | K1 | C1 | S1 | G2 | P2 | add a corpus/upstream test |
| `properties` | maintained | L2 | V0 | E0 | K1 | C1 | S1 | G0 | P2 | add a corpus/upstream test |
| `resx` | maintained | L3 | V0 | E0 | K2 | C0 | S1 | G0 | P2 | none |
...
| `tmx` | maintained | L2 | V0 | E0 | K1 | C1 | S1 | G0 | P2 | add a corpus/upstream test |
...
| `xml` | maintained | L2 | V0 | E0 | K2 | C1 | S1 | G0 | P2 | add cli/parity spec_test |
| `yaml` | maintained | L2 | V0 | E0 | K1 | C1 | S1 | G0 | P2 | add a corpus/upstream test |

### Languages (Prose axis)

| Language | Presence | Prose | P1 | P2 | P3 | P4 | Presence reason |
|---|---|---|---|---|---|---|---|
| `bash` | present | P2 | met | met | not-met | not-met | TestProseP0_bash passed |
| `c` | present | P2 | met | met | not-met | not-met | TestProseP0_c passed |
| `cpp` | present | P2 | met | met | not-met | not-met | TestProseP0_cpp passed |
| `csharp` | present | P2 | met | met | not-met | not-met | TestProseP0_csharp passed |
| `css` | present | P2 | met | met | not-met | not-met | TestProseP0_css passed |
| `go` | present | P3 | met | met | met | not-met | TestProseP0_go passed |
| `java` | present | P2 | met | met | not-met | not-met | TestProseP0_java passed |
| `javascript` | present | P2 | met | met | not-met | not-met | TestProseP0_javascript passed |
...
| `tsx` | present | P2 | met | met | not-met | not-met | TestProseP0_tsx passed |
| `typescript` | present | P2 | met | met | not-met | not-met | TestProseP0_typescript passed |
...
5. **Reporting denominator**, resolved: the real format dirs
   ...
   count is code-controlled, so the dashboard and the generated snapshot derive
   it from the universe rather than a hardcoded number.
