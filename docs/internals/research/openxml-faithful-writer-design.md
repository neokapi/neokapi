...
(preserve source `rPr`/styles, only substitute translated text, via the
...
1. **WSO is Okapi-mimicry** (`style_optimization.go`, gated by
   ...
   make native byte-match Okapi's *compact* output, which is nothing the spec asks
...
2. **WSO is globally order-coupled.** Synth-style IDs come from a single
   **monotonic counter** (`idCounter` in `writer.go:Write`, mirroring
   ...
   synthesise/skip decision per paragraph, or every subsequent ID shifts, so
   any per-paragraph fix cascades into the 178 passing fixtures. This is
...
|---|---|---|---|
| 1 | Faithful-default writer, WSO opt-in | #597, #598b, 847-3 cascade (in product) | **keep**: the toggle already exists |
...
| 3 | "Sound WSO": content-address synth IDs + faithful `RunFonts`/`RunMerger` port | #597, 847-3 cascade, residual-7 | **reject**: this is "port more Okapi"; it *adds* complexity; IDs must still match Okapi visit order |
| 4 | **Synthesis: 1 + effective-rPr comparator + §16 field machine + delete WSO** | all 3 bugs + residual-7 + 1083/1145 | **adopt**: a net *deletion*, spec-first, reusing the established xliff pattern |
...
currently doesn't); it is only relevant if we wanted an opt-in WSO-on parity
...
- **The 66 canon→div are pure style-indirection**: the *only* difference on
  ...
  synth def in `styles.xml`, while native keeps the formatting inline, which is
...
5. **Delete WSO** (`style_optimization.go` synth machinery, `idCounter`,
   `commonRFonts`, related strip/merge passes) once 4a-4d are green, the
   net simplification.
