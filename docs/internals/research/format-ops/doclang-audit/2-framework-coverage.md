...
**Run** (`run.go:123-148`) is a discriminated union, exactly one of:
...
|---|---|---|
| **G0** opaque/flat | `Media` part or undifferentiated text blocks; no `Role`, no Group nesting, no geometry | none (default) |
...
| **Overlays / standoff** (segmentation/term/entity/check/alignment) | YES: `Overlay`+`RunRange`+`Span` (`overlay.go`) | not a G concern (interpretations, not structure); meta.terminology/text-analysis on Vocabulary | N/A: doclang carries none |
