...
- **Axis gap**: the model represents it, but no maturity axis (G included) rewards it.
...
## COVERED (big-ticket confirmations, so the report is balanced)
...
| **Overlays / segmentation / change-tracking / L10n targets+variants** | `Overlay`+`RunRange`+`Span`; `Targets map[VariantKey]`; `meta.change-tracking` | Vocabulary | **Covered, and the model EXCEEDS DocLang** (DocLang is monolingual, no standoff/segmentation). |
| **Visibility / presence-condition** | `Visibility*` ×5 | none | **Model superset** (no DocLang counterpart). |
...
**Class: Floor-signal gap (calibration bug). Severity: HIGH, leaving format value
...
But DocLang's `<layer>` enum is only `{body, background, furniture}` (`spec.md:2587`), with
**no metadata value**, and the `doclang` reader emits no relations and drops captions on
...
**Class: Model gap (soft) + Floor-signal gap. Severity: HIGH, blocking faithful
...
**Class: Axis gap + Vocabulary gap (relation vocab). Severity: MODERATE-HIGH, blocking
...
"`SetSemanticRole` **AND** (table Groups **OR** AddRelation)", so relations are *optional* and a
...
deliberately OR-lenient, or the grade overstates relation recovery.

---

### #4. Forms / fields model is thin and entirely unmeasured
**Class: Axis gap (+ soft model). Severity: MODERATE, blocking faithful representation of
...
distinguishes form recovery, so a form reader scores generic G3. The doclang reader drops the
whole cluster.
...
Not a G-axis blocker; note it as a known under-measurement.
...
line 877), so there is no inline directionality run construct, and the Run union has no
...
**Class: Model gap (minor). Severity: LOW-MODERATE, since geometry is read-only reconstruction
metadata; only bites non-square normalized grids.**

DocLang `location@resolution` is a per-axis exclusive bound defaulting to
`default_resolution@width|height` (`spec.md:2568, 3025`), so width and height resolution can
...
treat as a provenance property, though DocLang models it as formatting).

---

### #8. Fine subclass / subtype dropped (code language, picture chart-type, free `<label>`)
**Class: Vocabulary/model nit. Severity: LOW, though code-language is a DNT signal.**
...
**Class: Floor-signal gap. Severity: LOW-MODERATE, from G2 false-positives.**
...
DocLang's biggest differentiator surface, `<head>` core metadata plus the FUTURE
...
marker, and the G4 signal greps `SetGeometry` without checking `Page`, so a reader can pass G4
