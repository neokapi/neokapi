...
The union (`core/model/run.go:127-135`):
...
- `core/model/displayhint.go:1-9` defines `DisplayHint{Preview, Context, MaxLength, ContentType}` ("heading", "button", "paragraph"), but **no format reader sets it**; only `bowrain/connector/figma.go:164` and the sync converters (`core/venue/convert.go:157`, `host/venue/client/sync_convert.go:219`).
...
This is the direct answer to "does any canonical cross-format vocabulary exist": **yes, and it is a real registry rather than a set of constants.**

- `core/model/vocabulary.go:69-127` defines `VocabularyRegistry` with `Load(json)`, `LoadDefaults()`, `Lookup/LookupOrFallback`, `IsEntityType`, `Categories()`. Each type resolves to a `SpanTypeInfo` (`vocabulary.go:12-22`): `Category, Label, HTML{open,close,placeholder}, Display, ChipLabel, Color{bg,border,text}, Equiv, Constraints`.
...
- **TS mirrors** (declared mirrors, but hand-copied): `packages/kapi-format/src/vocabulary.ts` ("Mirrors neokapi's core/model/vocabulary.go", `JSX_VOCABULARY`); Go-side mirror back at `core/kbf/preview.go:9-55` (`JSXVocabulary`, "Kept in sync by a round-trip test"); KBF files can even declare vocab deps through the `Vocabulary` field (`core/kbf/schema.go:117-129`); copied JSON packs at `packages/ui/src/vocabularies/` and `bowrain/packages/ui/src/vocabularies/`.
...
**Found drift (load-bearing for any V-axis):** `core/model/vocabularies/common-formatting.json` contains `fmt:strikethrough`/`fmt:superscript`/`fmt:subscript` entries that BOTH TS copies lack (`diff` confirms; the TS side happens to recover them from `rich-html.json`), and the TS `index.ts` loads only 3 packs, so `rich-jsx.json` never reaches the frontend registries. Three hand-maintained copies, **no drift gate** (contrast: `make check-contract-types` exists for IO-contract types).
...
- Mapping table `core/formats/html/reader.go:92-105`:
  ...
  ```
- Reader owns a registry (`reader.go:111,123-124` `vocab.LoadDefaults()`). Run construction `core/formats/html/tokenreader.go:816-852`: `b.AddPcOpen(spanID, semType, "html:"+childTag, string(rewriteInlineTagWithRefs(tokenRaw,…)), info.Display.Open, info.Equiv, constraints-from-vocab)`, giving **Type = canonical, SubType = `html:b`, Data = exact raw tag bytes**, with display/equiv/constraints pulled from the vocabulary. Untranslatable inline markup degrades to `"code:markup"` Ph (tokenreader.go:806-813).
...
- `rPr` → runs: `core/formats/openxml/runprops.go:1338-1366` `appendOpeningRuns` does `if rp.bold { emit(TypeBold, SubTypeBold, boldOnXML(rp)) }` etc.; bold/italic/underline/strike/vertAlign become **paired codes typed `fmt:*` with the raw `<w:b/>`/`<w:u w:val="...">` XML in Data**, closing runs in reverse order (1370-1392).
...
- `core/formats/json/reader.go:544-548`, `po/reader.go:1146`, `properties/reader.go:882`: placeholders get literal `Type: "code"` rather than the vocabulary's `code:placeholder`.
...
Exists today:
- The registry + JSON packs (§2), a single canonical namespace `category:name` (`fmt:`, `link:`, `media:`, `struct:`, `code:`, `jsx:`, `entity:`, `ui:`).
...
So a stand-off `style` overlay (e.g. for formats whose styling can't be losslessly paired-coded, like ODF style spans, or for editor-authored emphasis) is architecturally trivial today. What is missing is a registered payload type and producers/consumers.
