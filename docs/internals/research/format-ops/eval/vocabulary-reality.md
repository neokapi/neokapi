...
- **TS mirrors** (declared mirrors, but hand-copied): `packages/kapi-format/src/vocabulary.ts` ("Mirrors neokapi's core/model/vocabulary.go", `JSX_VOCABULARY`); Go-side mirror back at `core/kbf/preview.go:9-55` (`JSXVocabulary`, "Kept in sync by a round-trip test"); KBF files can even declare vocab deps — `core/kbf/schema.go:117-129` `Vocabulary` field; copied JSON packs at `packages/ui/src/vocabularies/` and `bowrain/packages/ui/src/vocabularies/`.
