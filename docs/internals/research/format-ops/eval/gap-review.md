...
**Structurally missing.**
1. **Single-machine dependency.** The Okapi corpus lives at a hardcoded absolute path on one laptop (docs, both skills, and `format-triage.js:48`). No mirroring (e.g. a `docs-assets`-style GitHub release of the harvested corpus), so 15 specs and the whole parity harness die with that directory.
