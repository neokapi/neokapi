...
| json | 1 | 4KB | json:1 |
| jsx | 0 | 0 | NONE (inline spec.yaml snippets only) |
| markdown | 1 | 4KB | md:1 |
| mdx | 25 | 132KB | mdx:24 md:1 |
| memorytest | 0 | 0 | NONE (test-only package) |
| messageformat | 1 | 4KB | mf:1 |
| mif | 8 | 788KB | mif:7 md:1 |
| mo | 0 | 0 | NONE |
| mosestext | 2 | 8KB | txt:2 |
| odf | 0 | 0 | NONE (relies on okapi-testdata entirely) |
...
| tex | 6 | 24KB | tex:6 |
| tmx | 1 | 4KB | tmx:1 |
| transtable | 0 | 0 | NONE (okapi: spec inputs) |
| ts | 3 | 12KB | ts:3 |
| ttml | 3 | 12KB | ttml:3 |
| ttx | 0 | 0 | NONE |
| txml | 0 | 0 | NONE (okapi: spec inputs) |
| versifiedtext | 1 | 4KB | ver:1 |
| vignette | 0 | 0 | NONE (okapi: spec inputs) |
| vtt | 1 | 4KB | vtt:1 |
| wiki | 3 | 12KB | wiki:2 txt:1 |
| xcstrings | 11 | 84KB | xcstrings:8 md:2 json:1 |
| xliff | 1 | 4KB | xlf:1 |
| xliff2 | 0 | 0 | NONE (okapi-testdata byte-equal corpus) |
| xml | 1 | 4KB | xml:1 |
| yaml | 1 | 4KB | yaml:1 |
...
- **`okapi:` scheme:** at `core/format/spec/helpers.go:42-49`,
...
**CI:** `.github/workflows/format-acceptance.yml` runs on `macos-latest` (plutil is macOS-only),
...
2. Download current `docs-assets.tar.gz` from the release; extract into workdir
   ("base extracted (existing assets preserved)"); tolerate absence by creating a fresh
   tarball.
...
hard CI failure; CI **never records/produces** assets. The comment at lines 96-98: *"All
...
2. **Spec table:** `OKAPI_FIXTURE_SPECS` (`Makefile:547-559`) maps 12 formats to Java
   ...
   The Makefile says *"the class lists below are the source of truth … keep them in
   committed order so regeneration stays byte-stable"*.
...
1. **Release addressing: versioned tag, this repo.** Follow okapi-testdata's tag scheme,
   ...
   per-format granularity, since large binaries like idml/mif/pdf argue for per-format
   assets in ONE release so a format bump doesn't re-ship 3 MB+). Created on first
   publish with the bowrain guard: `gh release view || gh release create "$TAG" --latest=false --notes
   "Managed by scripts/publish-corpus.sh. Do not edit by hand."`. `gh` addresses by tag
   in the repo from the git remote, with no `-R` needed.
...
4. **Per-format corpus manifest**: promote the existing `SOURCES.md` convention
   ...
   `roundtrip_contract` (`byte-exact` | `semantic`, the arb vs idml distinction), plus the
...
| wiki(local) | n/a | counted under some-real above |
