...
- `expected_fail: <reason>` makes assertion failures log+record instead of failing; an unexpected PASS logs "remove the expected_fail tag" (spectest/runner.go:97–99)
...
`Assertions` struct (spec.go:225–233) has **7 fields, all reader-side, all over translatable-block source text only**:
...
Dispatch is `EvalAssertions` (helpers.go:153–198), a flat field-by-field check rather than a switch; the universe of observable state is `BlockTexts(parts)` (helpers.go:131–148), i.e. `model.RunsText(blk.Source)` for `Translatable` blocks with non-empty text. **Nothing about run structure, targets, overlays, layers, Data parts, notes, IDs, or writer output is assertable.**
...
**skip cleanly** for `okapi:` inputs (spectest/runner.go:65–69,
parity/spec/runner.go:121–127); a missing corpus never fails CI.
...
because "spec.yaml stays monolingual in neokapi terms" (runner.go:52–54). Five
translators exist:
`cli/parity/formats/{csv,fixedwidth,regex,ttml,xmlstream}_bridge_config.go`.
...
examples, and the annotation file's analogue is `severity` + `skip`.
...
- `core/formats/csv/spec.yaml:671–675`: these behaviours are "not expressible through this spec's BlockTexts assertion vocabulary", and **"there is no roundtrip / writer-output assertion type. Captured here as a known coverage gap."**
- `docs/internals/format-engineering.md:133` says assertions see only translatable block source text, and that **there is no round-trip / writer-output assertion type**.
...
- `cli/parity/roundtrip/doc.go:11–14` states the gap as design rationale: "The spec.yaml runners verify reader contracts (Block count, source text, IDs) but never invoke a writer." They prove "we can read" rather than "we can read, modify, and write back coherently".
...
- **3 features, 3 examples** (1 example/feature), zero config keys, zero `okapi_refs` (native-only: the bridge has no okf_subrip because upstream handles SubRip via okf_regex, so no parity test exists; header explains the absence of `srt_spec_test.go` in cli/parity).
...
across 30 specs but explicit `divergence_kind` only 3 times (all `bridge-gap`), so
...
- **Provenance**: `Origin` (spec.go:216) already defines three canonical grep-able forms; add a fourth: `"generated: <model>/<date> verified-by: <human|test-run>"`. Optionally promote to structured fields (`origin_kind`, `origin_ref`), though the existing free-text-after-tag convention ("the form is for grep not for parsing") suggests keeping it one string.
...
1. **Implement the promised round-trip assertion type** (`roundtrip:` on `Assertions`, `NewWriter` hook on both runners, `EvalOutputAssertions`). This resolves format-maturity.md Open question 4 and the html/csv spec-header IOUs; report under the existing `Kind="format-roundtrip"`.
2. **Add run-shape semantic assertions** (`block_runs` signatures + `has_run_with_type`) keyed to the `core/model/vocabulary.go` registry, native-strict / parity-opt-in. This makes typed-code behavior (fmt:bold, placeholders, plurals) part of the executable contract.
...
5. **Make `divergence_kind` mandatory for every `expected_fail`** (currently 3 explicit vs 140+ xfails relying on the dashboard's text heuristic). Enforce it in `Validate()` or contract-audit; the fault-attribution taxonomy is the most valuable judgment data in the system and it is mostly implicit today.
