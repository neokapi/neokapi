...
reader's 'glyphs' option)", which lets a consumer render character-precise highlights
...
Layer, with no typed/strongly-resolved reference, just an ID.
...
model. `analyze.go:11` says so explicitly: "Merged cells, nested tables, and
...
LayerStart/End, GroupStart/End, Block, Data, Media, RawDocument, Custom, where values
...
- **The asset itself** = `model.Media` (`core/model/media.go:7-17`): `Data` |
  ...
  stream as bytes; it is emitted **by URI** (M-03 `:60-65`).
...
|---|---|---|---|---|---|---|
| **doclang** | ✓ | ✓ | no | ✓ (Resolution=512 grid) | no | `formats/doclang/reader.go:308,311,314,345` |
| **docling** | ✓ | ✓ furniture | no | ✓ (SourceRef=SelfRef) | caption(✓) | `formats/docling/reader.go:222,225,228,301,346,348` |
| **html** | ✓ | ✓ plane | ✓ | no | no | `formats/html/reader.go:557,566,569` |
| **openxml** (wml/sml/dml/roles) | ✓ | ✓ | ✓ hidden | ✓ (XLSX cell grid `sml.go:295`, DML `dml.go:176`) | no | `formats/openxml/roles.go:227-237`, `dml.go:68-72,176`, `sml.go:295` |
| **markdown** | ✓ (heading/list-item) | no | no | no | no | `formats/markdown/reader.go:850,1478,1544` |
| **csv** | ✓ (table-header/cell) | no | no | no | no | `formats/csv/reader.go:234,328` |
| **odf** | no | no | no | ✓ | no | `formats/odf/geometry.go:103` |
| **idml** | no | no | no | ✓ | no | `formats/idml/geometry.go:161` |
| **pdf** (wasm bridge / kapi-pdfium plugin) | (tier tree) | no | no | ✓ (+glyphs) | no | `formats/pdf/wasm_bridge.go:198` |
| **image** (+ kapi-vision tier-3) | ✓ caption / via vision | metadata | no | ✓ via vision | ✓ caption-of | `formats/image/reader.go:213-214` |
| **docmeta** (shared) | ✓ | ✓ metadata | no | no | no | `core/docmeta/docmeta.go:65,67` |
...
| Page container / page size | **ABSENT** (only `Page int` on geometry) | none |
...
first-class representation, so multi-column/page geometry collapses to stream
...
declared read-only / writer-ignored (`structure.go:124-126`), so only DocLang/
