...
2. **An authority tier** (Tier 1/2/3, from E-08) = how that structure was *obtained*: authoritative tags vs geometric heuristic vs ML heuristic. This is provenance/confidence, NOT richness. A format can reach the top richness rung via Tier 1 (authoritative) or Tier 3 (ML guess), at the same payloads with different trust.

A new Structure/Geometry axis should **measure the richness ladder** and **carry the authority tier as a confidence qualifier**, never collapsing them.

---

## 1. The content-model carriers (where "depth" is physically stored)

All structural depth rides three **block-scoped standoff annotations** registered on the existing payload registry, additively, with no proto/KBF schema change (`core/model/structure.go:1-18`):
...
Above blocks, structural containment is carried by **Groups** (`table` / `table-row` / `picture` / `list`) emitted as `PartGroupStart/End`. The whole-image asset itself is a `model.Media` part (`PartMedia`) at depth zero, the unit a flow replaces wholesale.
...
`web/docs/contribute/architecture/multilingual/m-03-multimodal-content.md:36-42` says "treating 'image' as 'OCR' conflates [several modes]":
...
```
So a single format (`image`) **spans R0→R5 at runtime**, gated on (a) plugin installed (`vision.Available("")`, `reader.go:226`), (b) `ocr` toggle, (c) `layout` toggle + engine implementing `LayoutEngine`. The depth of an image is therefore environment-conditional rather than a static property of the format code. This is the single most important fact for a Structure/Geometry axis: for `image`/`pdf`, the achievable rung depends on installed plugins.
...
- Default config is `{OCR:true, Layout:true}` (`reader.go:57`), i.e. it aims for the top rung when the plugin is present, R0 when absent.
...
- **`vision-pdf-e2e` job** (L132-195+): builds real `kapi` + `kapi-pdfium` + `kapi-vision`, installs them, reads a real PDF with `tier3` on so pdfium renders pages and vision runs layout over the raster, exercising the **cross-process Tier-3 wiring** the smoke lane can't cover. Reuses the onnxruntime/models caches; caches `libpdfium` (`chromium/7891`).
