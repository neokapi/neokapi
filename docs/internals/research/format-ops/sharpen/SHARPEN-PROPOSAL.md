...
`core/model/structure.go`, additively, with no proto/KBF schema change
(`structure.go:1-18`).
...
the **top** rung, the hardest to recover faithfully and, for
...
| **G2** | Linear body text | The body content recovered as text in correct reading order (stream order), optionally grouped by the `Layer`/`Group` tree, with roles still generic (`Block.Type` only) and no normalized `StructureAnnotation.Role`. | OCR plain text (`vision.BlocksFromOCR`); catalog/line formats that emit `PartGroupStart` |
...
`vocabtypes` is grepped (`audit-format.py:466-475`). It is a floor-only axis with no
...
- **vs Vocabulary (V), inline meaning.** V measures *inline / run-level* meaning
  ...
  `constructs.yaml` → the free-form `Block.Type` field. G measures
  *block-level + cross-block + spatial* structure via the
  `StructureAnnotation.Role` standoff layer, a *different field* the structure-aware readers
...
authoritative tags (tier 1) or an ML guess (tier 3), at the same depth with different trust
...
| **pdf** | (L0; excluded†) | n/a | n/a | n/a | n/a | n/a | **G1 (Info) → G4** (the **only** Tier-1 holder; tagged tree=G3, geometry/glyphs=G4) | tagged (T1) › geometric › ML |
...
(`lib.mjs:94-101`), so it will vanish from the dashboard even though it is the G
...
- **docling** → fix the read-only classification so Engine isn't artificially L0;
  it is already a clean G3–G4. Inline runs (`reader.go:18-20`) keep it V0, which is
  correct, not a gap.
...
**`support.yaml` backfill**: none required for launch, because G is non-gating (§5), and
...
- **Geometry rung is `na` for non-spatial formats.** Pure key-value catalogs
  ...
  cap**, so the audit sets `structure.ceiling = G3` (or G2) and they cannot reach
  ...
  this rung", or a roles-only catalog would falsely promote to G4. Mirror
...
5. **Does `pdf` (and other plugin-provided/out-of-core formats) get a dashboard
   ...
   where the actual is 51, at `lib.mjs:85`, rubric `:57,:667`; they violate the voice rule
