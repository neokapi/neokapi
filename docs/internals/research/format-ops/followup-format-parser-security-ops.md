...
**Concrete production config (tika-server).** `taskTimeoutMillis` = "number of milliseconds to allow per task (parse, detection, unzipping, etc.)" **default 300000 (5 min)**, and on expiry "shutting down the forked server process and restarting it." `maxRestarts` default `-1` (always restart). `-spawnChild` became the default in 2.0. (https://tika.apache.org/3.0.0/api/org/apache/tika/server/core/TikaServerConfig.html ; https://www.mail-archive.com/commits@tika.apache.org/msg08017.html)
...
- **klauspost/compress zstd**: `WithDecoderMaxMemory` will "set a maximum decoded size for in-memory non-streaming operations or maximum window size for streaming operations … to control memory usage of potentially hostile content" (default 64 GiB, far too high; set explicitly), plus `WithDecoderMaxWindow` to "reject packets that will cause big memory usage." (https://pkg.go.dev/github.com/klauspost/compress/zstd)
- **archive/zip**: no built-in ratio limit, so you wrap each `File.Open()` reader in a counting `io.LimitReader` and compare uncompressed-bytes-out to compressed-bytes-in per entry (the POI `MinInflateRatio` pattern, reimplemented).
...
- No widely-adopted *per-component* "hardened against malicious input" cert exists, so neokapi's proposed "parses UNSAFE corpus without crash/hang/OOM under X MB / Y s" gate would be novel and defensible, analogous to the DARPA SafeDocs program's premise.
...
- **S3 (Hostile-hardened)**: **passes the UNSAFE corpus**, terminating for every file as `EXPECTED_REJECT`/`OK` within fixed limits (proposal: **≤256 MB RSS, ≤10 s wall-clock per file** for text formats; tunable per family), 0 `CRASH`/`HANG`/`OOM`; govulncheck clean for the format's dependency closure; round-trip drift = 0 on the wild corpus.
