...
| `web/walkthroughs/kapi-terminology-checks` | Same command change; narration ties term-check to `up`'s bound checks and the gate. | full re-capture + re-dub | none | n/a | same |
| `harness/demos/01-translate-landing-page` | Narration: `kapi translate` porcelain (content memory reuse → AI → checks); hardcoded provider name dropped. | re-dub; spot-check the capture shows `kapi translate`; if the session ran a different spelling, re-capture (AI needed). | none for re-dub | n/a | `make harness-videos` → `make publish-cdn-videos` |
