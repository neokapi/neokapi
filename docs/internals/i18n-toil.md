...
adjusts axes the same way any other tool does — by criteria, not by fiat:
...
`status` semantics: `recommended` — the skill's default pick for its ecosystem;
`supported` — sound choice, playbook provided; `caution` — adoption gets an
explicit warning (grade T3+, or E ≥ 2); `avoid` — do not start new work on it
...
- **Lockstep** — preset ids, format ids, and flags named in the references are
  claims about the CLI; a change to either side lands in one PR (AD-024's
  lockstep rule).
