...
export function TMExplorer({ project, onBack }: TMExplorerProps) {
  ...
  const [addSourceLocale, setAddSourceLocale] = useState(project.source_locale);
  const [addTargetLocale, setAddTargetLocale] = useState(
  ...
  const fetchEntries = useCallback(
    async (q: string, srcLocale: string, tgtLocale: string, p: number) => {
      try {
        const result = await tmApi.getTMEntries(
          project.id,
          q,
          srcLocale,
          tgtLocale,
  ...
  const handleSaveEdit = useCallback(
    ...
    [project.id, tmApi, editTarget, fetchEntries, query, sourceLocaleFilter, targetLocaleFilter, page],
  );
  ...
  const handleDelete = useCallback(
    async (entryId: string) => {
      try {
        await tmApi.deleteTMEntry(project.id, entryId);
        fetchEntries(query, sourceLocaleFilter, targetLocaleFilter, page);
      } catch (e) {
        console.error("Failed to delete TM entry:", e);
      }
    },
    [project.id, tmApi, fetchEntries, query, sourceLocaleFilter, targetLocaleFilter, page],
  );

  const handleAdd = useCallback(async () => {
    if (!addSource.trim() || !addTarget.trim()) return;
    try {
      await tmApi.addTMEntry(
        project.id,
        addSource,
        addTarget,
        addSourceLocale,
  ...
  return (
    <div data-testid="tm-explorer">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        <button onClick={onBack} style={backBtnStyle} data-testid="tm-back-btn">
          &#8592; Back
        </button>
      ...
      {showAddForm && (
        <div style={addFormStyle} data-testid="tm-add-form">
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            />
            <select
              value={addSourceLocale}
              onChange={(e) => setAddSourceLocale(e.target.value)}
              style={selectStyle}
            ...
            <select
              value={addTargetLocale}
              onChange={(e) => setAddTargetLocale(e.target.value)}
              style={selectStyle}
            ...
            <tbody>
              {entries.map((entry) => (
                <tr key={entry.id} style={rowStyle} data-testid={`tm-entry-${entry.id}`}>
                  <td style={tdStyle} data-testid={`tm-source-${entry.id}`}>
                    {entry.source}
                  </td>
                    ...
                    {editingId === entry.id ? (
                      <div style={{ display: "flex", gap: 4 }}>
                        ...
                        <button
                          ...
                          style={saveBtnSmallStyle}
                          data-testid={`tm-save-btn-${entry.id}`}
                        >
                          Save
                        </button>
                        <button
                          onClick={handleCancelEdit}
                          style={cancelBtnSmallStyle}
                          data-testid={`tm-cancel-btn-${entry.id}`}
                        >
                          Cancel
                        </button>
                      </div>
                    ...
                    {editingId !== entry.id && (
                      <div style={{ display: "flex", gap: 4 }}>
                        <button
                          ...
                          data-testid={`tm-edit-btn-${entry.id}`}
                        >
                          Edit
                        </button>
                        <button
                          ...
                          data-testid={`tm-delete-btn-${entry.id}`}
                        >
                          Delete
                        </button>
                      </div>
          ...
          {totalPages > 1 && (
            <div style={paginationStyle} data-testid="tm-pagination">
              <button
                ...
                disabled={page === 0}
                style={pageBtnStyle}
                data-testid="tm-prev-page"
              >
                Previous
              </button>
              <span data-testid="tm-page-info">
...
const saveBtnSmallStyle: React.CSSProperties = {
  padding: "4px 8px",
  backgroundColor: "var(--accent)",
  color: "#fff",
  border: "none",
...
const cancelBtnSmallStyle: React.CSSProperties = {
  padding: "4px 8px",
  backgroundColor: "var(--bg-tertiary)",
  color: "var(--text-primary)",
  border: "1px solid var(--border)",
