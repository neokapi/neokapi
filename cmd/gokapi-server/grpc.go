...
func (g *GRPCServer) StreamBlocks(req *pb.StreamBlocksRequest, stream pb.GokapiService_StreamBlocksServer) error {
	...
	blocks, err := g.srv.services.Project.GetBlocks(ctx(stream), query)
	if err != nil {
...
func (g *GRPCServer) ExecuteFlow(req *pb.ExecuteFlowRequest, stream pb.GokapiService_ExecuteFlowServer) error {
	...
	// Process blocks through the tool chain: feed blocks as Parts into a
	// channel pipeline where each tool reads from one channel and writes to the next.
	ctx := stream.Context()
	...
	// Collect processed blocks and update store.
	var processed int
	for part := range out {
...
// ctx extracts context from a gRPC server stream.
func ctx(stream interface{ Context() context.Context }) context.Context {
	return stream.Context()
}

// Ensure ConnectorService.Push returns a count — let's check the signature.
// The service Push needs to return (int, error).
// If it doesn't, we'll adapt here.
func init() {
	// Compile-time check that GRPCServer implements the interface.
