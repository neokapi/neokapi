...
function ToolConfig({ schemaName, source }: { schemaName: string; source: "builtIn" | "bridge" }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: hasProps ? "1fr 1fr" : "1fr", gap: 24, maxWidth: 1100 }}>
      <div>
        {schema.description && <p className="text-sm text-muted-foreground mb-3">{schema.description}</p>}
        {hasProps ? <SchemaForm schema={schema} values={values} onChange={setValues} /> : <p className="text-sm text-muted-foreground">No configurable parameters.</p>}
      </div>
...
const meta: Meta = {
  title: "Formats & Tools/Tools/Inline Codes Quality Check",
...
export const OkapiConfig: Story = {
  name: "Configuration",
  render: () => <ToolConfig schemaName="inline-codes-checker" source="bridge" />,
};
