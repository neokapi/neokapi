...
func CompareGolden(t *testing.T, goldenPath string, actual []byte) {
	...
	assert.Equal(t, string(expected), string(actual),
		"output does not match golden file %s\nRun with NEOKAPI_UPDATE_GOLDEN=1 to update", goldenPath)
}
