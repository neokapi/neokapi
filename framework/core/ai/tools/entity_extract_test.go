...
func TestAIEntityExtractTool_LLMOnly(t *testing.T) {
	...
	mock.ChatStructuredFunc = func(_ context.Context, messages []provider.Message, schema provider.JSONSchema) (*provider.ChatResponse, error) {
...
func TestAIEntityExtractTool_WithNER(t *testing.T) {
	...
	mock := provider.NewMockProvider()
	mock.ChatStructuredFunc = func(_ context.Context, _ []provider.Message, _ provider.JSONSchema) (*provider.ChatResponse, error) {
		return &provider.ChatResponse{
			Content: extractionResponse([]extractionBlock{
				{
					BlockID: "tu1",
