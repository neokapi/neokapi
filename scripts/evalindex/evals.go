...
var evals = []Eval{
	...
	},
	{
		...
		Misses:    "The corpus is synthesized, which is disclosed in the data itself. The declared fields (active_voice, person_pov, sentence_length) have no offline implementation at all, so their row is a property of the design rather than a defect to fix. And a rule stated as a regex is only as good as the regex: every false positive here is one passive-voice pattern firing on a predicate adjective.",
		Reproduce: "make authoring-eval",
		Data:      "web/src/pages/authoring-eval/_authoringeval.json",
		Page:      "/authoring-eval",
	},
