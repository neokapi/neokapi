...
var evals = []Eval{
	...
	},
	{
		...
		Covers:    "Whether the guide moves writing toward its own profile specifically. Any competent writing guidance raises any reasonable score, so the reference gain alone proves nothing; the measurement is the difference between the two gains. Guided writing gained 5.8 points against the reference profile and lost 6.7 against the contrast, an effect of 12.5.",
		Misses:    "Six documents on one model. Scoring is the offline check, which evaluates forbidden terms and prohibited patterns and not the declared fields — so the person and sentence-length instructions the model read are not in the score. Part of the contrast arm's drop is mechanical: the contrast profile forbids `you`, and the guide asks for second person.",
