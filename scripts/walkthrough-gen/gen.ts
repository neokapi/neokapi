...
//
// Reads:   web/docs/walkthroughs/<id>.scene.yaml   (the single source)
// Emits:   web/docs/scenes/<id>/0N-<scene>.tape    (VHS recording source)
//          web/docs/src/components/KapiPlayground/embeds/<id>.embed.ts
//          web/docs/src/components/KapiPlayground/embeds/index.ts (registry)
//          keeps web/docs/walkthroughs/<id>.md smoke_contract: in sync
//
...
const WALKTHROUGHS_DIR = join(REPO_ROOT, "web/docs/walkthroughs");
const SCENES_DIR = join(REPO_ROOT, "web/docs/scenes");
const EMBEDS_DIR = join(REPO_ROOT, "web/docs/src/components/KapiPlayground/embeds");
...
function generateEmbedConfig(spec: SceneSpec): string {
  ...
  return `// GENERATED by scripts/walkthrough-gen from web/docs/walkthroughs/${spec.id}.scene.yaml.
...
// arrays from web/docs/walkthroughs/*.md front matter. We rewrite that one
...
function main(): void {
  ...
  if (ids.length === 0) {
    console.error(
      "No scene specs found. Pass an <id> or create web/docs/walkthroughs/<id>.scene.yaml.",
    );
    process.exit(1);
  }
