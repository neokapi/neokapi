...
func TestColdStartCodexConfigTrustsTheFixtureAndNamesNoServer(t *testing.T) {
	paths := coldStartPaths(filepath.Join(t.TempDir(), "release-note-codex"))
	prepared := ColdStartPrepared{
		Session: ColdStartSession{Host: PairedAgentSpec{Host: "codex", Model: "gpt-5.6-terra", Effort: "medium"}},
		Paths:   paths,
		Env:     coldStartEnv(paths),
	}

	config := coldStartCodexConfig(prepared)
...
func TestColdStartCodexForwardsEveryKapiRoot(t *testing.T) {
	paths := coldStartPaths(filepath.Join(t.TempDir(), "release-note-codex"))

	override := coldStartCodexForwardedEnv(coldStartEnv(paths))
...
func TestColdStartCellsDir(t *testing.T) {
	repo := t.TempDir()
	cells := t.TempDir()

	defaulted, err := coldStartCellsDir(ColdStartOptions{RepoRoot: repo})
	...
	named, err := coldStartCellsDir(ColdStartOptions{RepoRoot: repo, CellsDir: cells})
	require.NoError(t, err)
	assert.Equal(t, cells, named)

	for _, inside := range []string{repo, filepath.Join(repo, "harness", "out", "cells")} {
		_, err := coldStartCellsDir(ColdStartOptions{RepoRoot: repo, CellsDir: inside})
...
func TestColdStartCellsDirIsCoveredByTheAncestorCheck(t *testing.T) {
	cells := t.TempDir()
	repo := filepath.Join(cells, "release-note-codex", coldStartRepoName)
