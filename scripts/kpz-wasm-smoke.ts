...
//
// Usage: node --experimental-strip-types scripts/kpz-wasm-smoke.ts \
...
// 1) .kpz workspace lifecycle on a text format.
console.log("kpz-wasm-smoke: .kpz workspace (JSON)");
mem.vol.writeFile("/p/app.json", enc.encode('{"a":"Hello world"}'));
ok("extract", await run("extract", "/p/app.json", "-o", "/p/w.kpz", "--target-lang", "qps"));
ok("transform", await run("pseudo-translate", "/p/w.kpz"));
const dirty = await run("info", "/p/w.kpz");
if (!/dirty/.test(dirty.out)) { console.error("FAIL: info should report dirty\n" + dirty.out); process.exit(1); }
ok("info(dirty)", dirty);
ok("pack", await run("pack", "/p/w.kpz"));
const clean = await run("info", "/p/w.kpz");
if (!/clean/.test(clean.out)) { console.error("FAIL: info should report clean after pack\n" + clean.out); process.exit(1); }
ok("info(clean)", clean);
ok("merge", await run("merge", "/p/w.kpz", "-o", "/p/out/"));
...
console.log("kpz-wasm-smoke: .kpz workspace (Office .docx)");
const docx = new Uint8Array(readFileSync("core/formats/openxml/testdata/test_859.docx"));
mem.vol.writeFile("/p/sample.docx", docx);
ok("extract docx", await run("extract", "/p/sample.docx", "-o", "/p/d.kpz", "--target-lang", "qps"));
ok("transform docx", await run("pseudo-translate", "/p/d.kpz"));
ok("merge docx", await run("merge", "/p/d.kpz", "-o", "/p/dout/"));
