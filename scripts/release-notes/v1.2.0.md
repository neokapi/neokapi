...
- `kapi apply` and the MCP tool `apply_edits` take comment edits for Go and, with the sourcecode plugin 0.3.0, for TypeScript, TSX and JavaScript. An edit in one of those three languages is written only when the project's formatter agrees with it, and running that formatter needs execution trust: `kapi apply` asks at a terminal or reads `KAPI_TRUST_EXEC`, and `apply_edits` runs it only where trust is already recorded. Without trust, the edit reports that it did not run and nothing is written.
