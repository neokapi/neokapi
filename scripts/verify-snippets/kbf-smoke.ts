#!/usr/bin/env node
// Headless parity smoke for the KBF wasm endpoint: boots kapi-cli.wasm in Node,
// then for the core spec operations compares the canonical Go engine (via the
// `kbf` endpoint) against the TypeScript mirror (@neokapi/kapi-format imported
// from source). This is the headless equivalent of the docs KBF Tests page.
...
const goRound = kbf({ op: "roundtrip", kbf: `${JSON.stringify(file, null, 2)}\n` });
...
// Edge cases that previously diverged Go↔TS (see /kbf-tests conformance):
...
const goEdge = kbf({ op: "roundtrip", kbf: `${JSON.stringify(edge, null, 2)}\n` });
...
for (const b of [filesHeading, shoppingCart]) {
  const goHtml = kbf({ op: "renderHtml", block: b }).html;
...
for (const [name, t] of [["valid", validTarget], ["missing", missingTarget]] as const) {
  const goK = normKinds((kbf({ op: "validateTarget", source: filesHeading, target: t }).errors ?? []).map((e: any) => e.kind));
...
const ucErrs = (kbf({ op: "validateBlock", block: unclosed }).errors ?? []).map((e: any) => e.kind);
ok("validateBlock flags unclosed-paired-code", ucErrs.includes("unclosed-paired-code"), JSON.stringify(ucErrs));

console.log(failures === 0 ? "\nALL KBF PARITY CHECKS PASSED" : `\n${failures} KBF CHECK(S) FAILED`);
