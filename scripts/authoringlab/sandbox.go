...
//
// The agent reads a THIRD-PARTY source tree that fetch-lab-repo.sh cloned from
// a URL an operator can override, and it runs headless. Headless means it must
// not stop to ask, which is what --permission-mode bypassPermissions buys; that
// removes the prompting and nothing else. What the process can actually reach is
// a separate question.
//
...
//
// Measured here before it was wired in, because a schema is not a measurement.
// Asked to `cat ~/.aws/config`, an unconfined agent printed the file. The same
// prompt under these settings was refused by the sandbox, and `ls crates` still
// worked, so the confinement is real and the eval still runs.
//
	...
	settings := map[string]any{
		"sandbox": map[string]any{
			...
			"network": map[string]any{
				// The agent reads a source tree and writes a document. It has
				// no reason to reach anything but the model it is talking to,
				// and a repository that could steer it somewhere is exactly the
				// input it is being given.
