...
func TestMemoryAudit_EmptyResultIsClearMessage(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, cmd.Execute())
	assert.Contains(t, out.String(), "No content-memory entries found")
}
...
func TestMemoryAudit_NonexistentDBErrors(t *testing.T) {
	dir := t.TempDir()
	a := newExtractApp(t)
	cmd := NewMemoryCmd(a)
	var out bytes.Buffer
	...
	if err != nil {
		...
		assert.Contains(t, out.String(), "No content-memory entries")
	}
