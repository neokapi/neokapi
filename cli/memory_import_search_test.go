...
// `kapi memory import <file>` RunE (with --file pointing at a fresh SQLite content memory).
// It returns the resolved content memory db path so callers can reopen and assert.
func runSingleTMXImport(t *testing.T) string {
	...
	// seeds cmd.Context(); set it here so the ctx-aware content-memory import path has a
...
// TestMemoryImportSingleFile_RebuildsSearchIndex is the regression test for
// finding #38: a single-file `kapi memory import` must rebuild the FTS5 side-tables
// so the imported entry is visible to `kapi memory search` (SearchEntries), not
...
func TestMemoryImportSingleFile_RebuildsSearchIndex(t *testing.T) {
	...
	// FTS5-backed search — the surface `kapi memory search` exercises.
	entries, total, err := tm.SearchEntries(t.Context(), memory.SearchParams{
...
// TestMemoryImportSingleFile_FuzzyLookup asserts the fuzzy (trigram) index is also
...
func TestMemoryImportSingleFile_FuzzyLookup(t *testing.T) {
	dbPath := runSingleTMXImport(t)
	...
	matches, err := tm.LookupText(t.Context(), "Welcome to the dashboard!", "en", "fr", memory.LookupOptions{
		MinScore:   0.5,
		MaxResults: 5,
	})
	require.NoError(t, err)
