...
- **Comments in code**: `comments: true` on a content item checks the comments
  ...
  JavaScript, Python, Bash, CSS, Rust, Java, C#, C and C++ comments are read
  the same way once the sourcecode plugin is installed (`kapi plugins install sourcecode`); without it
