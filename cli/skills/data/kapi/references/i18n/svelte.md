...
`npx sv add paraglide`, which is what the Svelte CLI itself scaffolds, and
...
- **Recommended config (what earns T1):**
  - **SvelteKit: scaffold with `npx sv add paraglide`.** The official Svelte
    ...
    (`@inlang/paraglide-sveltekit`, `paraglide-astro`) are deprecated, so
  ...
  - **kapi pseudo-translate + translate-qa on `messages/*.json`** (the
    registry's `buys`). The compiler proves presence rather than correctness;
    kapi covers the runtime-correctness axis.
...
- **Footguns:** **static keys only**, because `m[runtimeKey]` defeats the
  model; apps with CMS/runtime-driven strings need a runtime library instead.
  All locales of a *used* message are bundled together: efficient under ~20
  locales, a real cost beyond that. The steward is Opral, a startup that
  already deprecated every v1 framework adapter in the 2.0 move, so expect
  API churn appetite. Variants are younger and less battle-tested than
  formatjs/ICU.
...
- **Migration path to Paraglide:** `npx sv add paraglide`; move each
  `{lang}.json` dictionary into `messages/{lang}.json` (flatten nested keys, so
  `page.title` becomes `page_title`); convert ICU plural strings to Paraglide
  v2 variants (or the ICU MF1 inlang plugin); replace `$_('page.title')` /
  `$t(...)` with `m.page_title()`; delete `register()`/`init()`/`$isLoading`
  wiring. The compiler then reports every miss as a type error, which is the
  migration checklist.
...
- **Built-in i18n routing** (Astro 4+, carried into 5): `i18n: { locales,
  ...
  only, with no message-catalog system**, so pair it with one:
  - **Anything real:** built-in routing + **Paraglide** via the unified Vite
    plugin, so messages are tree-shaken per island and only client-component
  ...
  - **Tiny sites** (<10 UI strings, few locales): the official docs-recipe
    `ui.ts` dictionary + `useTranslations()` helper, with zero dependencies.
