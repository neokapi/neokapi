...
- **Recommended config (what earns the grade):**
  1. **`HardcodedText` + `MissingTranslation` lint as CI ERRORS.** Both are
     ...
     than XML layouts, so pseudo-translation (below) is the backstop.
  2. **Per-app language (API 33+):** AGP auto-generates the locale config with
...
- **Plural traps (all real, all shipped by someone):**
  ...
  - **A bad placeholder crashes only in that locale:** `%s` mistyped in one
    ...
    and stays invisible when only English is checked. kapi preserves printf
    placeholders and `translate-qa` validates them; that plus
    pseudo-translation is the guard.
...
- **kapi:** **you MUST pass `--format androidxml`**, because `.xml` is a
  shared extension and the format is never inferred from the filename:
  ```bash
  ...
  Target files may be partial, because resource resolution falls back to
...
- **Footguns:**
  - Missing translations are masked at runtime by fallback to the default
    locale; gaps surface in the checks at best. The lint-as-error config is
    the fix; without it, tell the user they're at the stock S2/R2 grade.
  ...
  - Compose: caching resolved strings in remembered state or ViewModels goes
    stale on locale change, so resolve `stringResource` in composition.
...
marking, no extraction, the same T2 profile; the registry has no separate
entry). JetBrains' built-in **Compose Multiplatform resources**
(`composeResources/values*/strings.xml`, generated `Res.string.*`,
`stringResource()` in common code) is the default for Compose-everywhere
apps. **moko-resources** fits when shared Kotlin logic must serve *native*
...
visible UI is translated. Remember that fallback masks gaps, so also run the
