...
- **Idiom:** **source-text-as-key with auto-generated hash IDs.** Mark
  templates with the `i18n` attribute (`i18n-title` etc. for attributes), and
  the English text stays in place; code strings use `` $localize`...` `` tagged
...
- **Recommended config (what earns T1).** The headline: **stock sync is S3
  ...
  `ng extract-i18n` regenerates only the *source* file, with no built-in
...
- **Idiom:** message IDs (nested JSON) with **scopes** as a first-class
  ...
  catalog loads, so no flicker), `| transloco` pipe, or the `translate()`
...
- **Recommended config (what earns T2):** upgrade to **v18** (Angular 18–22;
  ...
  **`@vendure/ngx-translate-extract`** in CI (the maintained fork, since the
  original extractor was itself abandoned) and
  **`ngx-translate-messageformat-compiler`** for ICU plurals, because there
  is no plural support built in, only interpolation. Add a typed-key wrapper;
  keys are otherwise unchecked strings that slip to production.
...
- **Footguns:** missing keys render the raw key (or fallback-language text)
  ...
  standalone providers/loader API, so budget that migration when upgrading old
  apps.
