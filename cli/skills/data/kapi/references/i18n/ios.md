...
| Legacy `.strings`/`.stringsdict` | n/a | Migration target; one-click Xcode migrator. kapi handles them until you migrate. |
...
for strings**, and warn the user if they ask: its xcstrings support was never
...
- **Recommended config (what earns T0):** the residual toil is git hygiene,
  ...
  1. `.gitattributes` **union merge driver**, because the
     single-JSON-all-languages design makes concurrent edits conflict
     structurally (a conflict marker mid-JSON makes the file unparseable):
     ```
  ...
  is the r-axis buy, with no export step needed.
...
Warn the user: this is a migration target rather than a place for new work.
Xcode's one-click "Migrate to String Catalog" moves both files losslessly, and
all new tooling investment (symbols, agent-driven translation) lands on
`.xcstrings` only. Until the migration happens, kapi reads and writes both via
`--format applestrings` (the flag is required, because `.strings` alone
doesn't identify the dialect):
