...
|---|---|---|
| **Docusaurus** | **T1** | UI strings extracted to JSON; content is whole-file copies, and kapi convergence supplies the missing drift detection. |
...
Docs content is prose in whole files rather than string catalogs, so the
failure mode is **drift** (source edited, translation stale) rather than
missing keys. Two
...
- **Footguns:** hand-edits inside `i18n/<locale>/` are silently lost the next
  time the pipeline regenerates the tree, so enforce generated-only; each
...
- Drift handling is real gettext: **paragraph-level msgids** mean an edited
  source paragraph goes fuzzy in every locale, detected mechanically rather
  than silently stale. The cost: **any edit fuzzies the whole paragraph**, so
  living docs generate heavy review churn. Budget for it, or batch doc edits.
- **kapi:** the catalogs are ordinary PO, with msgctxt and plural forms
  preserved:
  ```bash
...
- **Footguns:** reST/MyST markup inside msgids is easy to mangle in
  translation, so run `kapi run translate-qa` on the PO; keep `sphinx-intl
...
extraction**, so every `{{ i18n "key" }}` needs a hand-written entry per
...
- **Footguns:** missing UI keys render as empty/fallback silently (set
  ...
  missing from a locale 404s or falls back per config, so click through the
