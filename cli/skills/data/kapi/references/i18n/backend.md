...
|---|---|---|
| Go (go-i18n v2) | T2, caution | The de-facto standard; verbose structs, multi-step merge, workable but not paved. |
...
- **Idiom:** `nicksnyder/go-i18n` v2 is the de-facto standard
  (`golang.org/x/text`'s `gotext` extractor is still experimental, so
  don't build on it). Messages are `i18n.Message`/`LocalizeConfig` structs,
  the most verbose marking of any ecosystem here (W3 stock).
- **Recommended config (what earns T2):**
  - **Source-English-as-ID convention** (`ID: "Save changes"`, where the
    English *is* the ID): gettext-ish fallback for free, and it takes
    W3 → W2.
...
- **kapi:** no preset. Keep catalogs in JSON (goi18n supports
  `-format json`), because kapi has a JSON reader/writer but no TOML:
  ```bash
  kapi pseudo-translate active.en.json --target-lang qps
  kapi translate translate.fr.json --target-lang fr -o active.fr.json
  ```
- **Footguns:** `goi18n extract` only sees `i18n.Message` struct literals, so
  strings written another way are invisible, and there is no template
...
`ResourceBundle` is frozen bedrock: nothing has displaced it, and nothing has
improved its workflow.
...
- **Footguns:** positional `{0}` args hurt translator comprehension (ICU
  ...
  leaves stale translations in every other locale, and nothing detects it.

## .NET (.resx / IStringLocalizer): T3, warn first

**Warn the user: .NET has no extraction (X=3, so the T3 stands), and
...
The best *message syntax* ever designed, offering structural divergence from
the source (Fluent's own "asymmetric localization" model), terms, attributes
and per-locale variants, with runtime correctness at the top of the field
...
on it.** Also: **kapi has no `.ftl` reader (known gap), and translations
of Fluent files are not kapi-mediated yet**, so pseudo-translate/translate-qa
don't apply to them.
...
Run the service/app in the target locale and confirm the real output (not just
