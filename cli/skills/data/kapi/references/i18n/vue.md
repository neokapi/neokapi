...
- **Recommended config (what earns T2):**
  ...
  - **`@intlify/unplugin-vue-i18n` is mandatory, not optional** (registry
    ...
    stays out of the client bundle (perf) and the app is CSP-safe, because the
    full build's runtime compiler historically required `unsafe-eval`. Alias
    to the runtime-only build; the plugin's `include` glob defines the catalog
    layout.
  ...
  - **Schema-typed `useI18n`:** type the resource schema so keys get
    completion and checking. It is off by default, so turn it on.
...
- **Footguns:** messages use the **Intlify format, not ICU**: plurals are
  ...
  July 2025, so be on v11 (npm may show a spurious "deprecated" notice on v11;
  that was a v9/v10 deprecation mistake, not real). Renamed keys silently
  orphan translations, so rename via lint-verified refactors. Messages fetched
...
- **Recommended config (what earns T2):**
  ...
  - **`experimental.typedOptionsAndMessages`** for typed keys, though it
...
translated. Still in source language? The wiring is wrong rather than the
translation. Usual causes: key missing from the target catalog (vue-i18n
falls back to the source locale silently, which is why `no-missing-keys` runs
