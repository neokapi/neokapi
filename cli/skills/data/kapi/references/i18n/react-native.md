...
**Shared web+RN codebases: whichever the web side already uses wins**, and
never run two i18n frameworks over one catalog (see [react.md](react.md) for
the web-side setups).
...
- **Idiom:** developer-invented keys in namespaced JSON
  ...
  feature-scoped IDs + `defaultValue` is the maintainers' idiom, so keep it.
- **Recommended config (what earns T2; skip these and it degrades to T3, so
  say so):**
  ...
  2. **FormatJS Intl polyfills on iOS** (the r buy): Hermes ships full
     ECMA-402 on Android but **NOT on iOS**, so `Intl.PluralRules` (and
  ...
  3. **Typed keys via `CustomTypeOptions`** module augmentation, giving
     compile-time key checking when catalogs are statically imported.
...
- **Footguns:**
  ...
  - Polyfill weight/perf on iOS, hence `polyfill-force` + per-locale data
    rather than the blanket polyfills.
  - OTA-updated JS bundles carry catalogs that can drift from store-listing
    language metadata and the per-app language declarations, so update both
...
- **Idiom:** source-as-key macros (`<Trans>Welcome {name}</Trans>`,
  `` t`Welcome` ``) with no key invention; `lingui extract` walks source and
...
- **Needs the same iOS Intl polyfills as the default stack.** Warn the user
  that Lingui does not exempt them from the Hermes gap.
