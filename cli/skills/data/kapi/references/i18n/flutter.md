...
| easy_localization | **avoid** | Runtime string keys, so typos ship to production. Prototypes only. |
...
- **Recommended config (what earns T2):**
  1. `l10n.yaml` + `generate: true` in `pubspec.yaml` so codegen runs on
     ...
     **removed**, so generate into the source tree and import the generated
  ...
  3. The build **fails on template placeholder errors** (a missing
     placeholder referenced in `app_en.arb`); that's the r-axis working, so
     don't suppress it.
...
- **Footguns:** verbose call sites (the extension fixes it); the
  ...
  niche next to strings.xml/xcstrings, with fewer editors, though every
  major TMS and kapi handle it; adding a key always means touching the ARB *and*
  regenerating, and stale codegen shows as "getter not defined" errors.
...
- **Idiom:** hierarchical JSON/YAML/CSV in `i18n/` (per-locale files,
  ...
  `t.checkout.title` with no `context` needed. Codegen via `dart run slang`
...
incomplete (S=3, so the gap-finding is entirely on you); maintenance is
...
compile-safe default. If they're already on easy_localization, both
