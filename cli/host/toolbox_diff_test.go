...
<trans-unit id="settings"><source>Settings</source><target></target></trans-unit>
	...
	t.Run("text", func(t *testing.T) {
		out, err := captureStdout(t, func() error {
			return app.RunDiff(context.Background(), []string{xliff}, DiffOptions{TargetLocale: "fr"})
		})
	...
	t.Run("json", func(t *testing.T) {
		out, err := captureStdout(t, func() error {
			return app.RunDiff(context.Background(), []string{xliff}, DiffOptions{TargetLocale: "fr", JSON: true})
		})
		require.ErrorIs(t, err, ErrSilentExit)
