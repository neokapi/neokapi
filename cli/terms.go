...
A terms store is a glossary of approved terms stored as a SQLite database.
Use these commands to import, export, look up, and manage terms.

Resource location (mutually exclusive):
  --name <n>      Named terms in KAPI_HOME (~/.config/kapi/termbases/<n>.db)
  --local         Terms in current directory (./termbase.db)
