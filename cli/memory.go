...
kmb (Kapi Memory Bundle) is the native Memory form: a deterministic, lossless JSON
...
and for seeding a fresh Memory exactly. --locales does not apply.

kmz (--format kmz, or a -o path ending in .kmz): the same bundle inside a
compressed single-member container — the at-rest/handoff form for a large Memory.`,
		RunE: func(cmd *cobra.Command, args []string) error {
