...
// TestTBSQLiteGolden asserts the rendered SQLite DDL is byte-identical to the
// historical hand-written termbase migrations, so existing user databases need
// no schema migration.
func TestTBSQLiteGolden(t *testing.T) {
	cases := []struct {
		name string
		got  string
	}{
		{"tb_sqlite_v1.golden.sql", RenderTBSQLiteV1()},
		{"tb_sqlite_v2.golden.sql", RenderTBSQLiteV2()},
		{"tb_sqlite_v3.golden.sql", RenderTBSQLiteV3()},
	}
...
// The historical known-good Postgres termbase migrations (bowrain/termbase),
...
// TestTBPostgresSemanticEquivalence asserts the descriptor-rendered Postgres DDL
...
func TestTBPostgresSemanticEquivalence(t *testing.T) {
	assertStmtSetEqual(t, "v1 create", pgV1Reference, RenderTBPostgresV1())
	assertStmtSetEqual(t, "v2 stream", pgV2Reference, RenderTBPostgresV2())
	assertStmtSetEqual(t, "v3 fuzzy", pgV3Reference, RenderTBPostgresV3())
	assertStmtSetEqual(t, "v4 brand graph", pgV4Reference, RenderTBPostgresV4())
}
