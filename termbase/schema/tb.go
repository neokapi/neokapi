// Package schema declares the terminology (termbase) table layouts once, as
// data, and renders them to SQLite or PostgreSQL DDL through the dialect seam
// in core/storage/schema. The two termbase backends — framework SQLite
// (termbase) and bowrain Postgres — both build their migration SQL from these
...
//
// This mirrors the TM (sievepen/schema) dialect seam; see that package for the
// TM half of the same refactor.
package schema
...
// Termbase table descriptors. Column specs are written side by side per dialect
...
var (
	tbConcepts = sq.Table{
		...
		Columns: []sq.Column{
			...
			// project_id scopes local (per-file) termbases; Postgres scopes by
...
// RenderTBSQLiteV1 renders the v1 SQLite termbase migration body, byte-identical
...
// RenderTBSQLiteV2 renders the v2 SQLite migration: source on concepts and the
// competitor_term flag on terms. SQLite ADD COLUMN takes no IF NOT EXISTS.
func RenderTBSQLiteV2() string {
	return "\n" +
...
// RenderTBSQLiteV3 renders the v3 SQLite migration: the relations table plus the
// term validity columns.
func RenderTBSQLiteV3() string {
...
// RenderTBPostgresV1 renders the fresh v1 Postgres schema: concepts (no stream
...
func RenderTBPostgresV1() string {
...
// RenderTBPostgresV2 renders the v2 Postgres migration: the stream column on
// concepts and its index.
func RenderTBPostgresV2() string {
	o := sq.Opt{IfNotExists: true}
	return tbConcepts.AddColumn(sq.Postgres, sq.Opt{}, "stream") +
		tbConcepts.CreateIndexes(sq.Postgres, o, "idx_tb_concepts_stream")
}

// RenderTBPostgresV3 renders the v3 Postgres migration: the pg_trgm fuzzy index
// and the tsvector search column/trigger.
func RenderTBPostgresV3() string {
	return pgTermsFuzzyBlock
}

// RenderTBPostgresV4 renders the v4 Postgres migration: the source column on
// concepts, the competitor/validity columns on terms, and the relations table.
func RenderTBPostgresV4() string {
	o := sq.Opt{IfNotExists: true}
