...
func TestSQLiteMemory_MultilingualAdd(t *testing.T) {
...
func TestSQLiteMemory_MultilingualUpdate(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
...
func fullEntry(id string) memory.Entry {
...
// TestSQLiteMemory_DeleteRemovesAllChildRows asserts that Delete removes every
...
// TestSQLiteMemory_DeleteMissingEntry asserts that deleting an unknown ID reports
...
func TestSQLiteMemory_DeleteMissingEntry(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
...
func TestSQLiteMemory_NoVariantsError(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	err = tm.Add(context.Background(), memory.Entry{ID: "e1"})
	assert.ErrorIs(t, err, memory.ErrEntryNoVariants)
}

// --- Lookup ---

func TestSQLiteMemory_LookupPlain(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	require.NoError(t, tm.Add(context.Background(), trilingual("e1", "Save", "Enregistrer", "Speichern")))

	matches, err := tm.LookupText(context.Background(), "Save", "en", "fr", memory.DefaultLookupOptions())
	require.NoError(t, err)
...
func TestSQLiteMemory_LookupCrossDirection(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	require.NoError(t, tm.Add(context.Background(), trilingual("e1", "Save", "Enregistrer", "Speichern")))
	...
	matches, err := tm.LookupText(context.Background(), "Enregistrer", "fr", "de", memory.LookupOptions{MinScore: 1.0, MaxResults: 5})
	require.NoError(t, err)
	require.Len(t, matches, 1)
	assert.Equal(t, "Speichern", matches[0].Entry.VariantText("de"))
}

func TestSQLiteMemory_LookupMissingTarget(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	matches, err := tm.LookupText(context.Background(), "Save", "en", "de", memory.DefaultLookupOptions())
	require.NoError(t, err)
	assert.Empty(t, matches)
}

func TestSQLiteMemory_LookupFuzzy(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
...
func TestSQLiteMemory_SearchAnyLocale(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "hello", AnyLocale: "", RequireLocale: "", Offset: 0, Limit: 10})
...
func TestSQLiteMemory_SearchRequireLocale(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
...
func TestSQLiteMemory_SearchFilterSession(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()

	// Create sessions and tag entries with them.
	require.NoError(t, tm.CreateImportSession(context.Background(), memory.ImportSession{ID: "s1", FileKey: "a.tmx"}))
	require.NoError(t, tm.CreateImportSession(context.Background(), memory.ImportSession{ID: "s2", FileKey: "b.tmx"}))

	e1 := trilingual("e1", "one", "un", "eins")
	e1.Origins = []memory.Origin{{Source: "import", Key: "a.tmx", SessionID: "s1"}}
	e2 := trilingual("e2", "two", "deux", "zwei")
	e2.Origins = []memory.Origin{{Source: "import", Key: "b.tmx", SessionID: "s2"}}

	require.NoError(t, tm.Add(context.Background(), e1))
	require.NoError(t, tm.Add(context.Background(), e2))

	entries, total, _ := tm.SearchEntriesFiltered(context.Background(), memory.SearchParams{
...
func TestSQLiteMemory_FacetLocales(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	require.NoError(t, tm.Add(context.Background(), trilingual("e1", "a", "b", "c")))
...
func TestSQLiteMemory_FacetImportSessions(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()

	require.NoError(t, tm.CreateImportSession(context.Background(), memory.ImportSession{ID: "s1", FileKey: "norwegian.tmx", ToolName: "bitextor"}))
...
func TestSQLiteMemory_ImportSessionCRUD(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()

	s := memory.ImportSession{
...
func TestSQLiteMemory_DeleteSessionKeepsOrigins(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
...
func TestSQLiteMemory_EntityMappingRoundtrip(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()

	entry := trilingual("e1", "John works at Acme", "Jean travaille chez Acme", "Johann arbeitet bei Acme")
	entry.Entities = []memory.EntityMapping{
		{
			PlaceholderID: "e1",
			Type:          "entity:person",
			Values: map[model.LocaleID]memory.EntityValue{
...
func TestSQLiteMemory_EntityConceptIDRoundtrip(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
...
func TestSQLiteMemory_SearchArabic(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "البحث", AnyLocale: "ar-SA", RequireLocale: "", Offset: 0, Limit: 10})
...
func TestSQLiteMemory_SearchKorean(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "번역", AnyLocale: "ko-KR", RequireLocale: "", Offset: 0, Limit: 10})
...
// TestSQLiteMemory_SearchLatinRegression ensures Latin-script search still
// works correctly after switching from unicode61 to ICU.
func TestSQLiteMemory_SearchLatinRegression(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	entries, _, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "hello", AnyLocale: "", RequireLocale: "", Offset: 0, Limit: 10})
	require.Len(t, entries, 1)
	assert.Equal(t, "e1", entries[0].ID)

	entries, _, _ = tm.SearchEntries(context.Background(), memory.SearchParams{Query: "revoir", AnyLocale: "", RequireLocale: "", Offset: 0, Limit: 10})
	require.Len(t, entries, 1)
...
func TestSQLiteMemory_LocaleStats(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	require.NoError(t, tm.Add(context.Background(), trilingual("e1", "a", "b", "c")))
