...
// DefaultMaxEntries is the default maximum number of entries in an InMemoryStore.
...
// InMemoryStore is a thread-safe, in-memory implementation of Store with
...
func (tm *InMemoryStore) MaxEntries() int { return tm.maxEntries }

// Close is a no-op for InMemoryStore.
func (tm *InMemoryStore) Close() error { return nil }

// Count returns the total number of entries.
func (tm *InMemoryStore) Count(_ context.Context) (int, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
	return len(tm.entries), nil
}

// Add inserts or updates a content-memory entry.
func (tm *InMemoryStore) Add(ctx context.Context, entry Entry) error {
...
func (tm *InMemoryStore) evictOldest() {
	if len(tm.entries) == 0 {
		return
	}
	oldest := tm.entries[0]
...
func (tm *InMemoryStore) Delete(_ context.Context, id string) error {
	tm.mu.Lock()
	defer tm.mu.Unlock()
	idx, ok := tm.byID[id]
	if !ok {
...
// are skipped. Returns Match results ordered by match priority and score.
func (tm *InMemoryStore) Lookup(_ context.Context, source *model.Block, sourceLocale, targetLocale model.LocaleID, opts LookupOptions) ([]Match, error) {
	if source == nil {
		return nil, nil
	}
...
func (tm *InMemoryStore) LookupSegment(_ context.Context, source *model.Block, segmentIdx int, sourceLocale, targetLocale model.LocaleID, opts LookupOptions) ([]Match, error) {
	if source == nil {
		return nil, nil
	}
...
func (tm *InMemoryStore) LookupText(_ context.Context, source string, sourceLocale, targetLocale model.LocaleID, opts LookupOptions) ([]Match, error) {
	opts = ApplyDefaults(opts)
...
func (tm *InMemoryStore) GetEntry(_ context.Context, id string) (Entry, bool, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
	idx, ok := tm.byID[id]
	if !ok {
...
func (tm *InMemoryStore) Entries(_ context.Context) ([]Entry, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
	out := make([]Entry, len(tm.entries))
...
func (tm *InMemoryStore) SearchEntries(ctx context.Context, params SearchParams) ([]Entry, int, error) {
	return tm.searchInternal(ctx, params)
}

// SearchEntriesFiltered applies additional facet filters.
func (tm *InMemoryStore) SearchEntriesFiltered(ctx context.Context, params SearchParams) ([]Entry, int, error) {
	return tm.searchInternal(ctx, params)
}

// SearchEntriesForStream performs a search with stream priority ordering.
func (tm *InMemoryStore) SearchEntriesForStream(ctx context.Context, params SearchParams) ([]Entry, int, error) {
	return tm.searchInternal(ctx, params)
}
...
func variantTextContains(e Entry, anyLocale, needle string) bool {
	if anyLocale != "" {
...
func matchesSearchFilter(entry Entry, filter SearchFilter) bool {
...
func entryHasCodes(entry Entry) bool {
...
func (tm *InMemoryStore) FacetStats(ctx context.Context) (FacetData, error) {
...
func (tm *InMemoryStore) LocaleStats(_ context.Context) ([]LocaleFacet, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
...
func (tm *InMemoryStore) ActivityStats(_ context.Context) ([]ActivityStat, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
	counts := make(map[string]int)
	for _, se := range tm.entries {
...
func (tm *InMemoryStore) CreateImportSession(_ context.Context, session ImportSession) error {
	if session.ID == "" {
...
func (tm *InMemoryStore) GetImportSession(_ context.Context, id string) (ImportSession, bool, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
	s, ok := tm.sessions[id]
	return s, ok, nil
...
func (tm *InMemoryStore) FindImportSessionByHash(_ context.Context, hash string) (ImportSession, bool, error) {
	if hash == "" {
...
func (tm *InMemoryStore) ListImportSessions(_ context.Context) ([]ImportSession, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
...
func (tm *InMemoryStore) UpdateImportSessionCount(_ context.Context, id string, count int) error {
	tm.mu.Lock()
	defer tm.mu.Unlock()
	s, ok := tm.sessions[id]
	if !ok {
...
func (tm *InMemoryStore) DeleteImportSession(_ context.Context, id string) error {
	tm.mu.Lock()
	defer tm.mu.Unlock()
...
func LimitResults(matches []Match, max int) []Match {
	if len(matches) > max {
		return matches[:max]
	}
	return matches
