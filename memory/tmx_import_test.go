...
func importString(t *testing.T, tm memory.Store, body string, opts memory.ImportTMXOptions) (string, int) {
	t.Helper()
	sid, n, err := memory.ImportTMXSession(context.Background(), tm, strings.NewReader(body), opts)
...
<tmx version="1.4">
  ...
  <body>
	...
	sid, n := importString(t, tm, body, memory.ImportTMXOptions{OriginKey: "f.tmx"})
...
<tmx version="1.4">
  ...
  <body>
	...
	_, n := importString(t, tm, body, memory.ImportTMXOptions{OriginKey: "f.tmx"})
	assert.Equal(t, 1, n)
...
<tmx version="1.4">
  ...
  <body>
	...
	_, n, err := memory.ImportTMXSession(context.Background(), tm, bytes.NewReader(utf16Body), memory.ImportTMXOptions{OriginKey: "f16.tmx"})
	require.NoError(t, err)
	assert.Equal(t, 1, n)
...
<tmx version="1.4">
  ...
  <body>
	...
	_, n := importString(t, tm, body, memory.ImportTMXOptions{OriginKey: "eurlex.tmx"})
	assert.Equal(t, 1, n)
	entries := mustEntries(t, tm)
	require.Len(t, entries, 1)
...
<tu><tuv xml:lang="en"><seg>Click <ph x="1">{0}</ph> to continue</seg></tuv><tuv xml:lang="fr"><seg>Cliquer <ph x="1">{0}</ph> pour continuer</seg></tuv></tu>
	...
	_, n := importString(t, tm, body, memory.ImportTMXOptions{})
	assert.Equal(t, 1, n)
...
<tmx version="1.4">
  ...
  <body>
	...
	sid, _ := importString(t, tm, body, memory.ImportTMXOptions{OriginKey: "bitextor.tmx"})
...
<tmx version="1.4">
  ...
  <body>
	...
	_, _ = importString(t, tm, body, memory.ImportTMXOptions{OriginKey: "bitextor.tmx"})
	entries := mustEntries(t, tm)
	require.Len(t, entries, 1)
	require.Len(t, entries[0].Origins, 1)
...
<body><tu><tuv xml:lang="en"><seg>a</seg></tuv><tuv xml:lang="fr"><seg>b</seg></tuv></tu></body></tmx>`
	...
	_, _ = importString(t, tm, body, memory.ImportTMXOptions{OriginKey: "f.tmx"})

	var warned string
	warnFn := func(msg string) { warned = msg }
	_, n := importString(t, tm, body, memory.ImportTMXOptions{OriginKey: "f.tmx", WarnFunc: warnFn})
...
<tu><tuv xml:lang="en"><seg>a</seg></tuv><tuv xml:lang="fr"><seg>b</seg></tuv><tuv xml:lang="de"><seg>c</seg></tuv></tu>
	...
	_, _ = importString(t, tm, body, memory.ImportTMXOptions{Locales: []model.LocaleID{"en", "fr"}})
	e := mustEntries(t, tm)[0]
...
func TestImportTMX_CustomMappingFile(t *testing.T) {
	...
	m, err := memory.DefaultTMXMapping()
	require.NoError(t, err)
...
<body><tu><tuv xml:lang="en"><seg>hello</seg></tuv><tuv xml:lang="fr"><seg>bonjour</seg></tuv></tu></body></tmx>`)
	n, err := memory.ImportTMXWithOptions(context.Background(), tm, body, "en", "fr", memory.ImportTMXOptions{OriginKey: "legacy.tmx"})
	require.NoError(t, err)
	assert.Equal(t, 1, n)
	e := mustEntries(t, tm)[0]
