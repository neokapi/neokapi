...
// TestSQLiteMemory_SearchCJK_Unicode61 documents that unicode61 treats a
// space-less CJK string as one token: a sub-word query does not match, but a
// query for the whole token does.
func TestSQLiteMemory_SearchCJK_Unicode61(t *testing.T) {
	...
	_, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "经济", AnyLocale: "zh-CN", RequireLocale: "", Offset: 0, Limit: 10})
	...
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "中国经济发展报告", AnyLocale: "zh-CN", RequireLocale: "", Offset: 0, Limit: 10})
...
func TestSQLiteMemory_SearchThai_Unicode61(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	_, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "ภาษา", AnyLocale: "th-TH", RequireLocale: "", Offset: 0, Limit: 10})
	assert.Equal(t, 0, total, "unicode61 does not segment Thai sub-words")

	// Whole-string search matches.
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "การทดสอบภาษาไทยในระบบค้นหา", AnyLocale: "th-TH", RequireLocale: "", Offset: 0, Limit: 10})
...
// TestSQLiteMemory_SearchSpaceSeparated_Unicode61 confirms that word-search still
// works for scripts with explicit word boundaries under unicode61.
func TestSQLiteMemory_SearchSpaceSeparated_Unicode61(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "テスト", AnyLocale: "ja-JP", RequireLocale: "", Offset: 0, Limit: 10})
