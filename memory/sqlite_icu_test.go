...
// TestSQLiteMemory_SearchCJK verifies that the ICU tokenizer correctly segments
...
func TestSQLiteMemory_SearchCJK(t *testing.T) {
	...
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "经济", AnyLocale: "zh-CN", RequireLocale: "", Offset: 0, Limit: 10})
	...
	entries, total, _ = tm.SearchEntries(context.Background(), memory.SearchParams{Query: "贸易", AnyLocale: "zh-CN", RequireLocale: "", Offset: 0, Limit: 10})
...
func TestSQLiteMemory_SearchJapanese(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
...
func TestSQLiteMemory_SearchThai(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	defer tm.Close()
	...
	entries, total, _ := tm.SearchEntries(context.Background(), memory.SearchParams{Query: "ภาษา", AnyLocale: "th-TH", RequireLocale: "", Offset: 0, Limit: 10})
