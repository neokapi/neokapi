...
// TestSQLiteMemory_ConcurrentLookup exercises concurrent Lookup, LookupText, and
// Count calls from 16 goroutines to verify the SQLite content memory is safe under
// parallel read access. Run with -race to catch data races.
func TestSQLiteMemory_ConcurrentLookup(t *testing.T) {
	...
	require.NoError(t, g.Wait(), "concurrent content memory operations must not return errors")
...
// TestSQLiteMemory_ConcurrentFacetStats exercises concurrent FacetStatsFiltered
// calls alongside reads.
func TestSQLiteMemory_ConcurrentFacetStats(t *testing.T) {
	tm, err := memory.NewSQLiteStore(":memory:")
	require.NoError(t, err)
	t.Cleanup(func() { tm.Close() })
	...
	for i := range goroutines {
		...
		g.Go(func() error {
			for range 3 {
				stats, err := tm.FacetStatsFiltered(context.Background(), memory.SearchParams{AnyLocale: "en", RequireLocale: "fr"})
				if err != nil {
