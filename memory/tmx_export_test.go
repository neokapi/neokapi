...
func TestExportTMX_LocalesFilter(t *testing.T) {
	...
	require.NoError(t, tm.Add(context.Background(), trilingual("e1", "Hello", "Bonjour", "Hallo")))
	var buf bytes.Buffer
	require.NoError(t, memory.ExportTMX(context.Background(), tm, &buf, []model.LocaleID{"en", "fr"}))
	out := buf.String()
	assert.Contains(t, out, `xml:lang="en"`)
	assert.Contains(t, out, `xml:lang="fr"`)
...
<body><tu tuid="x1"><tuv xml:lang="en"><seg>Hello</seg></tuv><tuv xml:lang="fr"><seg>Bonjour</seg></tuv></tu></body></tmx>`
	...
	_, n, err := memory.ImportTMXSession(context.Background(), tm1, strings.NewReader(body), memory.ImportTMXOptions{})
	require.NoError(t, err)
	require.Equal(t, 1, n)

	var buf bytes.Buffer
	require.NoError(t, memory.ExportTMX(context.Background(), tm1, &buf, nil))

	tm2 := memory.NewInMemoryStore()
	_, _, err = memory.ImportTMXSession(context.Background(), tm2, &buf, memory.ImportTMXOptions{})
	require.NoError(t, err)
...
<body><tu><tuv xml:lang="en"><seg>A</seg></tuv><tuv xml:lang="fr"><seg>B</seg></tuv><tuv xml:lang="de"><seg>C</seg></tuv><tuv xml:lang="es"><seg>D</seg></tuv></tu></body></tmx>`
	...
	tm1 := memory.NewInMemoryStore()
	_, _, err := memory.ImportTMXSession(context.Background(), tm1, strings.NewReader(body), memory.ImportTMXOptions{})
	require.NoError(t, err)
