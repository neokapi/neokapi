...
func samplePackage() *Package {
	return &Package{
		...
		TM:          kmb.FromModel(sampleTM(), nil),
		Termbase:    ktb.FromConcepts(sampleTermbase()),
...
// termbase stores, pack them into a .kpz, unpack into FRESH stores, and prove
...
func TestCacheInternalRoundTrip(t *testing.T) {
	...
	require.NoError(t, err)
	pkg := &Package{
		TM:       kmb.FromModel(tm1Entries, tm1Sessions),
		Termbase: ktb.FromConcepts(tb1Concepts),
	}
	data, err := pkg.Marshal()
	require.NoError(t, err)
	...
	repacked, err := (&Package{
		TM:       kmb.FromModel(tm2Entries, tm2Sessions),
		Termbase: ktb.FromConcepts(tb2Concepts),
	}).Marshal()
	require.NoError(t, err)
