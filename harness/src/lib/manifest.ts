...
export function loadManifest(id: string): DemoManifest {
  ...
  if (m.locales) {
    throw new Error(
      `demo ${id}: inline "locales:" blocks are no longer supported — localized narration lives in generated demo.<lang>.yaml sidecars (make l10n-demos). Fold the translations into context/memory/demo-narration-<lang>.memory.json and delete the block.`,
    );
  }
