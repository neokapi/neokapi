...
lint: check-abs-paths check-em-dashes check-docs-palette check-eval-publishable check-local-actions check-deploy-paths check-vocabulary check-desktop-interchange check-vocab-packs check-comment-history check-reference-provenance check-run-projection check-comment-coverage check-walk-selectors check-locale-display check-sidebar-ids check-package-licenses check-archive-licenses check-plugin-licenses check-plugin-release-latest check-tracked-binaries check-extract-fixtures check-gofmt ## Run golangci-lint (all modules) + repo hygiene guards
...
# rename is silent until a re-record: one sweep found nine of twelve walks
# broken, four of them on a single word.
check-walk-selectors: ## Guard: every selector a recorded walk drives is still rendered by an app
	@./scripts/check-walk-selectors.sh
...
# needs its own module directory here because `./...` at the root stops at a
# nested go.mod.
...
sourcecode-comment-goldens: ## Regenerate the goldens the kapi-sourcecode comment tests compare with (needs node, python3, ruby, cargo, a JDK, the .NET SDK and libclang)
...
L10N_DERIVED := \
	...
	host/i18n/catalogs/qps.json \
	scripts/gen-refs/nativedocs/qps \
	packages/reference-data/data/qps \
...
L10N_LOOP_LOCALE = core/i18n/catalogs/$(1).json host/i18n/catalogs/$(1).json \
	...
	scripts/gen-refs/nativedocs/$(1) \
	packages/reference-data/data/$(1)
...
l10n-pseudo: bin/kapi ## Pseudo-translate every surface into the qps probe locale
	...
	$(KAPI_ISO_ENV) ./bin/kapi pseudo-translate host/i18n/commands.json --target-lang qps -f json -o host/i18n/catalogs/qps.json -q
	@# The reference dossiers: the prose behind each tool's and format's card
	@# and page. Probed file by file under the recipe's yaml reader config,
	@# into the <lang>/<kind>/<id>.yaml layout the loop writes its targets to,
	@# and cleared first for the same reason the i18n-qps trees are.
	rm -rf scripts/gen-refs/nativedocs/qps
	@for f in scripts/gen-refs/nativedocs/*/*.yaml; do \
		rel=$${f#scripts/gen-refs/nativedocs/}; \
		mkdir -p scripts/gen-refs/nativedocs/qps/$$(dirname $$rel); \
		$(KAPI_ISO_ENV) ./bin/kapi pseudo-translate $$f --target-lang qps -p $(CURDIR)/kapi.yaml -f yaml -o scripts/gen-refs/nativedocs/qps/$$rel -q || exit 1; \
	done
...
l10n-compile: i18n-react-build ## Stage 3: catalogs → runtime dictionaries, embedded MO, rendered email templates
	...
	@# The reference dataset in each locale: the docs site's reference pages
	@# read packages/reference-data/data/<lang>/, derived here from the
...
L10N_REPORT_PAIRS = \
	...
	host/i18n/catalogs/$(1).json:host/i18n/catalogs/qps.json \
