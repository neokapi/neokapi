...
export interface KbfConformanceProps {
...
interface ConfCase {
  ...
  expected?: string;
  /** Run against the canonical Go engine (via the wasm `kbf` endpoint). */
...
// KbfConformance executes the KBF spec conformance suite in the browser against
// BOTH implementations: the canonical Go engine (core/kbf, compiled to wasm)
...
export default function KbfConformance({ assets }: KbfConformanceProps): React.ReactElement {
  ...
  return (
    <div className={`kapi-reference relative ${styles.lab}`}>
      ...
      <GateOverlay
        gate={gate}
        title="KBF conformance"
        description="Run the KBF conformance suite in your browser."
      />
    </div>
  );
}
...
const fullFile: File = kbfSampleById("full").file;
...
function badEnvelope(mutate: (f: File) => void): string {
  const clone = JSON.parse(kbfText(kbfSampleById("files-heading").file)) as File;
  mutate(clone);
  return kbfText(clone);
}
