#!/usr/bin/env node
// Build @neokapi/engine for publishing: transpile each source file in place
// (no bundling, so the subpath exports — ./runtime, ./memfs, … — resolve to
// their own dist files) and emit declarations with tsc. Mirrors
// packages/kapi-format/build.mjs; in-repo consumers use the TS source directly
// (see the package.json exports vs publishConfig split).
