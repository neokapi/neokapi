...
import { validateAnchor, type Annotation, type AnnotationAnchor } from "../src/annotation.ts";
...
console.log(
  `validator/valid: ${errorsA.length === 0 ? "\u2713" : "\u2717"} (${errorsA.length} errors)`,
);
...
console.log(
  `validator/missing: ${errorsB.length === 1 ? "\u2713" : "\u2717"} (${errorsB.length} errors)`,
);
if (errorsB.length === 1) {
...
console.log(
  `validator/optional-drop: ${errorsC.length === 0 ? "\u2713" : "\u2717"} (${errorsC.length} errors)`,
);
...
console.log(
  `validator/plural-preserves-pivot: ${errorsD.length === 0 ? "\u2713" : "\u2717"} (${errorsD.length} errors)`,
);
