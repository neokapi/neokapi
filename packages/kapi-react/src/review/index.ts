/**
 * @neokapi/kapi-react/review — in-context review overlay (browser).
 *
 ...
 *     annotations. Saving writes back into the local `.klf` file
...
import { refreshTranslations, setTranslations } from "../runtime/index.ts";
...
async function openPanel(endpoint: string, hash: string, el: Element): Promise<void> {
  ...
  try {
    ...
    showToast("kapi review: block not found in the KLF tree — run extract first");
    return;
  }
  ...
  panel.innerHTML = `
    ...
    <div class="kapi-row">
      <button class="kapi-primary" id="kapi-review-save">Save to .klf</button>
  ...
  panel.querySelector("#kapi-review-save")?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`${res.status}`);
      // Repaint in place without waiting for the SSE echo.
...
function installToolbar(endpoint: string): void {
  ...
  bar.innerHTML = `<span>⌥ kapi review</span><button id="kapi-review-hl" title="Highlight terms & QA">terms/QA</button>`;
