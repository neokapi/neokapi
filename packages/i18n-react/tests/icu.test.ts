...
describe("ICU resolver", () => {
  describe("plural", () => {
    ...
    it("handles tokens within branches", () => {
      const text =
        "{count, plural, one {{count} unread message from {name}} other {{count} unread messages from {name}}}";
      expect(resolveICU(text, { count: 3, name: "Alice" }, "en")).toBe(
        "3 unread messages from Alice",
      );
    });
  });
  ...
  describe("select", () => {
    ...
    it("handles tokens within branches", () => {
      const text =
        "{gender, select, male {{name} updated his profile} female {{name} updated her profile} other {{name} updated their profile}}";
      expect(resolveICU(text, { gender: "female", name: "Alice" }, "en")).toBe(
        "Alice updated her profile",
      );
    });
  });
