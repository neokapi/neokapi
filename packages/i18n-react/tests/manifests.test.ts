...
describe("collectImports", () => {
  it("groups non-relative imports by source", () => {
    ...
    expect(Array.from(imports.keys())).toEqual(["@radix-ui/react-tabs", "@/components/ui/button"]);
...
describe("resolveLibraryComponentMap", () => {
  ...
  it("applies the owning package manifest when filename is provided", () => {
    ...
    `);
    const uiFile = new URL("../../ui/src/components/ui/dialog.tsx", import.meta.url).pathname;
    const map = resolveLibraryComponentMap(mod, process.cwd(), undefined, uiFile);
