...
function tempProject() {
  const dir = mkdtempSync(join(tmpdir(), "i18n-react-extract-"));
...
describe("neokapi-i18n extract", () => {
  ...
  it("emits NDJSON block records that match the KBF-written set", async () => {
    const dir = tempProject();
    const cwd = process.cwd();
    process.chdir(dir);
    try {
      ...
      expect(streamed).toHaveLength(kbfBlocks.length);
      expect(new Set(streamed.map((r) => r.block.hash))).toEqual(
        new Set(kbfBlocks.map((b) => b.hash)),
      );
