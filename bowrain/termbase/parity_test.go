...
// the framework SQLite termbase and the bowrain Postgres termbase. Both stores
// delegate ranking to termbase.LookupTiered / termbase.LookupAllTiered, so any
// divergence here is a real drift regression.
//
// This suite historically FAILED before the dialect-seam refactor: the Postgres
// termbase ignored term validity (valid_from/valid_to) in its fuzzy, exact and
...
// no ProjectID: the Postgres termbase is workspace-scoped and has no project_id
...
func projectTB(matches []termbase.TermMatch) []tbRow {
...
// openParitySQLiteTB opens a throwaway in-memory SQLite termbase.
func openParitySQLiteTB(t *testing.T) termbase.TermBase {
	t.Helper()
...
// maybePostgresTB opens a Postgres termbase if one is reachable, returning (nil,
// false) otherwise — unlike openTestPostgresTermBase it does NOT skip the test,
...
func maybePostgresTB(t *testing.T) (termbase.TermBase, bool) {
	t.Helper()
...
func enTerm(id, text string, validity *graph.Validity) termbase.Concept {
...
func paritySeed(t *testing.T, tb termbase.TermBase) {
	t.Helper()
...
func runParity(t *testing.T, sq, pg termbase.TermBase, name string, lookup func(termbase.TermBase) ([]termbase.TermMatch, error)) {
	t.Helper()
...
func TestTBParity_SQLiteVsPostgres(t *testing.T) {
	...
	cases := []struct {
		name   string
		lookup func(termbase.TermBase) ([]termbase.TermMatch, error)
	}{
		{
			...
			lookup: func(tb termbase.TermBase) ([]termbase.TermMatch, error) {
				return tb.Lookup(ctx, "Install", termbase.LookupOptions{SourceLocale: "en", Scope: &now})
			},
		},
		{
			...
			name: "exact-no-scope",
			lookup: func(tb termbase.TermBase) ([]termbase.TermMatch, error) {
				return tb.Lookup(ctx, "Install", termbase.LookupOptions{SourceLocale: "en"})
			},
		},
		{
			// Fuzzy "installations" under a now-scope: the expired and not-yet
			// copies are filtered — the specific drift the TM pass flagged as
			// deferred (Postgres fuzzy ignored validity entirely).
			name: "fuzzy-validity-now",
			lookup: func(tb termbase.TermBase) ([]termbase.TermMatch, error) {
				return tb.Lookup(ctx, "installations", termbase.LookupOptions{SourceLocale: "en", MinScore: 0.5, MatchModes: fuzzy, Scope: &now})
			},
		},
		{
			...
			name: "fuzzy-no-scope",
			lookup: func(tb termbase.TermBase) ([]termbase.TermMatch, error) {
				return tb.Lookup(ctx, "installations", termbase.LookupOptions{SourceLocale: "en", MinScore: 0.5, MatchModes: fuzzy})
			},
		},
		{
			...
			lookup: func(tb termbase.TermBase) ([]termbase.TermMatch, error) {
				return tb.LookupAll(ctx, "Please Save now", termbase.LookupOptions{SourceLocale: "en"})
			},
		},
		{
			...
			lookup: func(tb termbase.TermBase) ([]termbase.TermMatch, error) {
				return tb.LookupAll(ctx, "Install the installation", termbase.LookupOptions{SourceLocale: "en", Scope: &now})
			},
		},
	}
...
func conceptIDs(matches []termbase.TermMatch) []string {
...
// TestTBParity_ExpectedSets pins the concrete result sets the parity fixtures
...
// Postgres IS present, TestTBParity_SQLiteVsPostgres additionally asserts the
// Postgres backend produces byte-for-byte the same rows.
func TestTBParity_ExpectedSets(t *testing.T) {
	sq := openParitySQLiteTB(t)
	pg, hasPG := maybePostgresTB(t)
	paritySeed(t, sq)
	if hasPG {
