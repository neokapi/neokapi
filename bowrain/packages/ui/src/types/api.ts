...
import type { IOPort, Run, Anchor, ReviewContext as ReviewModel } from "@neokapi/contract-types";
...
export interface ReviewContext extends ReviewModel {
  ...
  item_name: string;
  locale: string;
  /** The collection the block's item belongs to, "" for an item in none. The point names it. */
  collection_id: string;
  /** The terms the block's source matches, positioned over it. */
  terms: BlockTermMatch[];
  /** The block's notes, oldest first. */
  notes: BlockNote[];
  /**
   * The block's latest voice score, absent for a block nothing has scored, and
   * the bar the profile in force sets, present whenever a profile is bound.
   * The findings behind the score are the judgement's findings.
   */
  voice_score?: number;
  voice_bar?: number;
}
