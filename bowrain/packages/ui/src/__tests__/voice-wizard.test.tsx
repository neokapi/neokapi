...
describe("VoiceProfileWizard", () => {
  it("renders step 1 (Identity) by default with name and description inputs", () => {
    render(<VoiceProfileWizard {...defaultProps} />);
    expect(screen.getByText("Profile Identity")).toBeInTheDocument();
    expect(screen.getByLabelText("Name")).toBeInTheDocument();
    expect(screen.getByLabelText("Description")).toBeInTheDocument();
  });

  it("shows New Voice Profile title when creating", () => {
    render(<VoiceProfileWizard {...defaultProps} />);
    expect(screen.getByText("New Voice Profile")).toBeInTheDocument();
  });

  it("shows Edit Profile title when editing an existing profile", () => {
    render(<VoiceProfileWizard {...defaultProps} profile={makeProfile()} />);
  ...
  it("pre-populates form when profile prop is provided", () => {
    ...
    render(<VoiceProfileWizard {...defaultProps} profile={profile} />);
  ...
  it("Cancel button calls onCancel", async () => {
    ...
    render(<VoiceProfileWizard onSave={vi.fn()} onCancel={onCancel} />);
