// The on-brand bar and the score bands derived from it — the client mirror of
...
/** Carries a profile's own on-brand bar; anything with a `min_score` field. */
export interface HasMinScore {
  min_score?: number;
}

/**
 * The effective on-brand bar for a profile: its `min_score` when set (capped at
