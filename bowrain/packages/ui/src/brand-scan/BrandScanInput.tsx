...
import type { BrandScanRequest, SkippedFile } from "../types/api";
...
export interface BrandScanInputProps {
  /** Called once the scan job is enqueued. */
  onStarted: (jobId: string, request: BrandScanRequest) => void;
  /** Prefill, e.g. when regenerating a scan with the same request. */
  initialRequest?: BrandScanRequest;
}
 ...
 */
export function BrandScanInput({ onStarted, initialRequest }: BrandScanInputProps) {
  const api = useApi();
    ...
    try {
      ...
      if (files.length > 0) {
        if (pendingKeys) {
          ...
          setSkipped([]);
          setPhase("uploading");
          const result = await api.uploadBrandScanSources(ws, files);
      ...
      const request: BrandScanRequest = {};
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-5 space-y-3">
        ...
        <Textarea
          data-testid={TEST_IDS.brandScan.input}
          value={pasteText}
      ...
      <Card className="p-5 space-y-3">
        ...
        <div className="flex gap-2">
          <Input
            data-testid={TEST_IDS.brandScan.urlInput}
            value={urlDraft}
      ...
      <Card className="p-5 space-y-3">
        ...
        <div
          data-testid={TEST_IDS.brandScan.fileDrop}
          role="button"
          tabIndex={0}
      ...
      <Card className="p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label htmlFor="brand-scan-profile-name">Profile name</Label>
            <Input
              id="brand-scan-profile-name"
              value={profileName}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="brand-scan-domain">Subject domain</Label>
            <Input
              id="brand-scan-domain"
              value={domain}
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <Button
          data-testid={TEST_IDS.brandScan.start}
          onClick={handleStart}
