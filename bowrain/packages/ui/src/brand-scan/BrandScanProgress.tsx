...
import type { BrandScanJob } from "../types/api";
import { TEST_IDS } from "../test-ids";
import { AlertTriangle, Check, Loader2 } from "../components/icons";

export interface BrandScanProgressProps {
  job: BrandScanJob;
 ...
 */
export function BrandScanProgress({ job, onRetry }: BrandScanProgressProps) {
  if (job.status === "failed") {
    return (
      <Card data-testid={TEST_IDS.brandScan.progress} className="p-6 space-y-4">
  ...
  return (
    <Card data-testid={TEST_IDS.brandScan.progress} className="p-6 space-y-5">
