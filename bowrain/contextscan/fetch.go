// Package contextscan ingests brand source material (pasted text, public web
...
func (f *fetcher) fetch(ctx context.Context, rawURL string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("contextscan: build request: %w", err)
	}
	req.Header.Set("User-Agent", "neokapi-contextscan/1.0")
	req.Header.Set("Accept", "text/html,application/xhtml+xml")

	resp, err := client.Do(req)
	if err != nil {
		return "", fmt.Errorf("contextscan: fetch %s: %w", u.Host, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("contextscan: fetch %s: unexpected status %s", u.Host, resp.Status)
	}
	contentType := resp.Header.Get("Content-Type")
	mediaType, _, mtErr := mime.ParseMediaType(contentType)
	if mtErr != nil || !isHTMLMediaType(mediaType) {
		return "", fmt.Errorf("contextscan: fetch %s: content type %q is not HTML", u.Host, contentType)
	}
	...
	if err != nil {
		return "", fmt.Errorf("contextscan: extract text from %s: %w", u.Host, err)
	}
	return text, nil
}
