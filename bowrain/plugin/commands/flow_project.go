...
// listProjectFlows returns flow info entries from the recipe's `flows_dir:`,
// which the plugin registers as the CLI's extra-flows source so `kapi flows`
// lists a project's own flows beside the built-in catalog.
//
// Running one of them is the project runner's job: `kapi run <flow>` resolves
// a flow file there the same way it resolves an inline `flows:` entry
