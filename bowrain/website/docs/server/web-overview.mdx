...
The neokapi web application is a browser-based translation workbench that provides the same powerful editing experience as the [Bowrain desktop app](/desktop/overview), accessible from any modern browser. It runs as part of `bowrain-server` for multi-user SaaS deployments, or as part of `bowrain serve` for local single-user workflows.
