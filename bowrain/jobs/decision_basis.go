...
type decisionLedger map[decisionUnitKey]venue.UnitDecision
...
func loadDecisionLedger(ctx context.Context, cs store.ContentStore, projectID, stream string) decisionLedger {
	...
	for _, d := range records {
		index[decisionUnitKey{item: d.ItemName, unit: d.Unit, variant: d.Variant}] = d
	}
	return index
}

// record returns the ledger's record for a block's variant.
func (l decisionLedger) record(sb *venue.StoredBlock, key model.VariantKey) (venue.UnitDecision, bool) {
...
// wording the block no longer holds.
//
// This is the predicate the recycle pass partitions on and the estimate prices
// from, so a quote and the run it precedes describe the same set of units.
...
// loop.
//
// The records carry a basis and nothing else: no rung, no review state, no
// decider. The store projects a status only for a record that claims one, so a
// basis leaves the target on the rung its producer put it on.
//
...
func recordProducedBasis(
	...
	for _, b := range blocks {
		...
		prev, had := ledger[key]
		if had && prev.ReviewState != "" {
			continue // decided: the basis is the decision's
		}
		basis := blockSourceHash(sb)
		target := state.TargetHash(text)
		if had && prev.ContentHash == basis && prev.TargetHash == target {
