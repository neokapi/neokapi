...
type fakeUploadSweepStore struct {
	candidates []ContextScanUploadCleanup
	cleared    []string
}

func (f *fakeUploadSweepStore) SweepExpiredContextScanUploads(context.Context, time.Duration, int) ([]ContextScanUploadCleanup, error) {
	return f.candidates, nil
}

func (f *fakeUploadSweepStore) ClearContextScanUploadKeys(_ context.Context, id string) error {
...
// TestContextScanUploadSweeper_DeletesAndClears: the happy path — every
// envelope of an expired job is deleted and the job's keys are cleared.
func TestContextScanUploadSweeper_DeletesAndClears(t *testing.T) {
	store := &fakeUploadSweepStore{candidates: []ContextScanUploadCleanup{
		{JobID: "job-1", UploadKeys: []string{"blob-a", "blob-b"}},
	}}
	blobs := newFakeSweepBlobStore("blob-a", "blob-b", "blob-unrelated")

	s := NewContextScanUploadSweeper(store, blobs, 0, 0)
...
// TestContextScanUploadSweeper_TransientFailureRetries: when a delete fails and
// the blob still exists, the job is NOT cleared, so the next sweep retries it
// instead of orphaning the remaining envelope.
func TestContextScanUploadSweeper_TransientFailureRetries(t *testing.T) {
	store := &fakeUploadSweepStore{candidates: []ContextScanUploadCleanup{
		{JobID: "job-1", UploadKeys: []string{"blob-a", "blob-b"}},
...
// TestContextScanUploadSweeper_AbsentBlobCountsAsGone: a key whose blob no
...
func TestContextScanUploadSweeper_AbsentBlobCountsAsGone(t *testing.T) {
	store := &fakeUploadSweepStore{candidates: []ContextScanUploadCleanup{
