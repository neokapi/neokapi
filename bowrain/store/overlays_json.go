...
// the same mechanism the annotations column uses (see overlay_sync.go and
// bowrain/core/sync.marshalAnnotations), rehydrated through the model payload
// registry (model.NewPayload) so every registered overlay kind round-trips
// faithfully and an unknown kind degrades to a GenericAnnotation rather than
// being silently dropped.

// emptyOverlaysJSON is the byte-stable encoding of a block with no overlays,
// matching the `overlays` column default so a nil/empty round-trip is a no-op.
...
// the same discriminated shape the annotation store uses.
...
// column. Nil/empty overlays encode to the byte-stable "[]" default so an
// unset round-trip is a no-op.
