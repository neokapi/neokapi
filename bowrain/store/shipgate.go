...
func (s *PostgresStore) ShipGateRollup(ctx context.Context, q platstore.ShipGateQuery) (platstore.ShipGateRollup, error) {
	...
	// An empty list rather than NULL, so ANY($8) reads as no locale.
	voiceGoverned := append([]string{}, q.VoiceGoverned...)
	...
	const rollupSQL = `
		WITH scored AS (
			...
			SELECT t.block_id, t.locale, v.fails, v.terms, COALESCE(i.collection_id, '') AS collection_id
			FROM translations t
		...
		SELECT h.collection_id, h.locale,
			...
			count(*) FILTER (WHERE NOT h.fails AND h.terms = $9),
			count(*) FILTER (WHERE NOT h.fails AND NOT COALESCE(s.below_bar, false)
				AND (h.terms = $9 OR (h.locale = ANY($8) AND s.block_id IS NULL)))
		FROM held h
	...
	rows, err := s.db.QueryContext(ctx, rollupSQL,
		...
		voiceGoverned, string(platstore.TermComplianceUnchecked))
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&cid, &loc, &c.Failing, &c.Clean, &c.Scored, &c.CleanBelowBar, &c.TermsNotChecked, &c.NotChecked); err != nil {
...
func (s *PostgresStore) PutShipGateVerdicts(ctx context.Context, projectID, stream, gate string, verdicts []platstore.ShipGateVerdict) error {
	...
	termVerdicts := make([]string, 0, len(verdicts))
	for _, v := range verdicts {
		...
		termVerdicts = append(termVerdicts, string(v.Terms))
	}

	const upsertSQL = `
		INSERT INTO ship_verdicts (project_id, stream, block_id, locale, gate, basis, fails, terms, updated_at)
		SELECT $1, $2, p.block_id, p.locale, $3, p.basis, p.fails, p.terms, NOW()
		FROM unnest($4::text[], $5::text[], $6::text[], $7::bool[], $8::text[]) AS p(block_id, locale, basis, fails, terms)
		...
		SET gate = EXCLUDED.gate, basis = EXCLUDED.basis,
			fails = EXCLUDED.fails, terms = EXCLUDED.terms, updated_at = EXCLUDED.updated_at`

	if _, err := s.db.ExecContext(ctx, upsertSQL,
		projectID, stream, gate, blockIDs, locales, bases, fails, termVerdicts); err != nil {
