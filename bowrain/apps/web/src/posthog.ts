...
function startAuthenticatedSession(): boolean {
  ...
  initialized = initPostHogSurface(posthog, {
    ...
    init: {
      ...
      // The identifier stays on this host. PostHog defaults to the registrable
      // domain, which here is `.bowrain.cloud` — shared with the landing and
      // documentation sites, which publish themselves as cookieless and carry no
      // consent banner on the strength of it. Those sites measure themselves
      // through @neokapi/docs-shared under `persistence: "memory"` and read no
      // cookie, so a parent-scoped identifier buys nothing there and only makes
      // their published claim false for a signed-in visitor. Campaign
      // attribution across the hop does not depend on it either: the landing
      // forwards utm_* onto every app link, ungated by any key.
