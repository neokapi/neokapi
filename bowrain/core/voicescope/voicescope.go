// Package voicescope bridges the framework's hierarchical voice resolver
// (core/profile) to the bowrain platform's organizational scopes. It reads the
// voice binding that a project, stream, or collection carries in its
...
//
// See core/profile/resolve.go for the ladder itself; this package only populates
...
// Scope identifies the organizational-hierarchy positions a voice check runs
...
// ScopeStore is the subset of store.ContentStore voicescope needs to read the
// project, stream, and collection carrying a voice binding.
...
// WorkspaceDefault resolves the workspace-level default voice profile ID.
...
type WorkspaceDefault interface {
	WorkspaceVoiceProfileID(ctx context.Context, workspaceID string) (string, error)
}
...
func populateContext(ctx context.Context, cs ScopeStore, wd WorkspaceDefault, rc *coreprofile.ResolveContext, sc Scope) {
	...
	if wd != nil && workspaceID != "" {
		if id, err := wd.WorkspaceVoiceProfileID(ctx, workspaceID); err == nil {
			rc.RootProfileID = id
		}
	}
}
