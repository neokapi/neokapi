...
The most common way to run server-side flows is through automation rules. See [Automation](/bowrain/server/automation) for details on creating rules.
...
In the Bowrain web UI, navigate to **Project > Flows** to:

1. Browse available flows (built-in and custom)
2. Start a flow execution with custom parameters
3. View running and completed executions
4. Inspect execution logs and results
...
curl -X POST https://bowrain.example.com/api/v1/projects/:id/flows \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    ...
    "steps": [
      ...
      {"tool": "qa-check", "config": {"rules": ["whitespace", "placeholders"]}}
    ]
  }'
```
...
This is managed automatically by the automation system. Loop prevention ensures chains cannot exceed 5 levels deep (see [Automation > Loop Prevention](/bowrain/server/automation#loop-prevention)).

## Related

- [Automation](/bowrain/server/automation) — event-driven automation rules
- [Translation Flows (CLI)](/bowrain/cli/flows/overview) — local flow execution
- [bowrain sync](/bowrain/cli/commands/sync) — push + wait + pull in one command
