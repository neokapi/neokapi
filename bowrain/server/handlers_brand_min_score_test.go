...
// brandProfileWrite posts payload to handler as a full-permission caller in
// wsID, optionally with an :id route param, and returns the recorder.
func brandProfileWrite(t *testing.T, srv *Server, handler echo.HandlerFunc, method, wsID, profileID, payload string) *httptest.ResponseRecorder {
	t.Helper()
...
// TestBrandProfile_MinScoreRoundTripsThroughCreateAndUpdate walks the bar
...
func TestBrandProfile_MinScoreRoundTripsThroughCreateAndUpdate(t *testing.T) {
	srv := setupBrandLoopServer(t)
	ctx := context.Background()
	const wsID = "ws-min-score-rt"

	rec := brandProfileWrite(t, srv, srv.HandleCreateBrandProfile, http.MethodPost, wsID, "",
...
// TestBrandProfile_MinScoreOutOfRangeIsRejected pins the 0–100 band on every
...
// nobody authored.
func TestBrandProfile_MinScoreOutOfRangeIsRejected(t *testing.T) {
	srv := setupBrandLoopServer(t)
	...
	cases := []struct {
		...
		{"create above the band", srv.HandleCreateBrandProfile, http.MethodPost, "", "Above", 101, http.StatusBadRequest},
		{"create below the band", srv.HandleCreateBrandProfile, http.MethodPost, "", "Below", -1, http.StatusBadRequest},
		{"create at the top of the band", srv.HandleCreateBrandProfile, http.MethodPost, "", "Top", 100, http.StatusCreated},
		{"update above the band", srv.HandleUpdateBrandProfile, http.MethodPut, created.ID, "Range Voice", 101, http.StatusBadRequest},
		{"update at the top of the band", srv.HandleUpdateBrandProfile, http.MethodPut, created.ID, "Range Voice", 100, http.StatusOK},
		{"upsert above the band", srv.HandleUpsertBrandProfile, http.MethodPost, "", "Range Voice", 101, http.StatusBadRequest},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			payload := fmt.Sprintf(`{"name":%q,"tone":{"formality":"neutral"},"min_score":%d}`,
				tc.profileName, tc.minScore)
			rec := brandProfileWrite(t, srv, tc.handler, tc.method, wsID, tc.id, payload)
...
// authoredProfileRequest is a BrandProfileRequest with EVERY field set to a
...
func authoredProfileRequest(name, mark string, minScore int) BrandProfileRequest {
	return BrandProfileRequest{
		Name:        name,
...
func assertProfileCarriesRequest(t *testing.T, req BrandProfileRequest, stored *coreprofile.VoiceProfile) {
	t.Helper()
...
// TestBrandProfile_EveryAuthoredFieldSurvivesTheWrite closes the whole class
...
// value with nothing failing anywhere. Every field of BrandProfileRequest is
...
func TestBrandProfile_EveryAuthoredFieldSurvivesTheWrite(t *testing.T) {
	srv := setupBrandLoopServer(t)
	ctx := context.Background()
	...
	require.NoError(t, err)
	rec := brandProfileWrite(t, srv, srv.HandleCreateBrandProfile, http.MethodPost, wsID, "", string(payload))
	require.Equal(t, http.StatusCreated, rec.Code, rec.Body.String())
...
// TestBrandUpsert_MinScoreIsAuthoredContent proves the push upsert treats the
...
// not the "unchanged" no-op it would be if brandContentOf ignored the field.
func TestBrandUpsert_MinScoreIsAuthoredContent(t *testing.T) {
	srv := setupBrandLoopServer(t)
	const wsID = "ws-min-score-upsert"

	rec, res := upsertBrandProfile(t, srv, wsID, "u-1",
	...
	_, same := upsertBrandProfile(t, srv, wsID, "u-1",
		`{"name":"Push Voice","tone":{"formality":"neutral"},"min_score":85}`)
	assert.Equal(t, "unchanged", same.Action, "an identical re-push stays a no-op")

	_, raised := upsertBrandProfile(t, srv, wsID, "u-1",
