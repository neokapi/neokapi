...
func (s *Server) HandleGetVoiceRollup(c echo.Context) error {
	if s.VoiceStore == nil {
	...
	var cs voicescope.ScopeStore
	if s.ContentStore != nil {
		cs = s.ContentStore
	}

	entries := make([]VoiceRollupEntry, 0, len(page))
	for _, p := range page {
		entries = append(entries, s.voiceRollupEntry(ctx, p, wsID, cs, wd, cfg, days))
	}

	return c.JSON(http.StatusOK, VoiceRollupResponse{
		Projects: entries,
		Total:    total,
		Limit:    limit,
		Offset:   offset,
...
// voiceRollupEntry builds one project's rollup row from stored data: the
...
func (s *Server) voiceRollupEntry(
	ctx context.Context,
	p *store.Project,
	wsID string,
	cs voicescope.ScopeStore,
	wd voicescope.WorkspaceDefault,
	cfg coreprofile.DriftConfig,
	days int,
) VoiceRollupEntry {
	entry := VoiceRollupEntry{ProjectID: p.ID, ProjectName: p.Name}
	...
	if profile, err := voicescope.Resolve(ctx, cs, wd, s.VoiceStore, voicescope.Scope{
		ProjectID:   p.ID,
		WorkspaceID: wsID,
	}); err == nil && profile != nil {
