...
func gtConcept(id, domain string, statuses ...model.TermStatus) terms.Concept {
	terms := make([]terms.Term, len(statuses))
	for i, st := range statuses {
		terms[i] = terms.Term{Text: id + "-" + string(st), Locale: "en", Status: st}
	}
	return terms.Concept{ID: id, Domain: domain, Terms: terms, CreatedAt: time.Now(), UpdatedAt: time.Now()}
}
