...
// nextOverlaySpanIndex assigns the stable "entity:N" / "term-candidate:N"
...
func TestNextFacetSpanIndex_Sequential(t *testing.T) {
	...
	b.AddOverlaySpan(model.OverlayEntity, model.Span{ID: "entity:1", Value: &model.EntityAnnotation{Text: "Paris"}})
...
func TestNextFacetSpanIndex_AfterDeleteUsesMaxPlusOne(t *testing.T) {
	...
	b.AddOverlaySpan(model.OverlayEntity, model.Span{ID: "entity:0", Value: &model.EntityAnnotation{Text: "John"}})
	b.AddOverlaySpan(model.OverlayEntity, model.Span{ID: "entity:1", Value: &model.EntityAnnotation{Text: "Paris"}})
	b.AddOverlaySpan(model.OverlayEntity, model.Span{ID: "entity:2", Value: &model.EntityAnnotation{Text: "today"}})
	// Delete the middle one: the next index is still max+1, so identities never
	// collide with a surviving span.
	require.True(t, b.RemoveOverlaySpan(model.OverlayEntity, "entity:1"))
...
func TestNextFacetSpanIndex_PerFacetType(t *testing.T) {
	...
	assert.Equal(t, 0, nextOverlaySpanIndex(b, model.OverlayTermCandidate, "term-candidate:"))
	b.AddOverlaySpan(model.OverlayTermCandidate, model.Span{ID: "term-candidate:0", Value: &model.TermCandidateAnnotation{Text: "Sprint"}})
	assert.Equal(t, 1, nextOverlaySpanIndex(b, model.OverlayTermCandidate, "term-candidate:"))
	// Entity index unaffected.
	assert.Equal(t, 1, nextOverlaySpanIndex(b, model.OverlayEntity, "entity:"))
}
