...
// wireBrandScan attaches a real Postgres brand-scan store, an in-memory
// queue, and a local blob store to a test server.
func wireBrandScan(t *testing.T, s *Server) {
	t.Helper()
	db := pgtest.NewTestDB(t)
	store, err := jobs.NewBrandScanJobStore(db)
	require.NoError(t, err)
	s.BrandScanStore = store
	s.BrandScanQueue = jobs.NewChannelQueue(8)
	if s.BlobStore == nil {
...
// TestBrandScanInfoFeature: /api/v1/info advertises the brand_scan capability
...
func TestBrandScanInfoFeature(t *testing.T) {
	s, _ := newTestServer(t)
	...
	assert.False(t, fetchInfo().Features["brand_scan"], "unwired server must not advertise brand_scan")

	wireBrandScan(t, s)
	assert.True(t, fetchInfo().Features["brand_scan"], "wired server must advertise brand_scan")
}
...
// no-op job.
func TestBrandScanCreate_RequiresSource(t *testing.T) {
...
// TestBrandScanCreate_InsufficientCredits402: the handler's platform-credit
...
func TestBrandScanCreate_InsufficientCredits402(t *testing.T) {
	...
	require.NoError(t, s.HandleCreateBrandScan(c))
	...
	_, _, _, qerr := s.BrandScanQueue.(*jobs.ChannelQueue).Dequeue(canceledContext(t))
	assert.Error(t, qerr, "a refused scan must not enqueue")
}

// TestBrandScanCreate_ZeroCreditWorkspaceRefusedOnRoute: through the full
...
func TestBrandScanCreate_ZeroCreditWorkspaceRefusedOnRoute(t *testing.T) {
	s, ownerToken := newTestServer(t)
	wireBrandScan(t, s)
	...
	_, _, _, qerr := s.BrandScanQueue.Dequeue(canceledContext(t))
	assert.Error(t, qerr, "a refused scan must not enqueue")
}
...
// TestBrandScanCreate_EnqueuesJob: the happy path returns 202 with a job id,
// persists the queued row, and publishes exactly one queue message.
func TestBrandScanCreate_EnqueuesJob(t *testing.T) {
	s, ownerToken := newTestServer(t)
	wireBrandScan(t, s)
	...
	job, err := s.BrandScanStore.GetBrandScanJob(t.Context(), resp.JobID)
	require.NoError(t, err)
	...
	queued, _, _, err := s.BrandScanQueue.Dequeue(t.Context())
	require.NoError(t, err)
	assert.Equal(t, resp.JobID, queued)
}

// TestBrandScanGet_CrossTenant404: a scan owned by another workspace is 404
// (anti-enumeration) even for an authenticated owner of a different workspace.
func TestBrandScanGet_CrossTenant404(t *testing.T) {
	s, ownerToken := newTestServer(t)
	wireBrandScan(t, s)
	ctx := t.Context()

	victim := &jobs.BrandScanJob{
		ID:            "victim-scan",
	...
	require.NoError(t, s.BrandScanStore.CreateBrandScanJob(ctx, victim))
	...
	require.NoError(t, s.BrandScanStore.CreateBrandScanJob(ctx, mine))
...
// TestBrandScanGet_CompletedCarriesDraft: a completed scan returns the result
// payload under "draft".
func TestBrandScanGet_CompletedCarriesDraft(t *testing.T) {
	s, ownerToken := newTestServer(t)
	wireBrandScan(t, s)
	ctx := t.Context()
	...
	require.NoError(t, s.BrandScanStore.CreateBrandScanJob(ctx, job))
	claimed, epoch, err := s.BrandScanStore.ClaimBrandScanJob(ctx, job.ID)
	require.NoError(t, err)
	require.True(t, claimed)
	result := json.RawMessage(`{"profile":{"name":"Draft"},"terms":[],"sources":[],"truncated":false}`)
	owner, err := s.BrandScanStore.CompleteBrandScanJob(ctx, job.ID, epoch, result, 42)
	require.NoError(t, err)
...
// brandScanMultipart builds a multipart body with the given name → content
// files under the "files" field.
func brandScanMultipart(t *testing.T, files map[string][]byte) (*bytes.Buffer, string) {
	t.Helper()
...
func newBrandScanUploadContext(t *testing.T, body *bytes.Buffer, contentType string) (echo.Context, *httptest.ResponseRecorder) {
	t.Helper()
	e := echo.New()
...
type brandScanUploadResponse struct {
	Uploads []struct {
		Key      string `json:"key"`
...
func TestBrandScanUploads_StoresAndSkips(t *testing.T) {
	...
	c, rec := newBrandScanUploadContext(t, body, contentType)

	require.NoError(t, s.HandleBrandScanUploads(c))
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())

	var resp brandScanUploadResponse
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &resp))
	...
	require.NoError(t, err)
	defer rc.Close()
	var env jobs.BrandScanUploadEnvelope
...
// TestBrandScanUploads_Caps: more than 10 files is rejected outright.
func TestBrandScanUploads_Caps(t *testing.T) {
	blobs, err := bloblocal.New(t.TempDir())
	require.NoError(t, err)
	...
	body, contentType := brandScanMultipart(t, files)
	c, rec := newBrandScanUploadContext(t, body, contentType)
...
func TestBrandScanUploads_RequiresPermission(t *testing.T) {
	...
	body, contentType := brandScanMultipart(t, map[string][]byte{"a.md": []byte("x")})
	c, rec := newBrandScanUploadContext(t, body, contentType)
	c.Set("project_permissions", platauth.PermViewContent) // no manage-brand

	err = s.HandleBrandScanUploads(c)
	require.Error(t, err, "requirePermission signals denial via error")
	assert.Equal(t, http.StatusForbidden, rec.Code)
}

// TestBrandScanCheckDraft: the stateless tester scores sample text against an
// inline draft profile — forbidden vocabulary must be caught and scored.
func TestBrandScanCheckDraft(t *testing.T) {
