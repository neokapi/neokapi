...
// format registry — by extension first, then content type — so brandscan reads
...
func readerForFile(reg *registry.FormatRegistry, filename, contentType string) (format.DataFormatReader, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("brandscan: no reader for %q (%q): %w", fmtName, filename, err)
	}
	configureForHarvest(reader)
	return reader, nil
}

// configureForHarvest applies the format-specific harvesting settings brandscan
