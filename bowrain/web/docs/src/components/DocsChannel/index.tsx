 ...
 * to its tag, and is served at /docs/. Every push to main builds the same site
 * at /docs/next/. docusaurus.config.ts resolves which channel a build belongs
 * to and hands it over in `customFields.docsChannel`, so these components read
 * one object instead of inspecting the base URL.
 *
 * Strings go through `<Translate>` and `translate()`, which this site's
 * write-translations pass extracts into code.json.
 */
...
export function useDocsChannel(): DocsChannelInfo | null {
  ...
  return (
    (siteConfig.customFields?.docsChannel as DocsChannelInfo | undefined) ?? null
  );
}
...
function stableLabel(version: string): string {
  return translate(
    {
      id: "docsChannel.stable",
      message: "Stable {version}",
      description: "Label of the stable documentation channel",
    },
    { version },
  );
}
...
function nextLabel(): string {
  return translate({
    id: "docsChannel.next",
    message: "Next",
    description: "Label of the next-release documentation channel",
  });
}
...
export function DocsChannelBanner(): React.ReactElement | null {
  ...
  return (
    <div className={styles.banner} role="note">
      <span>
        <Translate
          id="docsChannel.banner"
          description="Banner shown on every page of the next-release documentation"
          values={{
            version: info.version,
            install: (
              <Link to={info.installTo}>
                {translate({
                  id: "docsChannel.banner.install",
                  message: "Install the beta",
                  description: "Link text to the installation page",
                })}
              </Link>
            ),
            stable: (
              <a href={info.stableUrl}>
                {translate(
                  {
                    id: "docsChannel.banner.stable",
                    message: "documentation for {version}",
                    description: "Link text to the stable documentation",
                  },
                  { version: info.stableVersion },
                )}
              </a>
            ),
          }}
        >
          {
            "These pages describe Bowrain {version}, the release in progress. {install} to follow them, or read the {stable}."
          }
        </Translate>
      </span>
    </div>
...
export default function DocsChannelNavbarItem({
  ...
  const links: ChannelLink[] = [
    {
      key: "stable",
      label: stableLabel(info.stableVersion),
  ...
  const current = links.find((link) => link.key === info.channel);

  if (mobile) {
    return (
      <li className="menu__list-item">
        <span className={`menu__link ${styles.mobileHeading}`}>
          <Translate
            id="docsChannel.heading"
            description="Heading above the channel switch in the mobile menu"
          >
            ...
            <li className="menu__list-item" key={link.key}>
              <a
                className={`menu__link ${
                  link.key === info.channel ? "menu__link--active" : ""
                }`}
                href={link.href}
  ...
  return (
    <div
      className={`navbar__item dropdown dropdown--hoverable dropdown--right ${styles.switch}`}
    >
      <a
        href={current?.href ?? info.stableUrl}
        className="navbar__link"
        aria-haspopup="true"
        aria-label={translate({
          id: "docsChannel.heading",
          message: "Documentation channel",
          description:
            "Heading above the channel switch in the mobile menu",
        })}
