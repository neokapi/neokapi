...
# Or run a custom flow declared in flows/translate-website.yaml
kapi run translate-website
```

A custom flow composes tools into a pipeline. `flows/translate-website.yaml`, with `flows_dir: flows` in `kapi.yaml`:
...
decision, and `kapi apply` records one in the project's decision ledger, where
every checkout reads it.
...
2. Every run and check reads the project's terms store with nothing bound; `kapi context import` reads a terms bundle into it
...
- **Initialize projects with `kapi init`** to write a `kapi.yaml` recipe and the checkout's `.kapi/` cache
- **Bind a voice profile and a terms store** in `defaults:` so drafts and checks read the same context
- **Define custom flows** in the directory `flows_dir:` names, for your specific workflow
