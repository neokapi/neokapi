...
#   flow translate: completed (42s)
#   flow qa: completed (8s)
...
<PipelineDiagram
  channelLabel=""
  stages={[
    { label: "Push", sub: "PushID abc123", role: "io" },
    { label: "Event", sub: "connector.push.completed" },
    {
      parallelLabel: "automations linked to abc123",
      lanes: [{ label: "auto_translate" }, { label: "auto_extract" }],
    },
    { label: "jobs (N/M completed)", sub: "polled by push_id", role: "io" },
  ]}
/>

The wait phase polls `GET /api/v1/projects/:id/sync/{ref}/status?push_id=abc123` until all linked flows complete or the timeout expires. This ensures that `kapi sync` only pulls translations that result from _this_ push, not from unrelated server activity.
