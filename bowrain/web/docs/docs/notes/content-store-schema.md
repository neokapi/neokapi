...
The Content Store schema is shared across SQLite (CLI) and PostgreSQL (server) backends, with the content memory system ([Framework AD-009](https://neokapi.github.io/contribute/architecture/009-content-memory)) and Terminology ([Framework AD-010](https://neokapi.github.io/contribute/architecture/010-terminology)). The schema below shows the table definitions using SQLite syntax for readability; server backends use equivalent types with `$N` parameter placeholders instead of `?`.
