...
Built-in connectors register via `init()`. Plugin connectors register at runtime via gRPC discovery ([Framework AD-007](https://neokapi.github.io/web/neokapi/contribute/architecture/007-plugin-system)). The bowrain CLI does not interact with the Registry -- it is a file-based tool that syncs with the server via API.
