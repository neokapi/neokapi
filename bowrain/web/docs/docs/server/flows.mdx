...
import { PhaseFlow, PipelineDiagram, SwimlaneDiagram } from "@neokapi/docs-shared";
...
<PhaseFlow
  nodes={[
    ...
    { label: "Flow execution starts", edge: "run_flow", role: "translate" },
    { label: "Read blocks from ContentStore", edge: "read", role: "io" },
    { label: "Process through tool pipeline", edge: "tools", role: "translate" },
    { label: "Write results to ContentStore", edge: "write", role: "io" },
    { label: "Emit flow.completed / flow.failed", edge: "emit", role: "qa" },
  ]}
/>
...
# Filter by flow name
curl "https://bowrain.cloud/api/v1/projects/:id/flows/executions?flow=ai-translate" \
  -H "Authorization: Bearer $TOKEN"
...
<SwimlaneDiagram
  ...
  messages={[
    ...
    { from: 1, to: 1, label: "Automation: run ai-translate", detail: "read → translate → write" },
    { from: 1, to: 1, label: "Event: flow.completed" },
    { from: 1, to: 1, label: "Automation: run qa-check", detail: "validate → write annotations" },
    { from: 1, to: 1, label: "Event: flow.completed → quality.gate.pass" },
