...
above chains translate and QA in one run. Automations react to content
events such as `connector.push.completed`; chaining flows on a
`flow.completed` event is intended but that event is not yet emitted (see
the lifecycle diagram above). See
