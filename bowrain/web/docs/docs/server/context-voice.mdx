...
profile *brand*; a workspace with several audiences carries several profiles. On
its own, a profile is just a set of
...
1. **Correct.** A person — or an AI assistant working through
   ...
   records the correction with provenance: the original text, the fix, the voice
...
The detection and scoring are the open framework's job (`core/profile`,
...
workspace, swapping a preferred term — the [Context](/server/context) hub bundles the
