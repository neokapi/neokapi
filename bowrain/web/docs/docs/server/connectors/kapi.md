...
- A release should be **gated** on language coverage with an explicit exit code
...
- A local pass is wanted occasionally without the server: kapi runs standalone
  on files with no account at all.
...
the same content memory, the same review queue.
...
into the team's [review queue](/server/review).
