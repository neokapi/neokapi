...
The editor works on content from any [connector](/server/connectors) — a content platform, a design tool, a repository, or a developer's checkout — held on the server, shared across a team and governed over time. These short tours run against a live workspace.
