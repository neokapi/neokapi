...
what the workspace has already settled. In the engine this store is the
translation memory (TM) — this page uses TM when referring to the mechanism
(entries, matches, scores). This page's own route,
`/server/translation-memory`, keeps that historical name too, so existing links
