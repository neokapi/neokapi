...
- **Apply exact TM** — [Review](./review.mdx) fills selected blocks from exact
  (100%) matches.
...
Bowrain's matching accounts for inline formatting differences, so a source segment and a stored entry can match even if their tag structure differs slightly. For more on how the underlying TM engine matches across projects, see the [Neokapi TM documentation](https://neokapi.github.io/web/neokapi/).
