...
- **Translate** — edit blocks with AI and memory assistance, and terminology insertion.
...
- **[Pre-process](/server/pre-process)** — file-wide source-prep (pseudo-translate,
  bulk reuse from memory) before editing begins.
...
Opening a file gives a side-by-side editor: source, target, live preview, and
per-block memory and terminology matches.
...
and per-block context — content memory matches and terminology. Navigate
...
[Inline Formatting](https://neokapi.github.io/framework/inline-formatting)
for more details.
