...
func (s *PostgresAuthStore) CreateWorkspace(ctx context.Context, w *platauth.Workspace) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO workspaces (id, name, slug, description, logo_url, type, plan, stripe_customer_id, dashboard_visibility, pulse_access_key, pulse_term_sources, preferred_model, created_at, updated_at)
		...
		w.PreferredModel, w.CreatedAt, w.UpdatedAt)
	if err != nil {
...
func (s *PostgresAuthStore) GetWorkspace(ctx context.Context, id string) (*platauth.Workspace, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, name, slug, description, logo_url, type, plan, stripe_customer_id, dashboard_visibility, pulse_access_key, pulse_term_sources, preferred_model, created_at, updated_at
...
func (s *PostgresAuthStore) ListWorkspaces(ctx context.Context, userID string) ([]*platauth.Workspace, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT w.id, w.name, w.slug, w.description, w.logo_url, w.type, w.plan, w.stripe_customer_id, w.dashboard_visibility, w.pulse_access_key, w.pulse_term_sources, w.preferred_model, w.created_at, w.updated_at, wm.role
		 FROM workspaces w
...
func (s *PostgresAuthStore) UpdateWorkspace(ctx context.Context, w *platauth.Workspace) error {
	...
	termSrcJSON, _ := json.Marshal(w.PulseTermSources)
	res, err := s.db.ExecContext(ctx,
		`UPDATE workspaces SET name=$1, slug=$2, description=$3, logo_url=$4, plan=$5, stripe_customer_id=$6, dashboard_visibility=$7, pulse_access_key=$8, pulse_term_sources=$9, preferred_model=$10, updated_at=$11 WHERE id=$12`,
		w.Name, w.Slug, w.Description, w.LogoURL, w.Plan, w.StripeCustomerID,
		string(w.DashboardVisibility), w.PulseAccessKey, string(termSrcJSON), w.PreferredModel, w.UpdatedAt, w.ID)
	if err != nil {
...
func scanWorkspacePg(row scanner) (*platauth.Workspace, error) {
	...
	err := row.Scan(&w.ID, &w.Name, &w.Slug, &w.Description, &w.LogoURL, &wsType, &w.Plan, &stripeCustomerID, &dashVis, &w.PulseAccessKey, &termSrc, &w.PreferredModel, &w.CreatedAt, &w.UpdatedAt)
	if err != nil {
...
func scanWorkspaceWithRolePg(row scanner) (*platauth.Workspace, error) {
	...
	var stripeCustomerID *string
	var dashVis, termSrc string
	err := row.Scan(&w.ID, &w.Name, &w.Slug, &w.Description, &w.LogoURL, &wsType, &w.Plan, &stripeCustomerID, &dashVis, &w.PulseAccessKey, &termSrc, &w.PreferredModel, &w.CreatedAt, &w.UpdatedAt, &role)
	if err != nil {
