...
/**
 * Gradient background with soft orbs for glassmorphism UIs.
 * Orbs are static (no animation) to avoid continuous CPU/GPU usage.
...
export function AnimatedBackgroundGlass({ className, showCenterOrb }: AnimatedBackgroundProps) {
  ...
  return (
    <div
      className={cn(
        "fixed inset-0 transition-colors duration-500 overflow-hidden pointer-events-none",
        className,
      )}
    ...
    >
      <div
        className="absolute top-20 -left-20 w-[600px] h-[600px] rounded-full will-change-transform"
        style={{ background: "var(--orb-1)", filter: "blur(80px)" }}
      />
      <div
        className="absolute -bottom-20 -right-20 w-[700px] h-[700px] rounded-full will-change-transform"
        style={{ background: "var(--orb-2)", filter: "blur(100px)" }}
      />
      <div
        className="absolute top-1/3 -right-10 w-[500px] h-[500px] rounded-full will-change-transform"
        style={{ background: "var(--orb-3)", filter: "blur(70px)" }}
      />
      <div
        className="absolute -bottom-10 left-1/4 w-[450px] h-[450px] rounded-full will-change-transform"
        style={{ background: "var(--orb-4)", filter: "blur(60px)" }}
      />
      {shouldShowCenterOrb && (
        <div
