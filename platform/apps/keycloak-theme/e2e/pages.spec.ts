...
const pages: { pageId: string; expect: string }[] = [
  { pageId: "login.ftl", expect: "glass-surface" },
  { pageId: "login-username.ftl", expect: "glass-surface" },
  { pageId: "register.ftl", expect: "glass-surface" },
  { pageId: "error.ftl", expect: "glass-surface" },
  { pageId: "info.ftl", expect: "glass-surface" },
  { pageId: "login-verify-email.ftl", expect: "glass-surface" },
  { pageId: "logout-confirm.ftl", expect: "glass-surface" },
  { pageId: "login-page-expired.ftl", expect: "glass-surface" },
  { pageId: "login-idp-link-confirm.ftl", expect: "glass-surface" },
  { pageId: "login-idp-link-email.ftl", expect: "glass-surface" },
  { pageId: "login-passkeys-conditional-authenticate.ftl", expect: "glass-surface" },
  { pageId: "webauthn-authenticate.ftl", expect: "glass-surface" },
  { pageId: "webauthn-register.ftl", expect: "glass-surface" },
  { pageId: "webauthn-error.ftl", expect: "glass-surface" },
];
...
for (const p of pages) {
  test(`renders ${p.pageId} with custom theme`, async ({ page }) => {
    ...
    // The glass-surface card should be present (not falling through to DefaultPage).
...
test("login-page-expired shows restart links", async ({ page }) => {
  ...
  const links = page.locator(".glass-surface a");
