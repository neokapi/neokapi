...
const isCI =
  (process.env.CI === "true" || process.env.CI === "1") && process.env.FORCE_RECORDINGS !== "true";
...
const describeOrSkip = isCI
  ? test.describe.skip.bind(test.describe)
  : test.describe.serial.bind(test.describe);
...
describeOrSkip("Video Recordings", () => {
  ...
  for (const theme of themes) {
    ...
    test(`record translation editor workflow [${theme}]`, async ({ page }) => {
      ...
      if (!useServerMode)
        await page.evaluate(async () => {
          ...
          const blocks = [
            ...
            {
              id: "block-p1",
              source: "We are a leading technology company founded in 2015.",
              targets: {},
              translatable: true,
              has_spans: false,
            ...
            {
              id: "block-p2",
              source: "Our mission is to deliver innovative solutions that transform businesses.",
              targets: {},
              translatable: true,
              has_spans: true,
              properties: { element: "p" },
            ...
            {
              id: "block-p3",
              source: "We believe in quality, integrity, and customer success.",
              targets: {},
              translatable: true,
              has_spans: true,
              properties: { element: "p" },
            ...
            {
              id: "block-p4",
              source: "Every project starts with understanding your unique needs.",
              targets: {},
              translatable: true,
              has_spans: false,
              properties: { element: "p" },
            ...
            {
              id: "block-p5",
              source: "Our talented team brings decades of combined experience.",
              targets: {},
              translatable: true,
              has_spans: false,
              properties: { element: "p" },
            ...
            {
              id: "block-p6",
              source: "Contact us to learn how we can help your business grow.",
              targets: {},
              translatable: true,
              has_spans: true,
              properties: { element: "p" },
            ...
            {
              id: "block-link",
              source: "Read more about our services",
              targets: {},
              translatable: true,
              has_spans: false,
          ...
          backend[IDS.GetItemBlocks] = (pid: string, fileName: string) => {
            ...
            if (customBlocks)
              return customBlocks.map((b: any) => ({ ...b, targets: { ...b.targets } }));
            return origGetItemBlocks(pid, fileName);
          };

          // Patch UpdateBlockTarget
          backend[IDS.UpdateBlockTarget] = (req: any) => {
            const blocks = (window as any).__projectFiles?.[req.project_id]?.[
              req.item_name || req.file_name
            ];
            if (blocks) {
          ...
          backend[IDS.RenderDocumentPreview] = (
            _projectID: string,
            _itemName: string,
            targetLocale: string,
          ) => {
      ...
      await humanType(
        page,
        page.getByTestId("edit-target-1"),
        "Vi er et ledende teknologiselskap grunnlagt i 2015.",
      );
      await pause(page, 200);
      await page.keyboard.press("Tab");
      ...
      await humanType(
        page,
        page.getByTestId("edit-target-2"),
        "Vår misjon er å levere innovative løsninger som transformerer bedrifter.",
      );
      await pause(page, 200);
      await page.keyboard.press("Tab");
      ...
      await humanType(
        page,
        page.getByTestId("edit-target-5"),
        "Vi tror på kvalitet, integritet og kundesuksess.",
      );
      await pause(page, 200);
      await page.keyboard.press("Tab");
      ...
      await humanType(
        page,
        page.getByTestId("edit-target-6"),
        "Hvert prosjekt starter med å forstå dine unike behov.",
      );
      await pause(page, 200);
      await page.keyboard.press("Tab");
    ...
    test(`record TM leverage workflow [${theme}]`, async ({ page }) => {
      ...
      if (leveragePid) {
        ...
        await callBackend(
          ...
          "AddTMEntry",
          leveragePid,
          "Hello from landing.html",
          "Bonjour depuis landing.html",
          "en",
          "fr",
        );
        await callBackend(
    ...
    test(`record visual editor workflow [${theme}]`, async ({ page }) => {
      ...
      if (useServerMode) {
        ...
        if (vePid) {
          ...
          await callBackend(
            ...
            "AddTMEntry",
            vePid,
            "Welcome to the CloudSync Pro documentation and support portal.",
            "Velkommen til CloudSync Pro-dokumentasjonen og støtteportalen.",
            "en",
            "nb",
          );
        ...
        await page.evaluate(
          async ({ mo, mc, mp }: { mo: string; mc: string; mp: string }) => {
            const backend = (window as any).__wailsMock;
            const IDS = (window as any).__wailsIDs;
            ...
            const blocks = [
              {
                id: "block-h1",
                source: "CloudSync Pro Help Center",
                targets: {} as Record<string, string>,
                targets_coded: {} as Record<string, string>,
                translatable: true,
                has_spans: false,
                properties: { element: "h1" },
              },
              {
                id: "block-p1",
                source: "Welcome to the CloudSync Pro documentation and support portal.",
                targets: {} as Record<string, string>,
                targets_coded: {} as Record<string, string>,
                translatable: true,
                has_spans: false,
              ...
              },
              {
                id: "block-p5",
                source:
                  "All data is encrypted with AES-256 encryption during transfer. Your privacy is our priority.",
              ...
              {
                id: "block-link",
                source: "Visit our community forum for tips and discussions",
                targets: {} as Record<string, string>,
                targets_coded: {} as Record<string, string>,
                translatable: true,
                has_spans: false,
            ...
            blocks[1].targets["sv"] =
              "Välkommen till CloudSync Pros dokumentation och supportportal.";
            blocks[2].targets["sv"] =
              "Lär dig hur du synkroniserar dina filer säkert på alla enheter.";
      ...
      await page.evaluate(() => {
        ...
        if (el) {
          ...
          document.execCommand(
            "insertText",
            false,
            "Lær hvordan du synkroniserer filene dine sikkert.",
          );
        }
      });
      await pause(page, 800);
      ...
      await page.evaluate(() => {
        const cardTabs = document.querySelectorAll(
          '[data-testid="visual-editor-card"] [role="tab"]',
        );
      ...
      await page.evaluate(() => {
        ...
        const translateTab = Array.from(cardTabs).find(
          (t) => t.textContent?.trim() === "Translate",
        );
