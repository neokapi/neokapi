...
export interface PulseHeatmapDay {
  date: string;
  count: number;
}
...
export async function fetchActivityHeatmap(
  workspace: string,
): Promise<{ days: PulseHeatmapDay[] }> {
  const res = await fetch(`${BASE}/${workspace}/activity/heatmap`);
...
export async function fetchProjects(
  workspace: string,
): Promise<{ projects: PulseProjectSummary[] }> {
...
): Promise<{ activities: PulseActivity[] }> {
