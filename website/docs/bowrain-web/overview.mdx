...
The gokapi web application is a browser-based translation workbench that provides the same powerful editing experience as the [Bowrain desktop app](/docs/bowrain-desktop/overview), accessible from any modern browser. It runs as part of `bowrain-server` for multi-user SaaS deployments, or as part of `brain serve` for local single-user workflows.
...
| **Local** | `brain serve` | None (single user) | Single, auto-created | Local development, personal use |
