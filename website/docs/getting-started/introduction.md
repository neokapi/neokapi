...
- **Terminology management** — Concept-oriented termbase with TBX-inspired data model and pipeline enforcement tools
- **Desktop, CLI, and server** — Bowrain (cross-platform GUI), `kapi` (CLI), and `gokapi-server` (REST API) all compile to standalone executables from one codebase
