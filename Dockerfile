...
COPY bowrain/packages/ui/package.json bowrain/packages/ui/package-lock.json* bowrain/packages/ui/
RUN cd bowrain/packages/ui && npm ci
...
# Cache module downloads (both modules + workspace).
COPY go.mod go.sum go.work ./
COPY bowrain/go.mod bowrain/go.sum ./bowrain/
RUN go mod download && cd bowrain && go mod download
...
# Build bowrain-server from platform module. Pure Go (modernc.org/sqlite), no CGO needed.
