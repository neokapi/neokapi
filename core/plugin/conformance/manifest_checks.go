...
func checkManifestParse(_ context.Context, r *runner) (Status, string, error) {
	switch {
	case r.readErr != nil:
		return Skip, fmt.Sprintf("%s unreadable — see manifest.present", ManifestFile), nil
...
func checkManifestNoUnknownFields(_ context.Context, r *runner) (Status, string, error) {
	...
	if err := dec.Decode(&probe); err != nil {
		...
		return Skip, fmt.Sprintf("%s did not decode — see manifest.parse", ManifestFile), nil
	}
