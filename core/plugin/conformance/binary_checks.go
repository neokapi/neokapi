...
func (r *runner) pluginEnv() []string {
	...
	env = append(env,
		...
		"KAPI_PLUGIN_NAME="+r.manifestField(func(m *manifestView) string { return m.plugin }),
		"KAPI_PLUGIN_VERSION="+r.manifestField(func(m *manifestView) string { return m.version }),
	)
	return append(env, r.suite.Env...)
}

// manifestView is a tiny read-only projection so pluginEnv works even when the
// manifest failed to parse (both fields are then empty).
type manifestView struct{ plugin, version string }

func (r *runner) manifestField(get func(*manifestView) string) string {
	v := &manifestView{}
	if r.man != nil {
		v.plugin, v.version = r.man.Plugin, r.man.Version
	}
	return get(v)
}
