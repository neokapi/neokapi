...
func TestMain(m *testing.M) {
	...
	if probeBin, err = build(root, buildDir, probePkg, "kapi-probe"); err != nil {
		fmt.Fprintln(os.Stderr, "conformance tests:", err)
		os.Exit(1)
	}
	if helloBin, err = build(root, buildDir, helloPkg, "kapi-hello"); err != nil {
		fmt.Fprintln(os.Stderr, "conformance tests:", err)
		os.Exit(1)
	}
	if helloManifest, err = os.ReadFile(
		...
		fmt.Fprintln(os.Stderr, "conformance tests: read hello manifest:", err)
		os.Exit(1)
	}
...
func build(root, outDir, pkg, name string) (string, error) {
...
func pluginDir(t *testing.T, plugin string, man map[string]any, binSrc string) string {
	...
	if binSrc != "" {
		...
		require.NoError(t, os.WriteFile(dst, data, 0o755)) //nolint:gosec // fixture must be executable
	}
...
func probeDaemonPIDs(t *testing.T) []string {
	t.Helper()
	out, err := exec.Command("pgrep", "-f", "kapi-probe daemon").Output()
	if err != nil {
