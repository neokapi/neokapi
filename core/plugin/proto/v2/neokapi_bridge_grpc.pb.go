...
const (
	...
	BridgeService_LocateComments_FullMethodName = "/neokapi.bridge.v2.BridgeService/LocateComments"
...
type BridgeServiceClient interface {
	...
	// LocateComments locates the comments in one source file for the comment
	// layer. It backs the comment languages declared via capabilities.comments:
	// the host sends a file's bytes and its language, and the daemon returns
	// every comment it located and every comment it set aside, with byte spans
	// into the bytes it was sent. Unary, and it writes nothing.
	LocateComments(ctx context.Context, in *LocateCommentsRequest, opts ...grpc.CallOption) (*LocateCommentsResponse, error)
...
func (c *bridgeServiceClient) LocateComments(ctx context.Context, in *LocateCommentsRequest, opts ...grpc.CallOption) (*LocateCommentsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(LocateCommentsResponse)
	err := c.cc.Invoke(ctx, BridgeService_LocateComments_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type BridgeServiceServer interface {
	...
	// every comment it located and every comment it set aside, with byte spans
	// into the bytes it was sent. Unary, and it writes nothing.
	LocateComments(context.Context, *LocateCommentsRequest) (*LocateCommentsResponse, error)
	// Shutdown gracefully shuts down the bridge server.
...
func (UnimplementedBridgeServiceServer) LocateComments(context.Context, *LocateCommentsRequest) (*LocateCommentsResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method LocateComments not implemented")
}
...
func _BridgeService_LocateComments_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(LocateCommentsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(BridgeServiceServer).LocateComments(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: BridgeService_LocateComments_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(BridgeServiceServer).LocateComments(ctx, req.(*LocateCommentsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var BridgeService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "LocateComments",
			Handler:    _BridgeService_LocateComments_Handler,
		},
		{
			MethodName: "Shutdown",
