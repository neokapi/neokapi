...
func (s *streamScanner) next() (token, error) {
	...
	default:
		...
		return token{}, fmt.Errorf("xcstrings scanner: unexpected character %q", b)
	}
}
...
func (s *streamScanner) scanString(prefix string) (token, error) {
	...
	for {
		b, err := s.r.ReadByte()
		if err != nil {
			return token{}, errors.New("xcstrings scanner: unterminated string")
		}
		raw = append(raw, b)
		if b == '"' {
		...
		if b == '\\' {
			...
			if err != nil {
				return token{}, errors.New("xcstrings scanner: unterminated escape")
			}
			raw = append(raw, esc)
			switch esc {
			case '"':
		...
		for range n {
			cb, err := s.r.ReadByte()
			if err != nil {
				return token{}, errors.New("xcstrings scanner: truncated UTF-8")
			}
			raw = append(raw, cb)
			mb = append(mb, cb)
		}
...
func (s *streamScanner) scanUnicodeEscape() (rune, int, []byte, error) {
	...
	if _, err := io.ReadFull(s.r, hex); err != nil {
		return 0, 0, nil, errors.New("xcstrings scanner: incomplete unicode escape")
	}
	r1, err := strconv.ParseUint(string(hex), 16, 32)
	if err != nil {
		return 0, 0, nil, fmt.Errorf("xcstrings scanner: invalid unicode escape \\u%s", hex)
	}
...
func (s *streamScanner) scanLiteral(first byte, expected string, typ tokenType, prefix string) (token, error) {
	...
	if _, err := io.ReadFull(s.r, rest); err != nil {
		return token{}, fmt.Errorf("xcstrings scanner: expected %q", expected)
	}
