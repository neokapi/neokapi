...
// TestCorpusByteFaithfulRoundTrip reads every real-world String Catalog vendored
// under testdata/corpus/ and asserts that an untouched read→write reproduces the
// original bytes exactly. These are genuine, permissively-licensed files (see
// testdata/corpus/SOURCES.md): a failure here is a real-world byte-fidelity bug
// in the reader/writer, not a fixture artefact.
func TestCorpusByteFaithfulRoundTrip(t *testing.T) {
	matches, err := filepath.Glob(filepath.Join("testdata", "corpus", "*.xcstrings"))
	if err != nil {
