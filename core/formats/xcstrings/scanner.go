...
// This file provides a whitespace-preserving JSON tokenizer used by the
// writer to rewrite leaf "value" strings inside the original document while
// leaving every other byte untouched. It mirrors the approach used by the
// native JSON format: each token carries its preceding whitespace as a
// prefix so the stream re-serializes byte-for-byte.
...
func (s *scanner) next() (token, error) {
	...
	default:
		...
		return token{}, fmt.Errorf("xcstrings scanner: unexpected character %q at position %d", ch, s.pos)
	}
}
...
func (s *scanner) scanString(prefix string) (token, error) {
	...
	for s.pos < len(s.input) {
		...
		if ch == '\\' {
			s.pos++
			if s.pos >= len(s.input) {
				return token{}, fmt.Errorf("xcstrings scanner: unterminated escape at %d", s.pos)
			}
			esc := s.input[s.pos]
			switch esc {
			case '"':
	...
	return token{}, fmt.Errorf("xcstrings scanner: unterminated string at %d", start)
}

func (s *scanner) scanUnicodeEscape() (rune, int, error) {
	s.pos++ // past 'u'
	if s.pos+4 > len(s.input) {
		return 0, 0, fmt.Errorf("xcstrings scanner: incomplete unicode escape at %d", s.pos)
	}
	hex1 := string(s.input[s.pos : s.pos+4])
	r1, err := strconv.ParseUint(hex1, 16, 32)
	if err != nil {
		return 0, 0, fmt.Errorf("xcstrings scanner: invalid unicode escape \\u%s", hex1)
	}
...
func (s *scanner) scanLiteral(expected string, typ tokenType, prefix string) (token, error) {
	if s.pos+len(expected) > len(s.input) || string(s.input[s.pos:s.pos+len(expected)]) != expected {
		return token{}, fmt.Errorf("xcstrings scanner: expected %q at %d", expected, s.pos)
	}
	s.pos += len(expected)
