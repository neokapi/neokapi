...
const (
	...
	// TypeDMLRunProps tags a paired code carrying a DrawingML run's <a:rPr>
	// (ECMA-376 Part 1 §21.1.2.3.9, CT_TextCharacterProperties) as the source
	// wrote it. The element holds far more than the model names: the language,
	// the size, the spelling and smart-tag flags, a <a:solidFill> colour, a
	// <a:latin> typeface, a <a:hlinkClick>. The named formatting travels
	// beside it as fmt:bold and its siblings, so a translator sees where the
	// bold starts and stops without the reader having to model the rest.
	//
	// Where the SpreadsheetML code carries a list of children, this one
	// carries one element, because a DrawingML run states most of its
	// formatting in attributes on <a:rPr> itself. The writer replays those
	// bytes and reconciles only the named attributes against the codes that
	// are open, so a slide nobody translated goes back byte for byte and a
	// translator who turns bold off still gets b="1" removed.
	TypeDMLRunProps = "fmt:dml-run-props"
)
...
// AttrDMLRPr is the run-attribute key under which a DrawingML code carries the
// source bytes of the <a:rPr> it stands for. It travels on an attribute for the
// same reason AttrSMLRPr does: Data is what a writer replays for a code whose
// type it does not know, and `<a:rPr lang="en-US"/>` is not markup any other
// format should echo.
const AttrDMLRPr = "openxml:dml-rpr"

// SubType constants provide format-specific refinement.
const (
	SubTypeSMLRunProps = "openxml:rPr"
	SubTypeDMLRunProps = "openxml:a:rPr"
	// SubTypeDMLField tags a captured <a:fld>, the DrawingML text field that
	// holds a slide number or a date (ECMA-376 Part 1 §21.1.2.2.4). Its text is
	// generated by the presentation, so the element travels whole.
	SubTypeDMLField = "openxml:a:fld"
	// SubTypeDMLParaChild tags any other direct child of <a:p> the reader
	// captured rather than modelled, such as <a:m> (§21.1.2.2.7) or a
	// paragraph-level <mc:AlternateContent>.
	SubTypeDMLParaChild  = "openxml:a:p-child"
