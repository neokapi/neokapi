...
// ARB reader (and the writer for the roundtrip view). Harvest cohort:
...
func TestSpec(t *testing.T) {
	...
	r := &spectest.NativeRunner{
		Spec:      s,
		NewReader: func(_ string) format.DataFormatReader { return arb.NewReader() },
		NewWriter: func(_ string) format.DataFormatWriter { return arb.NewWriter() },
	}
	r.Run(t)
}
