...
- **TM absorb on merge** — every accepted target segment becomes a new
  ...
  originating XLIFF filename) so a later `kapi memory audit --batch <id>`
...
```bash
kapi memory import ./corporate-en-fr.tmx -p app.kapi
```
...
```bash
kapi memory audit --batch <merge-batch-id>
```
