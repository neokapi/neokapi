...
func (w *Writer) flush() (retErr error) {
	...
	for _, part := range w.parts {
		...
		// `@id` is what the document says; a block's ID is an identity for the
		// store, and the two are the same for every conforming document.
		fmt.Fprintf(ew, `      <trans-unit id="%s"`, xmlesc.Attr(model.DocumentID(block)))
