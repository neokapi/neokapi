...
// genRESX streams a RESX document with n translatable <data> entries through an
// io.Pipe, so the document never exists as a whole buffer on the producer side.
func genRESX(n int) (io.Reader, int) {
	...
	entry := func(i int) string {
		return fmt.Sprintf("  <data name=\"Key_%d\" xml:space=\"preserve\">\n"+
			"    <value>Translatable value number %d with several words</value>\n"+
			"    <comment>note for %d</comment>\n  </data>\n", i, i, i)
	}
...
// TestStreamingReaderBoundedMemory asserts the resx reader's peak heap stays
...
// document or the whole token slice.
...
func TestStreamingByteExact_Large(t *testing.T) {
	...
	assert.Equal(t, string(input), buf.String(), "large RESX must round-trip byte-exact through the streaming path")
}
