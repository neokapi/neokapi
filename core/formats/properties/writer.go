...
// Ensure Writer implements SkeletonStoreConsumer, WriterConfigurable, and
// StreamingWriter.
var (
...
// paths so both produce identical output. A nil block contributes nothing,
// matching the buffered path's map miss.
func (w *Writer) renderRef(block *model.Block) ([]byte, error) {
	if block == nil {
		return nil, nil
	}
	return []byte(w.blockValue(block)), nil
}
