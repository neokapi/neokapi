...
func TestPlaceholderProtection(t *testing.T) {
	...
	assert.True(t, model.RunsHaveInlineCodes(welcome.SourceRuns()), "%1$s should be a code")
	...
	assert.True(t, model.RunsHaveInlineCodes(fileSize.SourceRuns()))
	assert.Equal(t,
	...
	assert.True(t, model.RunsHaveInlineCodes(rich.SourceRuns()))
	assert.Equal(t,
