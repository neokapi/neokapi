...
func TestRejectsUnknownKind(t *testing.T) {
	data := []byte(`{"schemaVersion":"1.0","kind":"not-a-kbf","generator":{"id":"x","version":"1"},"project":{"id":"p","sourceLocale":"en"},"documents":[]}`)
	_, err := Unmarshal(data)
	require.Error(t, err)
...
// The TypeScript mirror (packages/kapi-format/tests/kbf-canonical.test.ts)
// asserts this same literal, so a change to how either side writes a target's
// provenance fails on both.
func TestMarshalTargetOriginsParity(t *testing.T) {
	b := Block{
		ID: "b", Hash: "h", Translatable: true, Type: BlockTypeJSXElement,
		Source: []Run{{Text: &TextRun{Text: "hi"}}},
		Targets: map[LocaleID][]Run{
			"qps": {{Text: &TextRun{Text: "ĥî"}}},
			"fr":  {{Text: &TextRun{Text: "salut"}}},
		},
		TargetOrigins: map[LocaleID]TargetOrigin{
			"qps": {Kind: "mt", Tool: "pseudo-translate", Timestamp: "2026-09-04T00:00:00Z"},
			"fr": {
				Kind: "ai", Engine: "claude", Confidence: 0.5, Profile: "voice",
				ProfileVersion: "3", ContextFingerprint: "abc", Reference: "batch-1",
			},
			"de": {},
		},
		Placeholders: []Placeholder{},
		Properties:   BlockProperties{File: "a.tsx", Line: 1, Component: "C", JSXPath: "p", Element: "p"},
	}
	f := &File{
		Generator: GeneratorInfo{ID: "t", Version: "1"},
		Project:   ProjectInfo{ID: "p", SourceLocale: "en"},
		Documents: []Document{{ID: "d", DocumentType: DocumentTypeJSX, Path: "a.tsx", Blocks: []Block{b}}},
	}
	out, err := Marshal(f)
	require.NoError(t, err)
	assert.Equal(t, targetOriginsGolden, string(out))

	back, err := Unmarshal(out)
	require.NoError(t, err)
	assert.Equal(t, b.TargetOrigins, back.Documents[0].Blocks[0].TargetOrigins)
}
