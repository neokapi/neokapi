...
func (s RunPathStep) MarshalJSON() ([]byte, error) {
	...
	return nil, fmt.Errorf("kbf: run path step has unknown kind %d", s.Kind)
}
...
func (s *RunPathStep) UnmarshalJSON(data []byte) error {
	...
	if len(trimmed) > 0 && (trimmed[0] == '-' || (trimmed[0] >= '0' && trimmed[0] <= '9')) {
		...
		if err := json.Unmarshal(data, &n); err != nil {
			return fmt.Errorf("kbf: decode run path index step: %w", err)
		}
	...
	if err := json.Unmarshal(data, &obj); err != nil {
		return fmt.Errorf("kbf: decode run path step: %w", err)
	}
	if raw, ok := obj["plural"]; ok {
		var form PluralForm
		if err := json.Unmarshal(raw, &form); err != nil {
			return fmt.Errorf("kbf: decode run path plural step: %w", err)
		}
	...
	if raw, ok := obj["select"]; ok {
		...
		if err := json.Unmarshal(raw, &v); err != nil {
			return fmt.Errorf("kbf: decode run path select step: %w", err)
		}
		*s = RunPathStep{Kind: StepSelect, SelectValue: v}
		return nil
	}
	return errors.New("kbf: run path step has no recognized discriminator")
}
