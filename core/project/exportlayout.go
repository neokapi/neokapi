...
// ProfilesDir returns the absolute path of the per-profile directories.
func (e ExportLayout) ProfilesDir() string {
	return filepath.Join(e.StateDir, ProfilesDirName)
}

// ProfileDir returns the absolute path of one profile's directory. The name is
// the profile's key under `profiles:`.
