...
// Service). Provider configurations are stored as a JSON file; API keys never
// touch the disk.
//
// The package is host-agnostic: each host (kapi CLI, Kapi Desktop, bowrain
// server/worker/desktop) supplies its own config file path and keychain
// service name via NewStore.
package credentials
...
// NewStore creates a Store backed by the given JSON file path, keeping API
// keys under the given keychain service name. If the file does not exist, the
// store starts empty.
func NewStore(filePath, keyringService string) *Store {
	s := &Store{filePath: filePath, keyringService: keyringService}
	s.load()
	return s
}
