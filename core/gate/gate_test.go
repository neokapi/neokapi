...
func TestTargetLadder_Order(t *testing.T) {
	l := TargetLadder()
	assert.Equal(t, Ladder{"draft", "translated", "established"}, l)
	assert.True(t, l.Has("established"))
	...
	assert.Less(t, l.rank("translated"), l.rank("established"))
	assert.Equal(t, -1, l.rank("signed_off"), "underscore form is not a rung")
}

func TestGate_Validate(t *testing.T) {
	l := TargetLadder()
	require.NoError(t, Gate{"translated": {Pct: 100}, "established": {Pct: 80}}.Validate(l))
	require.Error(t, Gate{"bogus": {Pct: 100}}.Validate(l), "unknown state")
	require.Error(t, Gate{"established": {Pct: 101}}.Validate(l), "out of range high")
	require.Error(t, Gate{"established": {Pct: -1}}.Validate(l), "out of range low")
}
...
func ruleSet() RuleSet {
	return RuleSet{Rules: []Rule{
		{When: Selector{Collections: []string{"docs"}}, Gate: Gate{"translated": {Pct: 100}, "established": {Pct: 50}}},
		{When: Selector{Locales: []string{"ja"}}, Gate: Gate{"translated": {Pct: 100}, "established": {Pct: 0}}},
		{When: Selector{Collections: []string{"legal"}, Locales: []string{"nb"}}, Gate: Gate{"established": {Pct: 100}}},
		{When: Selector{}, Gate: Gate{"translated": {Pct: 100}, "established": {Pct: 100}}}, // default
	}}
}
...
func TestResolve_MostSpecificWins(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, Gate{"established": {Pct: 100}}, g)

	// ui in de → only the default matches.
	g, ok = rs.Resolve("ui", "de")
	require.True(t, ok)
	assert.Equal(t, Gate{"translated": {Pct: 100}, "established": {Pct: 100}}, g)

	// ja anything → the locale rule.
	g, ok = rs.Resolve("ui", "ja")
	require.True(t, ok)
	assert.Equal(t, Gate{"translated": {Pct: 100}, "established": {Pct: 0}}, g)
}
...
func TestResolve_TieBreaksBySourceOrder(t *testing.T) {
	...
	rs := RuleSet{Rules: []Rule{
		{When: Selector{Collections: []string{"docs"}}, Gate: Gate{"established": {Pct: 50}}},
		{When: Selector{Locales: []string{"ja"}}, Gate: Gate{"established": {Pct: 0}}},
	}}
	g, ok := rs.Resolve("docs", "ja")
	require.True(t, ok)
	assert.Equal(t, Gate{"established": {Pct: 50}}, g, "first matching same-specificity rule wins")
}
...
func TestCoverage_AtLeastPct(t *testing.T) {
	...
	c := NewCoverage([]string{"draft", "translated", "translated", "established"})
	...
	assert.InDelta(t, 25, c.AtLeastPct(l, "established"), 1e-9, "1 of 4 established")
}
...
func TestCoverage_EmptyScopeIsFullyCovered(t *testing.T) {
	...
	assert.InDelta(t, 100, c.AtLeastPct(l, "established"), 1e-9, "vacuous scope passes any gate")
}

func TestEvaluate_PassFailAndShortfalls(t *testing.T) {
	l := TargetLadder()
	c := NewCoverage([]string{"translated", "translated", "established", "established", "draft"}) // 5 units

	// 100% translated? 4 of 5 = 80 < 100 → fail. reviewed ≥ 40%? 2/5=40 → pass.
	res := Evaluate(Gate{"translated": {Pct: 100}, "established": {Pct: 40}}, c, l)
	...
	// A gate it satisfies.
	assert.True(t, Evaluate(Gate{"translated": {Pct: 80}, "established": {Pct: 40}}, c, l).Pass)
}
...
func TestEvaluate_ZeroThresholdNotRequired(t *testing.T) {
	...
	res := Evaluate(Gate{"translated": {Pct: 100}, "established": {Pct: 0}}, c, l)
...
func TestEvaluate_ExactPercentageNoFloatTrip(t *testing.T) {
	...
	c := NewCoverage([]string{"established", "established", "translated"})
	assert.True(t, Evaluate(Gate{"established": {Pct: 66}}, c, l).Pass)
	assert.False(t, Evaluate(Gate{"established": {Pct: 67}}, c, l).Pass)
	// 80% reviewed over 5 units = exactly 80; an 80 gate must pass.
	c5 := NewCoverage([]string{"established", "established", "established", "established", "translated"})
	assert.True(t, Evaluate(Gate{"established": {Pct: 80}}, c5, l).Pass)
}
...
func TestProgress(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			gate: Gate{translated: {Pct: 100}, established: {Pct: 100}},
			cov:  cov(translated, translated),
			want: 50,
		},
		{
			name: "fully established clears both bars",
			gate: Gate{translated: {Pct: 100}, established: {Pct: 100}},
			cov:  cov(established, established),
			want: 100,
		},
		{
			name: "exceeding one bar never compensates for missing another",
			gate: Gate{translated: {Pct: 50}, established: {Pct: 100}},
			cov:  cov(translated, translated),
			want: 50,
		},
		{
...
func TestEvaluateBlockingIsTheLowestUnmetRung(t *testing.T) {
	ladder := TargetLadder()
	g := Gate{"translated": {Pct: 100}, "established": {Pct: 100}}
	...
	res = Evaluate(g, NewCoverage([]string{"established", "translated"}), ladder)
	assert.False(t, res.Pass)
	assert.Equal(t, "established", res.Blocking, "only the top rung is left")
