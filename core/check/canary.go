...
type Canary struct {
	...
	// accepts any finding.
	Expect string
	// MustFail, when set, requires that finding to fail, for an analyzer whose
	// limits grade what they catch into failing and reported findings.
	MustFail bool
}
...
//
// Every canary must produce a finding (of its Expect category, and failing
// when MustFail is set). With
...
func Probe(canaries []Canary, uncheckable string, run func(*model.Block) ([]Finding, error)) (CanaryOutcome, error) {
	...
	for _, c := range canaries {
		...
		if !flags(findings, c.Expect, c.MustFail) {
...
func flags(findings []Finding, category string, mustFail bool) bool {
	for _, f := range findings {
		if (category == "" || f.Category == category) && (!mustFail || f.Fails) {
			return true
		}
	}
	return false
