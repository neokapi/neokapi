...
func runVoiceVocab(t *testing.T, p *profile.VoiceProfile, text string) []check.Finding {
	t.Helper()
	b := &model.Block{ID: "b", Translatable: true, Source: []model.Run{{Text: &model.TextRun{Text: text}}}}
	tl := NewVoiceVocabCheckTool(p, nil)
...
func TestCorrectionBecomesEnforcedCheck(t *testing.T) {
	...
	assert.Empty(t, runVoiceVocab(t, p, text), "term is not forbidden yet")
