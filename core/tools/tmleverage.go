...
// TranslationMemory.Lookup; the plain-text MemoryProvider methods remain the
// fallback path.
...
// cloneRuns deep-copies a Run sequence so a content-memory entry's stored runs are
// never aliased into block targets (an in-memory content memory hands out its own
