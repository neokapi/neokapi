...
func TestWordCountTool(t *testing.T) {
	...
	wc, ok := model.AnnoAs[*tools.WordCountAnnotation](resultBlock, string(model.AnnoWordCount))
	require.True(t, ok)
...
func TestWordCountToolSkipsNonTranslatable(t *testing.T) {
	...
	resultBlock := result.Resource.(*model.Block)
	_, ok := model.AnnoAs[*tools.WordCountAnnotation](resultBlock, string(model.AnnoWordCount))
	assert.False(t, ok)
}
...
func TestWordCountCollector(t *testing.T) {
	...
	block1.SetAnno(string(model.AnnoWordCount), &tools.WordCountAnnotation{Source: 3, Targets: map[model.LocaleID]int{model.LocaleFrench: 4}})
...
func TestWordCountCollectorMultipleDocuments(t *testing.T) {
	...
	for _, uri := range []string{"a.html", "b.html", "c.html"} {
		...
		block.SetAnno(string(model.AnnoWordCount), &tools.WordCountAnnotation{Source: 2, Targets: map[model.LocaleID]int{model.LocaleFrench: 3}})
...
func TestWordCountCollectorPerLocaleProperties(t *testing.T) {
	...
	block := model.NewBlock("tu1", "Hello world")
	block.SetAnno(string(model.AnnoWordCount), &tools.WordCountAnnotation{Source: 2, Targets: map[model.LocaleID]int{model.LocaleFrench: 3, model.LocaleGerman: 2}})
