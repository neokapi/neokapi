...
func TestPseudoTranslateHasDefaultLocale(t *testing.T) {
	...
	for _, info := range reg.ListWithSchemas() {
		if info.Name == "pseudo-translate" {
			...
			assert.Contains(t, info.Produces, schema.IOFacet{Type: model.FacetTarget, Side: model.SideTarget})
			return
		}
	}
...
func TestTMLeverageHasSideEffects(t *testing.T) {
	...
	for _, info := range reg.ListWithSchemas() {
		if info.Name == "tm-leverage" {
			...
			assert.Contains(t, info.Produces, schema.IOFacet{Type: model.FacetTMMatch, Side: model.SideSource})
			return
		}
	}
