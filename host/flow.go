...
// termbase, for the active source and target locales. It reads the compiled
// .kapi/termbase.db when present, and otherwise falls back to the committed
// termbase_source (a .klftb document) directly — so the terminology gate works
// at check time in CI, where the gitignored .db doesn't exist. Returns nil when
// no termbase is in scope or it has no terms for the locale pair. The result is
// suitable for injection into a term-check tool config under the "glossary" key.
...
// projectConcepts loads the project's termbase concepts, preferring the compiled
// .kapi/termbase.db and falling back to the committed termbase_source (.klftb)
// when the db isn't present — the common case at check time in CI, where the db
// is a gitignored build artifact regenerated by up/apply.
func (a *App) projectConcepts(cmd Command) ([]sqltb.Concept, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open termbase source %q: %w", srcPath, err)
	}
	defer f.Close()
	file, err := klftb.Decode(f)
	if err != nil {
		return nil, fmt.Errorf("decode termbase source %q: %w", srcPath, err)
	}
