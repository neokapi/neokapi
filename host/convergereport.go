...
func (a *App) ApplyReviewDecisionAs(ctx context.Context, projectPath, sourceLang string, ref ReviewUnitRef, decision, note, by string) (bool, error) {
	...
	for _, u := range units {
		...
		if berr != nil {
			...
			return false, NoReaderError(berr, relativeToRoot(root, u.SourcePath), u.SourceFormat, a.discoveredPlugins()...)
		}
		if missing {
			continue
		}
...
func (a *App) RecordAIReviews(ctx context.Context, projectPath, sourceLang, locale, file string, reviews map[string]state.AIReview) (int, error) {
	...
	for _, u := range units {
		...
		if berr != nil {
			...
			return recorded, NoReaderError(berr, relativeToRoot(root, u.SourcePath), u.SourceFormat, a.discoveredPlugins()...)
		}
		if missing {
			continue
		}
...
func (a *App) ReviewUnitWithOptions(ctx context.Context, projectPath, sourceLang string, ref ReviewUnitRef, opts ReviewUnitOptions) (*ReviewUnitInfo, error) {
	...
	for _, u := range units {
		...
		if berr != nil {
			...
			return nil, NoReaderError(berr, relativeToRoot(root, u.SourcePath), u.SourceFormat, a.discoveredPlugins()...)
		}
		if missing {
			continue
		}
