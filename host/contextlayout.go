...
// contextReadResult counts what one pass over a layout put into the store.
type contextReadResult struct {
	// TermsFiles and Concepts count the terms bundles read and the concepts
	// they carried.
	TermsFiles int
	Concepts   int
	// MemoryFiles and Entries count the content-memory bundles read and the
	// entries they carried.
	MemoryFiles int
	Entries     int
	// VoiceFiles counts the voice profiles read.
	VoiceFiles int
	// Skipped counts the sources this build has no store for.
	Skipped int
}

// readContextLayout reads one `.kapi/` layout into a project store, through the
// same importers `kapi apply` writes with. It is what `kapi context import`
// runs, and the only reader of a context file in the checkout.
//
// proj is the recipe whose bindings add to the conventional sources, and is nil
// for a layout that belongs to no loaded recipe.
//
// Reading the same layout twice leaves the store as it was: every importer
// upserts by the identity the file carries.
func (a *App) readContextLayout(ctx context.Context, db *projectdb.DB, layout project.Layout, proj *project.KapiProject) (contextReadResult, error) {
	var res contextReadResult
	...
	for _, src := range sources {
		...
		n, cerr := a.readContextSource(ctx, db, layout.Root, src)
		if cerr != nil {
			return res, cerr
		}
		switch src.kind {
