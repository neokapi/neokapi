...
func TestSettleSourceStatesSurvivesAMissingReader(t *testing.T) {
	...
	states, _, unreadable, err := a.settleSourceStates(context.Background(), "", "en", model.TranslateAfterNone, units)
...
func TestSettleSourceStatesStillFailsOnARealReadError(t *testing.T) {
	...
	_, _, _, err := a.settleSourceStates(context.Background(), "", "en", model.TranslateAfterNone, units)
