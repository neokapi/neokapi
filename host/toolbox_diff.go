...
func (a *App) runRevisionDiff(ctx context.Context, fileA, fileB string, opts DiffOptions) error {
	ablocks, _, err := a.collectDiffBlocks(ctx, fileA, opts.TargetLocale)
	if err != nil {
		return err
	}
	bblocks, _, err := a.collectDiffBlocks(ctx, fileB, opts.TargetLocale)
	if err != nil {
		return err
	}
	...
	if opts.JSON {
		return emitRevisionJSON(fileA, fileB, alignment, opts.TargetLocale, ops, sum, differ)
	}
	if opts.Brief {
		if differ {
...
func (a *App) runCoverageDiff(ctx context.Context, file string, opts DiffOptions) error {
	blocks, err := a.collectCoverage(ctx, file, opts.TargetLocale)
	if err != nil {
		return err
	}
	...
	if opts.JSON {
		return emitCoverageJSON(file, opts.TargetLocale, ops, translated, untranslated, identical)
	}

	summary := fmt.Sprintf("%s [%s]: %d translated, %d untranslated, %d identical to source",
		DisplayName(file), opts.TargetLocale, translated, untranslated, identical)
	if opts.Brief {
		fmt.Println(summary)
		if drift > 0 {
			return ErrSilentExit
...
func disambiguatedSide(blocks []diffBlock) bool {
	for _, b := range blocks {
		if strings.Contains(b.ID, model.NameOrdinalSeparator) {
			return true
		}
	}
	return false
