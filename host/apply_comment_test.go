...
func TestProseP3_go(t *testing.T) {
	...
	t.Run("set-aside comments, other languages and directive text are refused and write nothing", func(t *testing.T) {
		for _, tc := range []struct {
			name, file, src, id, text string
			lines                     *fmtpkg.LineRange
			reason                    string
		}{
			{"a directive", "d.go", "package demo\n\n//go:noinline\nfunc Parse() {}\n", "func/Parse", "Parse parses.", &fmtpkg.LineRange{First: 3, Last: 3}, string(comment.RefusedDirective)},
			{"a generated file", "g.go", "// Code generated by gen. DO NOT EDIT.\n\npackage demo\n\n// Parse parses.\nfunc Parse() {}\n", "func/Parse", "Parse reads.", nil, string(comment.RefusedGenerated)},
			{"the cgo preamble", "c.go", "package demo\n\n// #include <stdio.h>\nimport \"C\"\n", "import/C", "Nothing.", &fmtpkg.LineRange{First: 3, Last: 3}, string(comment.RefusedCgoPreamble)},
			{"an example's output", "x_test.go", "package demo\n\nimport \"fmt\"\n\nfunc ExampleParse() {\n\tfmt.Println(1)\n\t// Output: 1\n}\n", "func/ExampleParse/comment", "Output: 2", &fmtpkg.LineRange{First: 7, Last: 7}, string(comment.RefusedExampleOutput)},
			{"a block comment", "b.go", "package demo\n\n/* Parse parses. */\nfunc Parse() {}\n", "func/Parse", "Parse reads.", nil, string(comment.RefusedBlockComment)},
			{"a comment read at other lines", "s.go", repairGo, "func/Parse", repairedParse, &fmtpkg.LineRange{First: 4, Last: 6}, string(comment.RefusedStale)},
			{"text that forms a directive", "n.go", repairGo, "func/Other", "nolint", nil, string(comment.RefusedDirective)},
			{"a YAML comment", "config.yaml", "# A note.\nkey: value\n", "comment/key", "Another note.", nil, string(comment.RefusedUnsupported)},
		} {
			t.Run(tc.name, func(t *testing.T) {
				isolateCheckExecution(t)
				file := writeCheckInput(t, t.TempDir(), tc.file, tc.src)
				out, _, err := runApplyChangeSet(t, NewEnvCommand(t.Context(), "apply"), false, commentEntry(file, tc.id, tc.lines, tc.text))
				assert.Equal(t, ExitGate, ExitCode(nil, err))
				edit := out.Comments[0].Edits[0]
				assert.Equal(t, commentRefused, edit.Status)
...
func TestApplyCommentEntryValidation(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"content hash": {changeEntry{Kind: kindComment, File: "p.go", ID: "func/Parse", Text: "x", ContentHash: "h"}, `"lines"`},
	} {
		err := validateContentWording([]changeEntry{tc.entry})
		require.Error(t, err, name)
		assert.Contains(t, err.Error(), tc.want, name)
	}
	require.NoError(t, validateContentWording([]changeEntry{{Kind: kindComment, File: "p.go", ID: "func/Parse", Text: "x"}}))
}
