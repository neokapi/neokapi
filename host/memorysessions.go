...
//
// These three results exist so `kapi memory sessions list/show/delete` go through
...
// MemorySessionRow is one import session in a listing.
type MemorySessionRow struct {
	ID         string `json:"id"`
...
// MemorySessionListOutput is the structured result of `kapi memory sessions list`.
type MemorySessionListOutput struct {
	Sessions []MemorySessionRow `json:"sessions"`
}

// FormatText renders the session table.
func (o MemorySessionListOutput) FormatText(w io.Writer) error {
	if len(o.Sessions) == 0 {
...
// MemorySessionDetail is the full record of one import session.
...
func (o MemorySessionDetail) FormatText(w io.Writer) error {
	rows := [][2]string{
		{"ID", o.ID},
		{"File key", o.FileKey},
...
// MemorySessionDeleteOutput is the structured result of removing a session record.
type MemorySessionDeleteOutput struct {
	ID      string `json:"id"`
	Deleted bool   `json:"deleted"`
}

// FormatText reports the removal.
func (o MemorySessionDeleteOutput) FormatText(w io.Writer) error {
	fmt.Fprintf(w, "Deleted session %s.\n", o.ID)
	return nil
}

// MemorySessionRows converts store sessions into listing rows.
func MemorySessionRows(sessions []memory.ImportSession) []MemorySessionRow {
	rows := make([]MemorySessionRow, 0, len(sessions))
	for _, s := range sessions {
		rows = append(rows, MemorySessionRow{
			ID:         s.ID,
			FileKey:    s.FileKey,
			Tool:       s.ToolName,
...
// MemorySessionDetailFrom converts a store session into its full report.
func MemorySessionDetailFrom(s memory.ImportSession) MemorySessionDetail {
