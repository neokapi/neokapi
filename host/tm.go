...
// WarnSuspectTokenEntries scans the content memory for entries whose variants disagree
...
func WarnSuspectTokenEntries(ctx context.Context, tm memory.Store, out io.Writer) {
	...
	fmt.Fprintf(out, "Warning: %d content memory entr%s with markup tokens ({=mN}) in some variants but not others — format-specific projections in format-neutral text leak into other surfaces (e.g. %s). Store these entries run-structured instead.\n",
